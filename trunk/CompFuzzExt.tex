...
In \secref{sec:imp HILL sketch} we show that a sketch that retains HILL entropy implies a sketch that retains nearly the same amount of min-entropy.  In \secref{sec:imp unp sketch}, we show that the computational unpredictability of a sketch is at most $n - |B_t(\cdot)|$.  Since random linear codes already achieve min-entropy $n(1-o(1)) - B_t(\cdot)$ on uniform inputs, it seems that relaxing security of sketches from information-theoretic to computational does not help---at least not on uniform inputs. \lnote{$B_t$ notation is not correct here -- it denotes the ball, not the logarithm of its volume} \lnote{Should we say something about efficiency of decoding?  Also, should we be clearer that these bounds are tight for uniform inputs and Hamming metric, but not necessarily otherwise? Perhaps just a pointer to the last part of this section (which could be called 3.3?}
...
Let $(\mathcal{M}, \dis)$ be a metric space that is $(s_{sam}, t)$-neighborhood samplable.  Let $(\sketch, \rec)$ be an HILL-entropy $(\mathcal{M}, m, \tilde{m}, t)$-secure sketch that is $(\epsilon, s_{sec})$-secure with error $\delta$.  Let $s_{rec}$ denote the size of the circuit that computes $\rec$.  If $s_{sec}\geq (t(s_{neigh}+s_{rec}))$,  then there exists a value $s$ and a set $C$ with $|C|\geq 2^{\tilde{m}-2}$  that is a $(t, 4(\epsilon+t\delta))$-Shannon code with recovery procedure $\rec(\cdot, s)$.
...
Define $D$ as:
\begin{enumerate}
\item Input $y\in\mathcal{M}, z \in\{0, 1\}^*, t$.
\item For all $1\leq t'\leq t$: 
\subitem  $y'\leftarrow \sample(y, t')$.
\subitem If $\rec(y', z) \neq  y$ output $0$.
\item Output $1$.
\end{enumerate}
