...
Let $n$ be a security parameter, $m = \poly(n), q = \poly(n)$.  Let $\alpha < q$ and $\beta = \omega(1)$.  All operations are computed $(\mod q)$ and we omit this notation.  
...
|\Pr[\D(\vA, \vA\vx+\ve) = 1] - \Pr[\D(\vA, \vu) =1]|> \epsilon.
\]
We build a distinguisher that breaks $\distLWE_{m, n, q, \chi}$.  Let $\vA'$ be uniform over $\Fq^{m\times n}$, $\vx'$ be uniform over $\Fq^n$ and $\ve'$ be drawn from $\chi^m$.  It suffices to build a distinguisher $\D'$ whose size at most \bnote{fill in} bigger than $\D$ where 
\begin{align}
...
\label{fig:perfectLWEreduction}
\end{figure}
The distinguisher $\mathcal{D'}$ has an advantage when $S$ is of rank $\alpha$.  This occurs with overwhelming probability~(we use techniques from Cooper~\cite{cooper2000rank}):
\begin{claim}
Let $\vS \overset{\$}\leftarrow \Fq^{\alpha \times \alpha+\beta}$ be randomly generated.  Then $\Pr[\rank(\vS)<\alpha]<1 - e^{-1/\beta} = \ngl(n)$.
\end{claim}
\begin{proof}
\bnote{Note happy with this writeup.  Draws on On the Rank of Random Matrices by C. Cooper}
The probability that all $\alpha$ rows are linearly independent is the successive probability that each row is linearly independent of the previous rows, this probability is:
\begin{align*}
\Pr[\rank(\vS) = \alpha] &= \prod_{i=1}^\alpha\left( 1-q^{i-1}/q^{\alpha+\beta}\right)
=\prod_{i=1}^\alpha\left( 1-q^{i-\alpha- \beta-1}\right)\\
...
We restrict analysis to the case where $\rank(\vS) = \alpha$.  
...
\]
A solution $\vx_T$ exists as $\vS_T$ is full rank and $\vx_T$ is uniformly distributed.
...
\end{align*}
We consider the product $(\vR|| \vS) \vx$.  It suffices to show that $(\vR|| \vS) \vx = (b_{m+1}^*,..., b_{m+\alpha}^*)$,
\begin{align*}
...
\begin{align*}
&\left|\Pr\left[\D'(\vA', \vu') = 1\right]- \Pr\left[\D'(\vA', \vb'=\vA'\vx + \ve)=1\right]  \right|\\
&\left|\Pr\left[\D'(\vA', \vu') = 1 | \rank(\vS) = \alpha \right]- \Pr\left[\D'(\vA', \vb'=\vA'\vx + \ve)=1 | \rank(\vS) = \alpha\right] \Pr[\rank(\vS) = \alpha] \right|\\
