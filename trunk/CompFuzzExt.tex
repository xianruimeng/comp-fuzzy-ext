...
Let $d$ be a positive constant and assume that $\dis(W, W')\leq t$ where $t\leq d(\frac{m}{n}-2)\log n$.  Then $\decode_t$ runs in expected time $O(n^{4d+3})$ operations in $\Fq$~(this probability is over the choice of random coins of \rep).  It outputs $X$ with probability $1-e^{-\Omega(n)}$ (this probability is over the choice of the random matrix  $\vA$ and random choices made by $\decode_t$).
\end{lemma}
...
let $\vS \overset{\$}\leftarrow \Fq^{\alpha \times (\alpha+\beta)}$ be uniformly generated.  Then $\Pr[\rank(\vS)=\alpha]\geq  1- \alpha q^{\beta}$.
\end{claim}
\begin{proof}
...
\leq \alpha \frac{q^{\alpha}}{q^{\alpha+\beta} }= \alpha q^{-\beta}.
\end{align*}
\end{proof}
...
\end{figure}
The distinguisher $D'$ has an advantage when $S$ is of rank $\alpha$.  This occurs with overwhelming probability:
\begin{claim}
...
\end{claim}
\begin{proof}
Direct application of \clref{cl:full rank matrix} when $\alpha = \poly(n)$ and $\beta = \omega(1)$ in $n$.
\end{proof}
\begin{corollary}
$
...
\end{eqnarray*}
(the second to last line follows from \clref{cl:full rank matrix} when $\alpha, \beta = O(n)$ \bnote{need to say this is a randomly generated matrix, but there are previous iterations.  I know we abort if we find a non full rank matrix but this still bothers me.}).  
