...
Set $X'_{n/2}$ be the first half bits of $X'$.		 
\item Output $w = X'_{n/2}$.
\end{enumerate}
\end{minipage} 
\end{tabular}
\end{center}
...
\begin{lemma}
Assume $(\sketch, \rec)$ is an $(\mathcal{M}, m, \tilde{m}, s_{sec}, s_{sketch}, s_{rec}, t)$-noisy computational conductor, and let $\rext:\mathcal{M}\times \{0,1\}^d\rightarrow \{0,1\}^\ell \times \{0, 1\}^d$ be an reconstructive extractor with $(\tilde{m}-\log 1/\epsilon, \epsilon)$-reconstruction $(C, D)$.  Then the following $(\gen, \rep)$ is an $(\mathcal{M}, m, \ell, t, s_{sec}/(|C|+|D|), s_{rec}, 5\epsilon)$-computational fuzzy extractor:
\begin{itemize}
