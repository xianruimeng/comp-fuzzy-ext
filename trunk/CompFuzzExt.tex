...
\[ \Pr_{y\leftarrow Y|z}[y'\leftarrow \sample(y, t') \wedge \rec(y',s) = y]> 1-2(\epsilon+t\delta).\]  
...
Thus,  $Y|z$ is a $(t, 2(\epsilon+t\delta))$-average error Shannon code with recovery $\rec(\cdot,s)$ and $2^{k-1}$ points.  The statement of the theorem follows by application of \lemref{lem:averageToMaximalError}.  
\end{proof}
