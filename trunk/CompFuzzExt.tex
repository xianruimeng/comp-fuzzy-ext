...
Let $\mathcal{C}$ be a set over space $\mathcal{M}$.  We say that $\mathcal{C}$ is a $(t,\epsilon)$-\emph{Shannon code} if there exists an efficient procedure $\rec$ such that for all $t'\le t$ and for all $c\in \mathcal{C}$, $\Pr[\rec(\neigh(c, t')) \neq c]\le \epsilon$. To distinguish it from the average-error Shannon code defined below, we will sometimes call it \emph{maximal-error} Shannon code.
\end{definition}
...
Shannon codes work for all codewords. We can also consider a formulation that works for an ``average'' codeword. 
 ...
 \begin{definition}
Let $C$ be a distribution over space $\mathcal{M}$.  We say that $C$ is a $(t,\epsilon)$-\emph{average error Shannon code} if there exists an efficient procedure $\rec$ such that for all $t'\le t$
...
We will say that a pair of procedures $(\sketch, \rec)$ is an \emph{unpredictability-entropy $(\mathcal{M}, m, \tilde{m}, t)$ secure sketch} that is $(\epsilon, s_{sec})$-hard with error $\delta$ if it satisfies \defref{def:secure sketch}, with the security requirement replaced by $H^{\unprlx}_{\epsilon, s_{sec}}(W| \sketch(W))\geq \tilde{m}$.  
Note that this notion is quite natural: combining such a secure sketch in a sketch-and-extract construction of  \lemref{lem:fuzzy ext construction} with a particular type of extractor (called a extractor 
...
Let $Z$ be an alphabet and $(\sketch, \rec)$ be an unpredictability-entropy $(Z^n, m, \tilde{m}, t)$-secure sketch that is $(\epsilon, s_{sec})$-secure with error $\delta$.  If $s_{sec} \geq t(|\rec|+n\log |Z|)$, then $\tilde{m}\leq n\log |Z| - \log |B_t(\cdot)| + \log(1-\epsilon -t\delta)$.
\end{theorem}
...
We now state a setting of parameters that yields a lossless construction.  The intuition is that our key is $X_{1,\dots, k}$ which is of size $k \log q$ while our input $w$ is of length $m\log \rho q$.  Though we need $m>n>k$ for decoding, dimensions of $w$ are sampled using fewer bits than dimensions of $X$.  That is, $\log \rho q < \log q$.  Thus by increasing the size of $q$ we can set $k\log q = m\log \rho q$, yielding a key of the same size as our source.    
...
\begin{proof} Security follows by Lemmas~\ref{lem:uniform LWE decision} and~\ref{lem:many hardcore bits} and  \thref{thm:blockLWE} .  Since \lemref{lem:uniform LWE decision} requires that $q = O(n^{3/2})$, setting  $\beta = \omega(1)$ is sufficient for $q^{-\beta} = \ngl(n)$ as required in \thref{thm:blockLWE}.  Efficiency follows by \lemref{lem:i t poly time}.  For a more detailed explanation of parameters see \secref{ssec:block params}. 
\end{proof}
