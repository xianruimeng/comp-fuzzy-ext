...
In \secref{sec:imp HILL sketch} we show that a sketch that retains HILL entropy implies a sketch that retains nearly the same amount of min-entropy.  In \secref{sec:imp unp sketch}, we show that the unpredictability of a sketch must decrease with the size of a ball in the metric space.  
...
Let $C$ be a distribution over space $\mathcal{M}$ with $\Hoo(C)\geq k$ and let $C$ be a $(t, \epsilon)$-average error Shannon code with recovery procedure $\rec$.  There is a set $C'$ with $|C'|\ge2^{k-1}$ that  is a $(t, 2\epsilon)$-(maximal error) Shannon code with recovery procedure $\rec$.
\end{lemma}
...
Let $(\mathcal{M}, \dis)$ be a metric space that is $(s_{neigh}, t)$-neighborhood samplable.  Let $(\sketch, \rec)$ be an HILL-entropy $(\mathcal{M}, m, \tilde{m}, t)$-secure sketch that is $(\epsilon, s_{sec})$-secure with error $\delta$.  Assume $s_{sec}= \Omega(t(s_{neigh}+s_{rec}))$, where $s_{rec}$ is the circuit size of $\rec$.  Then there exists a value $s$ and a set $C$ with $|C'|\geq 2^{\tilde{m}-2}$ that is a $(t, 4(\epsilon+t\delta))$-Shannon code with recovery procedure $\rec(\cdot, s)$.
...
For the Hamming metric, any Shannon code (in the stronger sense defined by us) can be converted into an information-theoretic secure sketch~(see \cite[Section 8.2]{DBLP:journals/siamcomp/DodisORS08} and references therein).  The idea is to use the code offset construction, but randomize the order of the bits of $w$ first, via  permutation $\pi$  (which  becomes part of the sketch and is applied to $w'$ during $\rec$). The formal statement is in the following Lemma.
\begin{lemma}
...
If $s_{sec}=\Omega(t(s_{rec} + n\log |Z|))$, then there exists a   $(Z^n, n\log |Z|, \tilde{m}-2,t)$ (information-theoretic) secure sketch with error
$4(\epsilon+t\delta)$. 
...
We say that $\distLWE_{n, m, q, \chi}$ is $(\epsilon, t)$-secure if no (probabilistic) distinguisher running in time $t$ can distinguish the $\lwe$ instances from uniform except with probability $\epsilon$.
\end{definition}
...
Let $n>0$ be an integer, $q\geq 2$, let $\chi$ be an error distribution over $\zq$ and $k\leq n$ be an integer.  Let $\vA\in \zq^{m\times n} , X\in\zq^n$ and $E$ be distributed according to $\chi^m$.  If $\distLWE_{(n-k, m, q, \chi)}$ is $\epsilon, s_{sec}$ hard then $H^{\hill}_{\epsilon, s_{sec}}(X_{1,..., k}| \vA, \vA X+E )\geq k\log q$.
\end{lemma}
\bnote{Need to check on gap in the reduction, the claim here is that there is no loss.  I think it will go from $\ngl, \poly$ to $\ngl', \poly'$}
...
\consref{cons:informal construction} is only useful if $\mathcal{I}_t$ can be efficiently implemented.  We now present a particular $\mathcal{I}_t$ and consider its efficiency:\bnote{Unique witness.  BAD!!!}
...
\bnote{Need to get rid of this unique witness stuff, talk about high distance with high probability.}
...
\begin{theorem}
\label{thm:blockLWE}
Let $n$ be a security parameter and define $q, m = \poly(n)$, let $\alpha < q$ and $\beta = \omega(1)$.  Let $W$ be a $m$-symbol fixing source over $\{0,1\}^{(m+\alpha)b}$ where $\Hoo(W) \geq mb$.  Then the $\distLWE_{n, m,q, W}$ assumption implies the $\distLWE_{n+\alpha + \beta, m+\alpha, q, W}$ w/ symbol fixing sources problem.
\end{theorem}
\bnote{We should add some exposition here.  Some intuition about the proof.}
...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in\Fq^+$ be a constant and consider the Hamming metric with block length $b = \log (2n^3 (c+d)/d)$.  Let $W$ be $\alpha$-symbol fixing source over $\zo^{((c+d)nb/d+\alpha b}$.  Assume GAPSVP and SIVP are hard to approximate within polynomial factors~(on lattices of dimension $n$) using quantum algorithms.   There is a setting of $q = \poly(n)$ such that \consref{cons:informal construction} is a lossless fuzzy extractor.  That is, $H^{\hill}_{\epsilon, s_{sec}}(X_{1,..., n/2} |\vA, \vA X+W) = H_\infty(W)$ for %the following parameters:
%\[
...
b_{m+i}^*-\sum_{j=1}^n R_{i, j} x_j', & n+1\leq i\leq n+\alpha\,.
\end{cases}
\]
We denote by $\vA^*$ the matrix before step~\ref{step:randomization}.  The vector $\vx$ is uniformly random and $\vb^* = \vA^*\vx + \ve$~(where $\ve$ is $\ve'$ with $\alpha$ 0s appended).  Thus, it remains to show that step~\ref{step:randomization} preserves a solution.
%For convenience, denote by $\vx^*$ the vector where $x_i^* = x_i'$ for $1\leq i \leq n$ and $x_{n+i}^* = b_{m+i}^*$ otherwise.  
For a matrix $\vA$ we will denote the $i$-th row by $\va_i$.  We now show that for all rows $i$, if $b_i^* = \va^*_i \vx+e_i$  then $b_i = \va_i \vx + e_i$.
...
We have the following for all rows $i$:
\begin{align*}
...
\section{Proof of Theorem \ref{thm:blockLWE} Revised}
\label{sec:proof of block theorem}

%\bnote{We need to be clear about notation here.  Why are $x, e$ sometimes bold and sometimes capital?} 
\begin{proof}
We assume that all of the fixed blocks are located at the end and their fixed value is $0$, that is, $W_{m+1}, ..., W_{m+\alpha} =0$.  If the blocks are fixed to some other value the reduction is essentially the same.  For a matrix $\vA$ we will denote the $i$-th row by $\va_i$.  

%It should be clear to the reader where the reduction differs for another block fixing source.\lnote{I don't like the phrasing; maybe we should say that if the blocks are fixed to some non-0 value, the reduction is essentially the same}

Let $n$ be a security parameter, $m = \poly(n), q = \poly(n)$.  Let $\beta = \omega(1)$.  All operations are computed $\mod q$ and this notation is omitted.  
Let $\D$ be a distinguisher that can distinguish $(m+\alpha)$ $\LWE$ instances from uniform over $n+\alpha+\beta$ variables where $\alpha$ of the equations have no error added.  That is, let $\vA$ be the uniform distribution over $\Fq^{(m+\alpha)\times(n+\alpha+\beta)}$, let $\vx$ be uniform over $\Fq^{(n+\alpha+\beta)}$, $\ve= (e_1,..., e_{m+\alpha})$ where $e_1,..., e_m$ are generated from error distribution $\chi$ and $e_{m+i} =0$ for $i=1,..,\alpha$. Furthermore, let $\vu$ be the uniform distribution over $\Fq^{m+\alpha}$.  We assume that, 
\[
|\Pr[\D(\vA, \vA\vx+\ve) = 1] - \Pr[\D(\vA, \vu) =1]|> \epsilon.
\]

We build a distinguish $\D'$ that given $m$ samples over $n$ variables distinguisher LWE instances from uniform.  Let $\vA'$ be uniform over $\Fq^{m\times n}$, $\vx'$ be uniform over $\Fq^n$ and $\ve'$ be drawn from $\chi^m$.  Our goal is to prepare a random block fixing instance from the random instance we are given.  The code for $\D'$ is given in \figref{fig:perfectLWEreduction}.

We first note that $\mathcal{D'}$ has an advantage only when $S$ is of rank $\alpha$.  This occurs with overwhelming probability:
\begin{claim}
Let $S \overset{\$}\leftarrow \Fq^{\alpha \times \alpha+\beta}$ be randomly generated.  Then $\Pr[\rank(S)<\alpha]<1 - e^{-1/\beta} = \ngl(n)$
\end{claim}
\begin{proof}
\bnote{Note happy with this writeup.  Draws on On the Rank of Random Matrices by C. Cooper}
The probability that all $\alpha$ rows is linearly independent is the successive probability that each row lies in the span of the previous rows, this probability is:
\begin{align*}
\Pr[\rank(\vS) = \alpha] &= \prod_{i=1}^\alpha\left( 1-q^i/q^{\alpha+\beta}\right)
=\prod_{i=1}^\alpha\left( 1-q^{i-\alpha- \beta}\right)\\
&\geq\prod_{i=1}^\alpha\left( 1- q^{-\beta}\right) = \left(1-\frac{1}{q^{\beta}}\right)^\alpha
\end{align*}
\end{proof}
Recall that $\alpha < q$, thus,
\begin{align*}
\left(1-\frac{1}{q^{-\beta}}\right)^\alpha &\geq \left(1-\frac{1}{q^{\beta}}\right)^q\\
&=\left(\left(1-\frac{1}{q^{\beta}}\right)^{q^{\beta}}\right)^{1/\beta}\approx e^{-1/\beta}
\end{align*}
\begin{corollary}
$
...
Thus, we only consider the case where $\rank(\vS) = \alpha$.  
It suffices to show that $\D'$ properly prepares a random instance in this case.  We show this in three claims: 
\begin{enumerate}
...
\item If $\rank(\vect{S})<\alpha$, stop and output a random bit.
\item Find a set of $\alpha$ linearly independent columns.  Let  $T$ be the set of these indices.
\item Initialize $\vW \in \Fq^{n\times \alpha+\beta}$ to be the zero matrix.
\item Let $\vb^* = (\vb', b^*_{m+1}, \ldots,b^*_{m+\alpha})$, where $(b^*_{m+1}, \ldots, b^*_{m+\alpha} )\overset{\$} \leftarrow \Fq^\alpha$.\label{step:b generation}
...
For the matrix $\vW$, the columns whose indices are not included in $T$ are random.  They are the addition of a randomly generated value and uncorrelated random values.

Thus, we restrict attention to the columns of $\vW$ whose indices are in $T$.  We denote this restricted matrix by $\vW_T$.  Fix a particular row $i$.  Then $\vW_{i, T} = \gamma_i \vS_{T}$.  Since $\vS_T$ is full rank and $\gamma_i$ is uniformly generated, $\vW_{i, T}$ is uniformly generated.

Thus, the entire matrix $\vA$ after step~\ref{step:randomization} is uniformly generated conditioned on $\rank(S) = \alpha$.
% In the randomized matrix $\vect{A}$, for $1\le i \le m$ and $n+1\le j \le n+\alpha$, $A_{ij}$ is assigned the value $\gamma_{i,(j - n)}$, which is a random value.  Similarly, for $m+1\le i \le m+\alpha$ and $n+1\le j \le n+\alpha$, each entry $A_{ij}$ is random due to the original construction of the matrix $\vA^*$.  Then before step \ref{step:randomization} the matrices $\vA', \vect{R}$ are random submatrices.  Thus, it remains to show that $\vA', \vect{R}$ remain random after step 5. For $1\le i \le m+\alpha$ and $1\le j \le n$, by construction $A_{ij} = A_{ij}'+\sum_{k=1}^\alpha \gamma_{i, k} R_{kj}$, which is a truly random number. Overall, each row vector $\va_{i}$ in $\vA$ is the sum of a random vector and a random linear combination independent vectors. Therefore, the entire matrix $\vA$ is a truly random matrix.  This completes the claim.
\end{proof}
\begin{claim}
\label{cl:random ax+e}
...
\begin{proof}
Denote by $\vx'$ the value such that $\vA'\vx'+\ve = \vb$.  
Then define $\vx$ as $x_i = x_i'$ if $1\leq i \leq n$ and $x_i$ is uniformly generated if $i\not\in T$.  Denote by $\vS_{T^c}$ the restriction of $\vS$ to the columns whose indices are not $T$~(similarly for the vector $\vx$).

Denote by $\vx_T$ the restriction of $\vx$ to the indices in $T$.  Then, define $\vx$ as the solution to the following system of equations~(a solution exists as $\vS_T$ is full rank).
\[
\vS_T  \vx_T = \begin{pmatrix} b_{m+1}  \\ \vdots \\b_{m+\alpha} \end{pmatrix}  - \vR \begin{pmatrix} \vx_1  \\ \vdots \\\vx_n \end{pmatrix}- \vS_{T^c} \vx_{T^c}  
\]
Denote by $\vA^*$ the matrix before step~\ref{step:randomization}.  The vector $\vx$ is uniformly random and $\vb^* = \vA^*\vx + \ve$~(where $\ve$ is $\ve'$ with $\alpha$ 0s appended).  Thus, it remains to show that step~\ref{step:randomization} preserves a solution.
%For convenience, denote by $\vx^*$ the vector where $x_i^* = x_i'$ for $1\leq i \leq n$ and $x_{n+i}^* = b_{m+i}^*$ otherwise.  
...
\end{align*}
We consider the product $(\vR|| \vS) \vx$.  It suffices to show that $(\vR|| \vS) \vx = (b_{m+1}^*,..., b_{m+\alpha+\beta}^*)$,
\[
(\vR|| \vS) \vx = \vR \begin{pmatrix} \vx_1  \\ \vdots \\\vx_n \end{pmatrix}+  \vS_{T^c} \vx_{T^c}  + \vS_T \vx_T =  \begin{pmatrix} b_{m+1}  \\ \vdots \\b_{m+\alpha} \end{pmatrix}
\]
This completes the proof of the claim.
\end{proof}
\begin{claim}
If the pair $(\vA', \vb')$ is uniformly distributed, then $\vb |\vA$ is uniformly distributed.
\end{claim}
\begin{proof}
\bnote{This needs to be checked.}
Let $\vb^*$ be the vector generated after step~\ref{step:fill in matrix}.  The only correlation between $\vb^*$ and $\vA^*$ is in the matrix $\vW$.  That is, for $1\leq i\leq n$, $b_i^* = b_i + \sum_{n<j \wedge j\not\in T} \vW_{i, j} x_j$, thus conditioning on $\vW$, $b_i^*$ is uniform assuming $b_i$ is uniform.  Thus $b^* | \vA^*$ is uniformly distributed.  Instead of showing that $\vb | \vA$ is uniformly distributed after step~\ref{step:randomization}, we show that $\vb |( \vA, \vA^*)$ is uniformly distributed.  Let $\vgamma$ represent the matrix formed by $\gamma_{i}$.  
...
\end{proof}
Together Claims~\ref{cl:randomMatrixDist},~\ref{cl:random ax+e}, and~\ref{clm:random b} show that when $\rank(\vS) = \alpha$ the distinguisher $\D'$ properly prepares the instance thus, 
\begin{align*}
&\left|\Pr\left[\D'(\vA', \vu') = 1\right]- \Pr\left[\D'(\vA', \vb'=\vA'\vx + \ve)=1\right]  \right|\\
& =\left|\Pr\left[\D(\vA, \vu)=1 | \rank(S) =\alpha \right]- \Pr\left[\D(\vA, \vb)=1 | \rank(S) =\alpha \right]  \right|\geq \epsilon - \ngl(n)
\end{align*}
\end{proof}
...
\begin{proof}
Let $C$ be a distribution with $\Hoo(C)\geq k$ and let $C$ be a  $(t,\epsilon)$-average error Shannon code with recovery procedure $\rec$.  This implies that for all $t'\le t$
\[
\sum_{w\in W} \Pr[W=w]\Pr[ w'\leftarrow \sample (w, t') \wedge \rec(w') \neq w]\leq \epsilon.
\]
For $w$ denote by $\epsilon_w = \Pr[w'\leftarrow \sample(w, t') \wedge \rec(w') \neq w]$.  \bnote{There is something weird going on here.  The Markov inequality doesn't connect to what is below it.}Then by Markov inequality:
\[
\Pr_{w\in W}[ \epsilon_w \leq 2\expe_{w\leftarrow W} [\epsilon_w ] ] = \Pr_{w\in W} [\epsilon_w \le 2\epsilon ] \geq \frac{1}{2}
\]
...
\begin{proof}
  Let $W$ be an arbitrary distribution of min-entropy $m$.  Let $Y_s$ be a collection of distributions over $\mathcal{M}$ giving rise to $Y$ such that $\Hav(Y | \sketch(W))\geq k$ such that 
\[ 
\delta^{\mathcal{D}_{s_{sec}}}((W, \sketch(W)), (Y, \sketch(W)))<\epsilon.
\]  
where  $s_{sec} = O(t(s_{neigh}+s_{rec}))$.  One such $Y$ must exist by the definition of conditional HILL entropy.
...
$\delta^D((Y, \sketch(W)), (Y, \sketch(W)))<\epsilon.$ then $\Pr[D(Y, SS(W)) = 1]>1-\epsilon-t\delta$.  By Markov inequality there exists a set $S_W$ such that $\Pr[S_W]\geq 1/2$ and $\Pr[ D(Y, s) =1 | \sketch(W) = s \wedge s\in S_W]> 1- 2(\epsilon + t\delta)$.  Consider the random variable $1_{S_W}$ that is $1$ when $s\in S_X$ and otherwise.  Then \bnote{I am splitting average min-entropy based on an indicator variable, everyone is okay with this?}
\[
\Hav(Y  | W) = -\log \left( \Pr[W\in 1_{S_W}] 2^{-\Hav(Y | W\in 1_{S_W})} + \Pr[W\not\in 1_{S_W}] 2^{-\Hav (Y|W\not\in 1_{S_W})} \right)
\] 
Rearranging terms:
\begin{align*}
\Pr[W\in 1_{S_W}] 2^{-\Hav(Y | W\in 1_{S_W})} &= 2^{-\Hav(Y  | W) } - \Pr[W\not\in 1_{S_W}] 2^{-\Hav (Y|W\not\in 1_{S_W})}\\
&=2^{-\Hav(Y  | W) } = 2^{-k}
\end{align*}
This means that $2^{-\Hav(Y | W\in 1_{S_W})} \geq 2^{-k}/\Pr[W\in 1_{S_W}] = 2^{-k+1}$.  That is, $\Hav(Y|W\in 1_{S_W})\geq k-1$.  Fix one value $s \in 1_{S_W}$ such that $\Hav(Y | s)\geq k-1$.
%This means at least one $x'$ where this equation holds.  Fix one such $x'$.  
%This implies that $D(W, \sketch(x'))\geq 1-\epsilon$.  
Thus, for all $t', 1\leq t'\leq t$ on a random $y$
\[ \Pr_{y\leftarrow Y_s}[y'\leftarrow \sample(y, t') \wedge \rec(y',s) = y]\geq 1-2(\epsilon+t\delta).\]  
...
\end{align*}
To ensure $f$ is a constant, we set $t = c \log n$ for some constant $c$ and that $k = n /g$ for some constant $g> 1$.  Finally we assume that $t = d \log n(m/n-1)$~(that is, there are only as many dimensions as necessary for decoding using \lemref{lem:i t poly time}):
\begin{align*}
n_0&< n -k \\
