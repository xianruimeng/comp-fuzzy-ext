...
Fuzzy extractors derive cryptographically strong keys from noisy sources.  They are traditionally built by combining a secure sketch and a randomness extractor.  Secure sketches remove noise from the source while revealing little information about the source.  Randomness extractors convert high entropy sources to the uniform distribution.  %Secure sketches necessarily reveal some information about the noisy secret, and thus the derived key in fuzzy extractors has lower entropy than the source.  The minimal entropy loss 
...
\textbf{Alternative Approach}  Computational extractors~\cite{krawczyk2010cryptographic, barak2011leftover, dachman2012computational} have the same goal in the setting without errors.  One way to achieve a computational (fuzzy) extractor is by using a computational extractor instead in place of the information theoretic extractor.  In particular, one technique is ``extract-then-expand'', where the output of a randomness extractor is stretched using a pseudorandom generator.  This construction is only possible if the entropy loss due to a secure sketch leaves enough truly random bits.  
...
In \secref{sec:imp HILL sketch} we show that a sketch that retains HILL entropy implies a sketch that retains nearly the same amount of min-entropy.  In \secref{sec:imp unp sketch}, we show that the computational unpredictability of a sketch is at most $\log |\mathcal{M}| - \log |B_t(\cdot)|$.  The code-offset sketch instantiated with a random linear code~(in the Hamming metric)  achieve min-entropys $\log |\mathcal{M}|(1-o(1)) - \log B_t(\cdot)$ on uniform inputs\lnote{this point is confusing -- it's not the random linear codes, it's some construction applied to them in order to get a sketch -- we should cite appropriately and explain more clearly here and in 3.2}, so it seems that relaxing security of sketches from information-theoretic to computational does not help---at least not on uniform inputs.\bnote{I changed this bound to be size of metric space before it was just n which was wrong.  Do we prefer $n\log Z$?} \lnote{I think your change is fine}  In \secref{ssec:avoiding bounds}, we discuss mechanisms for avoiding these bounds.
...
Note this notion is quite natural: combining such a secure sketch via  \lemref{lem:fuzzy ext construction} with a particular type of extractor (called a extractor 
...
\end{theorem}
In particular, if the input is uniform, the entropy loss is about $\log |B_t(\cdot)|$.  Essentially the same entropy loss can be achieved with information-theoretic secure sketches, by using the code-offset construction\lnote{cite here, and is it the randomized construction or the deterministic one that we should use here?} applied to the best possible codes (e.g., random codes\lnote{cite performance of codes?}).  Thus, unpredictability-entropy secure sketches cannot improve on the best possible information-theoretic entropy loss (at least for uniform inputs).  However, because the best possible codes may not have efficient decoding, it is conceivable that unpredictability entropy secure sketch could achieve lower entropy loss than is possible with efficient information-theoretic sketches.
...
\consref{cons:informal construction} is only useful if $\decode_t$ can be efficiently implemented.  We now present a particular $\decode_t$ and consider its efficiency:
...
Thus, using the assumptions of \thref{thm:lossless secure conductor log}, we can obtain a computational extractor.  This means we could  build a computational fuzzy extractor using: 1) an information-theoretic secure sketch 2) a computational extractor.
We call the paradigm of running a secure sketch and using a computational extractor \emph{sketch-then-cextract}.  

Any improvement in computational extractors yields an improvement to the \emph{sketch-then-cextract} construction of a computational fuzzy extractor.  Similarly, a computational fuzzy extractor is also a computational extractor~(ignoring the \rep functionality).  Thus, improving computational fuzzy extractors may improvement computational extractors.  

The main advantage to analyzing the construction as a whole~(instead of \emph{sketch-then-cextract}) is that security need not depend on the decoding radius $t$.  In \emph{sketch-then-cextract} paradigm, the computational extractor must be able to handle sources with $\Hoo(W| \sketch(W))$.  As shown in \secref{sec:defCompFuzzyExtractors}, this value depends on bounds from error-correcting codes.  In our construction, the decoding radius does not affect the security of the construction.  The decoding radius only impacts the running time of the decoding algorithm.  Conceivably, a computational fuzzy extractor could be built where only correctness depends on $t$~(not efficiency or security).
...
\item Output $\rec(x, s)$.
\end{itemize}
Note that $|\mathcal{I}| =  |\rec|+ s_{sam}$.  Let $X'$ the random variable corresponding to the sample $x$ taken by $\decode_t$.
