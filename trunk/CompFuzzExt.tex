...
We build \emph{lossless} computational fuzzy extractors, where the derived key $r$ is as long as the entropy of the source $w$\footnote{If a source has enough entropy, the entropy losses of a information-theoretic fuzzy extractor can be overcome by running a pseudorandom generator on the derived key $r$.  However, when the source is low entropy, these entropy losses may not leave sufficient entropy to run a pseudorandom generator.}. % without conditioning on anything---i.e., before $\gen$ is run (see~\thref{thm:lossless secure conductor log})).  
