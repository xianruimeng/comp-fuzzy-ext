...
One of the primary building blocks for authentications is reliable key derivation.  Fuzzy extractors~\cite{DBLP:journals/siamcomp/DodisORS08} achieve reliable key derivation from noisy sources.  (\bnote{Are these references for fuzzy extractors or secure sketches?} as well as in other contexts, e.g.,~\cite{Boyen05secureremote,dodisWichs2009,chandran2010privacy}).  The setting 
...
the Reproduce ($\rep$) algorithm to reproduce $r$ given $p$ and some value $w'$ that is close (according to some predefined metric, such as Hamming distance) to $w$. 
Crucially for security,  knowledge of $p$ should not reveal $r$; that is, $r$ should be uniformly distributed conditioned on $p$.  This feature is needed because $p$ is not secret: for example, in a single-user setting (where the user wants to recover the original reading $w$ from a subsequent reading $w'$), it would be stored in the clear; and in a key agreement application~\cite{Boyen05secureremote} (where two parties have $w$ and $w'$, respectively), it would be transmitted between the parties.

As defined in \cite{DBLP:journals/siamcomp/DodisORS08}, the entropy loss of a fuzzy extractor is the difference in the entropy of $w$ and the length of the key $r$.  A goal of fuzzy extractor constructions is to minimize the entropy loss, increasing the security of the resulting application.  The resulting secret key may be too short to be useful if the entropy loss is too high.  Traditionally, fuzzy extractors are defined as information-theoretic objects.  In the information-theoretic setting, some entropy loss is necessary as the value $p$ contains enough information to reproduce $r$ from any close value $w'$.  
...
The minimum entropy loss for secure sketches has been quantified in past work.  Even if we don't require perfect correctness (i.e., allow $\rep$ to err with small probability~\cite[Section 8]{DBLP:journals/siamcomp/DodisORS08}), the entropy loss of a secure sketch that can reconcile $t$ errors with high probability is bounded by the redundancy of
the best code that can correct $t$ random errors with high probability~\cite[Proposition 8.2]{DBLP:journals/siamcomp/DodisORS08}.  Known constructions of fuzzy extractors use a secure sketch suffer from this bound.
...
To see if entropy loss in fuzzy extractors can be improved, we explore fuzzy extractors and secure sketches with the relaxed secrecy requirement that is computational rather than information-theoretic.  Our overall goal is to produce a fuzzy extractor~(with small entropy loss), where the key $r$ is pseudorandom even conditioned on $p$.
...
Another possible relaxation is to require that the value $w$ is still unpredictable conditioned on $s$.  Using an extractor with a reconstruction would still yield a pseudorandom key, see~\cite[Lemma 6]{DBLP:conf/eurocrypt/HsiaoLR07}.  We consider sketches that preserve the weaker notion of unpredictability for the Hamming metric.  In \thref{thm:imp of unp entropy}, we show  a significant loss in unpredictability is necessary for a secure sketch.  The entropy loss is proportional to the volume of the ball of radius $t$.

Both of these impossibility results arise because a sketch must function like an error-correcting code.  To avoid these impossibility results, we give up on reconstructing $w$ and focus directly on the entropy loss in fuzzy extractors.  
...
We build \emph{lossless} computational fuzzy extractors, in which $r$ is pseudorandom conditioned on $p$ and $r$ is as long as the entropy of $w$.  % without conditioning on anything---i.e., before $\gen$ is run (see~\thref{thm:lossless secure conductor log})).  
We demonstrate how to build such lossless fuzzy extractors~(\consref{cons:informal construction}) based on the Learning with Errors~(LWE) assumption due to Regev~\cite{regev2005LWE, regevLWEsurvey},  which says that decoding a random linear code is computationally difficult.  
...
Our lossless fuzzy extractor is built via the code-offset construction~\cite{JW99},\cite[Section 5]{DBLP:journals/siamcomp/DodisORS08} already used in prior work.  To be able to apply the LWE assumption, we use a random linear code (unfortunately, our decoding algorithm can only reconcile a logarithmic number of differences).  We utilize the recent result of D\"{o}ttling and M\"{u}ller-Quade~\cite{dottling2012} that shows security of LWE when the error vector comes from the uniform distribution, with each coordinate ranging over a small interval.  The LWE error vector, in our case, is the input $w$ itself.  To utilize the result of~\cite{dottling2012}, we need to assume that the input comes from the uniform distribution. We are able to relax this limitation somewhat and allow $w$ to come from a symbol-fixing source~\cite{KZ07} (where each dimension is either uniform or fixed). This relaxation requires new results about the hardness of LWE when samples have a fixed~(and adversarially known) error vector, which may be of independent interest~(\thref{thm:blockLWE}).
...
\bnote{Need to rework this statement based on addition of $\beta$ above.}
\begin{theorem}
\label{thm:lossless block sketch log}
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in\Fq^+$ be a constant and consider the Hamming metric with block length $b = \log (2n^3 (c+d)/d)$.  Let $W$ be $\alpha$-symbol fixing source over $\zo^{((c+d)nb/d+\alpha b})$.  Assume GAPSVP and SIVP are hard to approximate within polynomial factors~(on lattices of dimension $n/2$) using quantum algorithms.   There is a setting of $q = \poly(n)$ such that \consref{cons:informal construction} is a lossless fuzzy extractor.  That is, $H^{\hill}_{\epsilon, s_{sec}}(X_{1,..., n/2} |\vA, \vA X+W) = H_\infty(W)$ for %the following parameters:
%\[
...
\]
Let $C'$ denote the of  set all $c\in C$ where $\epsilon_c\leq 2\epsilon$.  Note that $\Pr_{c\leftarrow C}[c\in C']\geq 1/2$.  Since $H_\infty(C)\geq k, |C'|\geq 2^{k-1}$~(otherwise $C'$ would contain total weight less that $2^{k-1}\frac{1}{2^k} = 1/2$).  This completes the proof of the~\lemref{lem:averageToMaximalError}.
