...
\end{itemize}
\end{lemma}
The main parameter we will be concerned with is the entropy loss of the construction.  In this paper, we ask whether a smaller entropy loss can be achieved by considering a fuzzy extractor with a computational security requirement.  We therefore relax the security requirement of \defref{def:fuzzy extractor} to require a pseudorandom output instead of a truly random output.\footnote{In addition, we modify the notation so that we can specify a general class of sources for which the fuzzy extractor is designed to work (rather than limiting ourselves to the class of that consists of all sources of a given min-entropy $m$).}
...
\begin{construction}
Let $n$ be a security parameter and let $m> n$.  Let $q$ be a prime. %Let $W = W_1,..., W_m$ and let $b$ be a constant and let $W_i \in \zo^b$ for all $i$.  
...
Let $n$ be a security parameter.  Let $q = q(n) = \poly(n)$ be a prime and $m = m(n) = \poly(n)$ be an integer. Let $\gamma\in (0, 1)$ be a constant and let $\rho\in (0,1)$ such that $\rho q \geq 2n^{1/2+3\gamma}m$. If the approximate decision-version of the shortest vector problem (GAPSVP) and the shortest independent vectors problem (SIVP) are hard~(on lattices of dimension $n/2$) within a factor of $\tilde{O}(n^{1/2+3\gamma}m/\rho)$ for quantum algorithms in the worst case, then, for $\chi$ the uniform distribution over $[-\rho q, \rho q]$, then $\distLWE_{n, m, q, \chi}$ is secure.
\end{lemma}
...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.    Let $d\in \mathbb{Z}^+$ be a constant and consider the Hamming metric over the alphabet $Z$ of size $2^b$, where the  block length $b = \log (2n^3 (c+2d)/d)=O(\log n)$.  Let $W$ be uniform over $\mathcal{M}=Z^m$, where $m={(c+2d)n/d}=O(n)$.  If GAPSVP and SIVP are hard to approximate within polynomial factors (on lattices of dimension $n/4$) using quantum algorithms, then there is a setting of $q = \poly(n)$ such that for any polynomial $s_{sec}=\poly(n)$ there exists $\epsilon=\ngl(n)$ such that the following holds: \consref{cons:informal construction} is a $(\M, W, bm, t)$-computational fuzzy extractor that is $(\epsilon, s_{sec})$-hard with error $\delta = e^{-\Omega(n)}$.
...
\begin{definition}
Let $W = (W_1,..., W_{m+\alpha})$ be a distribution where each $W_i$ takes values over an alphabet $Z$ of size  $2^b$.  We say that it is a $(m+
\alpha, m, 2^b) $ \emph{symbol fixing source} if for $\alpha$ indices $i_1, \dots, i_\alpha$, the symbols $W_{i_\alpha}$ are fixed, and the remaining $m$  symbols are chosen uniformly at random.  Note that $H_\infty(W)=mb$.
\end{definition}
\lnote{do we need exactly $\alpha$ fixed, or can we do up to $\alpha$ fixed?  The KZ07 definition requires exactly. We shouldn't chagne the definition, but may restate the theorem to include all symbol fixing source of entropy $mb$ and up}
...
Let $n$ be a security parameter and define $m = \poly(n)$ and $q\geq 2$ be integers.  Let $W = W_1||...||W_{m+\alpha}$ be $(m+
\alpha, m, 2^b)$ symbol-fixing source over $\{0,1\}^{(m+\alpha)\ell}$. For a vector 
...
Let $n$ be a security parameter.  Let $q=q(n)=\poly(n)$ be a prime and  $m = m(n)=\poly(n)$ be an integer.  Let $W = W_1||...||W_{m+\alpha}$ be a$(m+
\alpha, m, 2^b)$ symbol fixing source over an $Z^{m+\alpha}$ for an alphabet $Z$\lnote{what alphabet?  it should be integers} of size $2^b$. For a vector $X \leftarrow \Fq^n$, we denote $A_{X, W}$ be the distribution obtained by choosing a uniform $\vA\in\Fq^{(m+\alpha)\times (n+\alpha)}$ and outputting $\vA X + W$. The decisional learning with errors problem $\distLWE_{n+\alpha, m+\alpha, q, W}$ with symbol fixing errors is to distinguish samples chosen according $A_{X, W}$ \lnote{why is $X$ a subscript in this notation by A is uniform? why not both?} for a uniformly random $X \in \Fq^{n+\alpha}$ from samples chosen according to the uniform distribution over $\Fq^{n+\alpha} \times \Fq$.
\end{problem}
...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in\mathbb{Z}^+$ be a constant and consider the Hamming metric over the alphabet $Z$ of size $2^b$, where the block length $b = \log (2n^3 (c+2d)/d) = O(\log n)$.  Let $\alpha \leq n/3$ and $m=(c+2d)n/d = O(n)$, and let $\mathcal{W}$ be the class of all $m$-symbol fixing sources over $\zo^{(m+\alpha)b}$.  If GAPSVP and SIVP are hard to approximate within polynomial factors~(on lattices of dimension $n/6$) using quantum algorithms, then  there is a setting of $q = \poly(n)$ such that for any polynomial $s_{sec} = \poly(n)$
 there exists $\epsilon = \ngl(n)$ 
such that the following holds: \consref{cons:informal construction} is a $(\M, \mathcal{W}, bm, t)$-computational fuzzy extractor that is $(\epsilon, s_{sec})$-hard with error $\delta = e^{-\Omega(n)}$.
