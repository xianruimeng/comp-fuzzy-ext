...
Shannon codes work for all codewords. We can also consider a formulation that works for an ``average'' codeword. \lnote{is there a source we can reference for the terms average-error and maximal-error?}\bnote{I think the right thing to reference is Cover-Thomas, but I don't have a copy to check right now}
...
\]  
where  $s_{sec} \geq t(s_{neigh}+s_{rec})$.  One such $Y$ must exist by the definition of conditional HILL entropy.
