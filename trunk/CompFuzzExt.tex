...
Unfortunately, unpredictability entropy must also decrease with $t$.  That is, $\forall Y$ such that $\delta^{\mathcal{D}_{s_{sec}}}((W, \sketch(W)) , (Y, \sketch(W))$ our ability to predict $Y$ increases as $|B_t(\cdot)|$ increases.  The formal statement is below~(proof in \secref{sec:proof of imp unp entropy})
\begin{theorem}
...
\end{itemize}
Since $(\rec, \sketch)$ has error $\delta$ we know that $\forall w, w'\in\zo^b$ such that \[\dis(w, w')\leq t, \Pr[s\leftarrow \sketch(w) \wedge \rec(w', s) =  w] \geq 1-\delta.\]  This immediately implies that $\Pr[\rec(W, \sketch(W))  ]\geq 1-\delta$.  The statement of the claim follows as 
\begin{align*}
\Pr[D(W, \sketch(W)) = 1]  - \Pr[D(Y, \sketch(W))] &=\\
\Pr[\rec(B_t(W), \sketch(W) )= W] - \Pr[\rec(B_t(Y), \sketch(W)) = Y] &> (1-\delta)-(1-\delta - \epsilon)>\epsilon.
\end{align*}
\end{proof}
We now consider a fixed $w\leftarrow W, s\leftarrow \sketch(w), y\leftarrow Y_{s}, y'\leftarrow B_t(y)$ and define the quantity $P_{y' , s, y} = \Pr[\rec(y', s) =y]$.  \clref{clm:y is recoverable} says that 
\[
 \Pr[w\leftarrow W, s\leftarrow \sketch(w), y\leftarrow Y_s] \sum_{y'\in \zo^b} \Pr[B_t(y) =y' | Y = y] P_{y',  s, y} \geq 1-\delta -\epsilon.
\]
We note that for any particular $y, y'$ such that $\dis(y, y')\leq t, \Pr[B_t(y) = y' | Y=y] = 1/|B_t(\cdot)|$ and if $\dis(y, y')>t, Pr[B_t(y) = y' | Y=y] = 0$.  We restate this as $y, y', \Pr[B_t(y) = y' | Y=y] \leq 1/|B_t(\cdot)|$
...
We now show that $\mathcal{I}$ predicts $Y$, defining :
\begin{align*}
...
&\geq  \Pr[w\leftarrow W, s\leftarrow \sketch(W), y\leftarrow Y_s] \sum_{y'\in \zo^b} \frac{|B_t(\cdot)|}{2^b} \Pr[B_t(y) = y' | Y=y ] P_{y', s, y}\\
