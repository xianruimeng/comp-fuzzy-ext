...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in\mathbb{Z}^+$ be a constant and consider the Hamming metric with block length $b = \log (2n^3 (c+2d)/d)$.  Let $\alpha \leq n/3$ and $m=(c+2d)n/d$, and let $W$ be $m$-symbol fixing source over $\zo^{(m+\alpha)b}$.  If GAPSVP and SIVP are hard to approximate within polynomial factors~(on lattices of dimension $n/6$) using quantum algorithms, then  there is a setting of $q = \poly(n)$ such that for any polynomial $s_{sec} = \poly(n)$
 there exists $\epsilon = \ngl(n)$ such that the following holds: 
\consref{cons:informal construction} is a $(\M, mb, mb, t)$-computational fuzzy extractor that is $(\epsilon, s_{sec})$-hard with error $\delta = e^{-\Omega(n)}$. Sketch takes $O(n^3)$ operations over $\Fq$, and recover takes expected time $O(n^{4d+3} \log n)$ operations over $\Fq$.
\end{theorem}
