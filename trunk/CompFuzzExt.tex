...
\item Find a set of $\alpha$ linearly independent columns in $\vS$.  Let $T$ be the set of indices of these columns.
\item For all $j\notin T$, $1\le j \le \alpha+\beta$:\lnote{swapped $i$, $j$ and shifted}\label{step:fill in matrix}
\subitem Choose $x_{j+n}\overset{\$}\leftarrow \Fq$ uniformly at random.  
\subitem For $i=1,..., m$:
...
\subitem Set $b_i \leftarrow b^*_i + \gamma_i (b^*_{m+1},..., b^*_{m+k})^{\mathsf{T}}$
...
If the pair $(\vb', \vA')$ is uniformly distributed, then $(\vb, \vA)$ is uniformly distributed \lnote{this can't possibly be true because $\vA$ is not uniform, because $S$ is full-rank}.
\end{claim}
\begin{proof}
