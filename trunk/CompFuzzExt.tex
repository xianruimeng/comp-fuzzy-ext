...
Fuzzy extractors derive cryptographically strong keys from noisy sources.  They are traditionally built by combining a secure sketch and a randomness extractor.  Secure sketches remove noise from correct errors in the source while revealing little about the source.  Randomness extractors convert high entropy sources to the uniform distribution.  %Secure sketches necessarily reveal some information about the noisy secret, and thus the derived key in fuzzy extractors has lower entropy than the source.  The minimal entropy loss 
...
Secure sketches necessarily reveal some information about the noisy secret, and thus the derived key in fuzzy extractors has lower entropy than the source.  
The minimal entropy loss is dictated by bounds on error-correcting codes.  Fuzzy extractors suffer from similar bounds imposed by coding theory.

We ask whether these bounds can be overcome by considering computational, rather than information-theoretic, secure sketches and fuzzy extractors.  %The answer we provide an answer for both secure sketches and fuzzy extractors.  
The answer to this question is mixed, For secure sketches the answer is no, and for fuzzy extractors the answer is yes.
\begin{itemize}
\item \textbf{Secure Sketches:} We show that a secure sketch that retains a large amount of HILL pseudo entropy implies a secure sketch which retains a similar amount of information-theoretic entropy.  We also consider a weaker computational notion of security, the unpredictability of a source conditioned on output of the secure sketch.  We show that in order to correct $t$ errors, the unpredictability of a source must decrease with the volume of a ball of radius $t$.
\item \textbf{Fuzzy Extractors:}  We show these impossibility results can be overcome by analyzing fuzzy extractors directly.  Namely, we show how to build a \emph{lossless} computational fuzzy extractor for uniform sources and certain other sources of high entropy~(namely, symbol-fixing sources, where each dimension is either uniform or fixed), based on the hardness of the Learning with Errors~(LWE) problem.  This construction requires us to show that the decision version of LWE is secure when a small number of dimension has no error---a result that may be of independent interest.
\end{itemize}
%mixed, and building them based on the Learning with Errors (LWE) problem.  We thus obtain the first secure sketch with no loss in entropy.   More specifically, our contributions are as follows:
%
%\begin{itemize}
...
Let $(W, X)$ be a pair of random variables.  $W$ has 
\emph{relaxed HILL entropy} at least $k$ conditioned on $X$,
denoted $H^{\hillrlx}_{\epsilon, s_{sec}}(W|X)\geq k$ if there exists a joint distribution $(Y, Z)$, such that $\tilde{H}_\infty(Y|Z)\geq k$ and $\delta^{\mathcal{D}_{s_{sec}}} ((W, X),(Y,Z))\leq \epsilon$.
\end{definition}
...
The intuition for building error-correcting codes from HILL-entropy secure sketches is as follows.  In order to have  $H^{\hillrlx}_{\epsilon, s_{sec}}(W|\sketch(W))\ge \tilde{m}$, there must be a distribution $Y, Z$ such that $\Hav(Y | Z)\geq \tilde{m}$ and $(Y, Z)$ is computationally indistinguishable from $(W, \sketch(W))$.  Sample a sketch $s\leftarrow \sketch(W)$. We know that $\sketch$ followed by $\rec$ likely succeeds on $W|s$  (i.e., $\rec (w', s) = w$ with high probability for $w\leftarrow W|s$ and $w'\leftarrow B_t(w)$).  %So, by indistinguishability, it must also succeed on $Y$. 
 Consider the following experiment: 1) sample $z\leftarrow Z$, 2) draw $y\leftarrow Y|z$ and 3) $y'\leftarrow B_t(y)$. By indistinguishability $\rec (y',z) = y$ with high probability.
 This means we can construct a large set $C$ from the support of $Y|z$.  $C$ will be an error correcting code and $\rec$ an efficient decoder.  We can then use standard arguments to turn this code into an information theoretic sketch.  
...
\begin{enumerate}
\item Input $\vA , \vc = \vA X + W - W'$
\item Randomly select rows without replacement $i_1,..., i_{2n}\leftarrow [1,m]$.  
\item Restrict $\vA, \vc$ to rows $i_1,...,i_{2n}$, denote this as $\vA_{i_1,...,i_{2n}}, \vc_{i_1,...,i_{2n}}$.
\item Find $n$ rows of $\vA_{i_1,..., i_{2n}}$ that are linearly independent.  
Denote by $\vA', \vc'$ the restriction of $\vA_{i_1,..., i_{2n}}, \vc_{i_1,..., i_{2n}}$ (respectively) to these rows.  If no such rows exist go to step (2).
\item Compute $X' = \vA'\vc'$.  If $C- AX'$ has more than $t$ nonzero coordinates, go to step (2).
\item Output $X'$.
\end{enumerate}
\end{construction}
...
Let $\vA, \vA X+W$ be a  $\distLWE_{n,m, q, \chi}$ instance where $m\geq 3n$.  Assume that $W$ is split into $m$ blocks of length $b$ and let $\dis$ be the Hamming distance over alphabet $2^b$.  Let $d$ be a constant and assume that $\dis(W, W')\leq t$ where $t\leq d(\frac{m}{n}-1)\log n$.  Then $\mathcal{I}_t$ runs in expected time $O(n^{4d}(n^3\log q))$ on input distribution $\vA, \vA X+W - W'$ and outputs $W$ with probability $1-e^{-\Omega(n)}$ when $\vA$ is a random matrix.
\end{lemma}
...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in \mathbb{Z}^+$ be a constant and consider the Hamming metric with block length $b = \log (2n^3 (c+2d)/d)$.  Let $W$ be uniform over $\zo^{(c+2d)nb/d}$.  If GAPSVP and SIVP are hard to approximate within polynomial factors~(on lattices of dimension $n/4$) using quantum algorithms, then there is a setting of $q = \poly(n)$ such that \consref{cons:informal construction} is a lossless fuzzy extractor.  That is, $H^{\hill}_{\epsilon, s_{sec}}(X_{1,..., n/2}|\vA, \vA X+W) = H_\infty(W)$
...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in\mathbb{Z}^+$ be a constant and consider the Hamming metric with block length $b = \log (2n^3 (c+2d)/d)$.  Let $\alpha \leq n/3$, and let $W$ be $m$-symbol fixing source over $\zo^{\left((c+2d)n/d+\alpha \right)b})$.  Assume GAPSVP and SIVP are hard to approximate within polynomial factors~(on lattices of dimension $n/6$) using quantum algorithms.   There is a setting of $q = \poly(n)$ such that \consref{cons:informal construction} is a lossless fuzzy extractor.  That is, $H^{\hill}_{\epsilon, s_{sec}}(X_{1,..., n/3} |\vA, \vA X+W) = H_\infty(W)$ for %the following parameters:
%\[
...
\begin{proof}
  Let $W$ be an arbitrary distribution of min-entropy $m$.  Let $(Y, Z)$ be a joint distribution such that $\Hav(Y | Z)\geq k$ and
\[ 
\delta^{\mathcal{D}_{s_{sec}}}((W, \sketch(W)), (Y, Z))<\epsilon.
\]  
where  $s_{sec} \geq t(s_{neigh}+s_{rec})$.  One such $(Y, Z)$ must exist by the definition of conditional HILL entropy. Let $Y_z$ denote the random variable $Y|Z=z$.
...
$\delta^D((W, \sketch(W)), (Y, Z))<\epsilon$, we know $\Pr[D(Y, Z) = 1]>1-\epsilon-t\delta$.  By Markov's inequality,  there exists a set $S_Z$ such that $\Pr[Z\in S_Z]\ge 1/2$ and for all $z\in S_Z$, $\Pr[ D(Y_z, z) =1]> 1- 2(\epsilon + t\delta)$.  

Because $\Hav(Y | Z)\geq k$, we know that $\expe_{z\leftarrow Z} \max_y \Pr[Y_z=y]\geq 2^k$.  Applying Markov's inequality to the random variable $\max_y \Pr[Y_z=y]$, there exists a set $S'_Z$ such that $\Pr[z\in S'_Z]> 1/2$, and for all $z\in S'_Z$, $\Hoo(Y_z)\ge k-1$ (we can use the strict version of Markov's inequality here, because the random variable $\max_y \Pr[Y_z=y]$ is positive).  Fix one value $z \in S_Z\cap S'_Z$ (which exists because the sum of probabilities of $S_Z$ and $S'_Z$ is greater than 1).  
Thus, for all such that $t', 1\leq t'\leq t$, 
\[ \Pr_{y\leftarrow Y_z}[y'\leftarrow \sample(y, t') \wedge \rec(y',z) = y]> 1-2(\epsilon+t\delta).\]  
...
Thus,  $Y_z$ is a $(t, 2(\epsilon+t\delta))$-average error Shannon code with recovery $\rec(\cdot,z)$ and $2^{k-1}$ points.  The statement of the theorem follows by application of \lemref{lem:averageToMaximalError}.  
\end{proof}
...
Instead of proving \thref{thm:imp of unp entropy}, we will prove a stronger result based on a weaker form of entropy.  This relaxed definition allows both the variable and the condition to be replaced.  This is analogous to the relaxed HILL entropy~(see \defref{def:relaxed hill}).%Gentry and Wichs utilize an analogous relaxation for HILL entropy in~\cite[Lemma 3.1]{gentry2011separating}~(we provide a formulation of this notion in \defref{def:relaxed hill}).
...
Let $W$ be a distribution over $\mathcal{M}$ and let $(\sketch, \rec)$ be an relaxed unpredictability-entropy $(\mathcal{M}, \Hoo(W), \tilde{m}, t)$ secure sketch that is $(\epsilon, s_{sec})$-secure with error $\delta$ where $s_{sec} \geq \max\{ t(|\rec| +s_{neigh}), |\rec| + s_{sam}\}$.  Let $Y, Z'$ be a joint distribution such that $\delta^{\mathcal{D}_{s_{sec}}}((W, \sketch(W)), (Y, Z'))\leq \epsilon$.  It suffices to show that $\exists \mathcal{I}$ of size $s_{sec}$ such that $\Pr[\mathcal{I}(Z') = Y]\geq 2^{\tilde{m}}$ where $\tilde{m} = n\log |Z| - \log |B_t(\cdot)| - \log(1-\epsilon -t\delta)$.  

Let $B_t(y)$ denote the random variable of a random neighbor of distance at most $t$ from $y$.  Recall, the algorithm $\neigh$ outputs neighbors at a fixed distance, the random variable $B_t(y)$ may not be efficiently computable, we only use it as a random variable.
We begin by showing that \rec must recover points of $Y$.  
\begin{claim}
\label{clm:y is recoverable}
\begin{align*}
\Pr[\rec(B_t(Y), Z') = Y]&=\\
\Pr[(y, z)\leftarrow (Y, Z) \wedge y'\leftarrow B_t(y) \wedge \rec(y', z) = y] &\geq 1-\epsilon -t\delta.
\end{align*}
\end{claim}
\begin{proof}
Suppose that $\Pr[\rec(B_t(Y), Z') = Y]<1-\epsilon -t\delta$.  We construct the following $D\in\mathcal{D}_{s_{sec}}$:
\begin{itemize}
...
\item Output $1$.
\end{itemize}
First note that $|D| = t( |\rec|+ s_{neigh} )$.  Since $(\rec, \sketch)$ has error $\delta$ we know that $\forall w, w'\in \mathcal{M}$ where $\dis(w, w')\leq t$ \[ \Pr[s\leftarrow \sketch(w) \wedge \rec(w', s) =  w] \geq 1-\delta.\]  This implies that for all $1\leq t'\leq t$, $\Pr[\rec(\neigh(W, t'), \sketch(W) )= W)  ]\geq 1-\delta$ and thus $\Pr[D(W, \sketch(W)) = 1]\geq 1-t\delta$.  If $\Pr[\rec(B_t(Y), Z') = Y] < 1-\epsilon -t\delta$ there must exist some $1\leq t'\leq t$ where this is true.  Then 
\begin{align*}
\Pr[D(W, \sketch(W)) = 1]  - \Pr[D(Y, Z')=1] &\geq\\
(1-t\delta) - \Pr[\rec(\neigh(Y, t'), Z') = Y] &> (1-t\delta)-(1-t\delta - \epsilon)>\epsilon.
\end{align*}
...
\[
 \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in \mathcal{M}} \Pr[B_t(y) =y' | Y = y] P_{y',  z, y} \geq 1-\epsilon - t\delta.
\]
...
&\geq \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in\mathcal{M}} \Pr[X = y' ]\Pr[\mathcal{I}(z) = y | X = y']\\
&\geq \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in\mathcal{M}} \Pr[X = y' ] P_{y', z, y}\\
&\geq \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in\mathcal{M}} \frac{\Pr[X = y']}{\Pr[B_t(y) = y' | Y = y]}\Pr[B_t(y) = y' | Y=y ] P_{y', z, y}\\
&\geq  \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in\mathcal{M}} \frac{1/|\mathcal{M}|}{1/|B_t(\cdot)|} \Pr[B_t(y) = y' | Y=y ] P_{y', z, y}\\
&\geq \frac{|B_t(\cdot)|}{|\mathcal{M}|} \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in\mathcal{M}} \Pr[B_t(y) = y' | Y=y ] P_{y', z, y}\\
...
By \corref{cor:code high distance}, the code generated by $\vA$ has distance at least $t$ with probability $1-e^{-\Omega(n)}$.  Thus, a false matches occurs in step (5) with probability $e^{-\Omega(n)}$.  This probability is over the coins of $\gen$ and thus does not grow with the number of iterations in $\mathcal{I}_t$.  We consider the expected number of times step 2 is executed when $t\leq d\log n\left(\frac{m}{n}-1\right)$.  Steps (3-5) can be accomplished in time $n^3\log q $.  An iteration of $\mathcal{I}_t$ succeeds when all selected rows of $\vA_{i_1,..., i_{2n}}, \vc_{i_1,..., i_{2n}}$ have no error and when there is a sub matrix of $A_{i_1,..., i_{2n}}$ that has full rank.  The probability that $A_{i_1,..., i_{2n}}$ is full rank depends only on the coins of $\gen$ and is independent of the probability that $\vc_{i_1,...,i_{2n}}$ has no errors.  The probability of each selected row having an error is at most $\frac{t}{m - i}$ where $i$ is the number of rows already selected.  That is,
\begin{align*}
...
\end{align*}
Now, we consider the probability that no sub matrix of $\vA_{i_1,..., i_{2n}}$ has full rank.  This probability is at least $1-e^{-\Omega(n)}$~(see \clref{cl:full rank matrix}).  Thus, the probability that a given iteration succeeds $\frac{1}{n^{4d}} - e^{-\Omega(n)}$.  
