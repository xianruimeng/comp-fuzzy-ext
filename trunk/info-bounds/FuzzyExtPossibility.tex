...
The conditional Hartley entropy of $X |Y $ is $\tilde{H}_0(X |Y) = \log ( \expe_{y\in Y} 2^{-H_0(X | Y=y)})$.
\end{definition}
...
\begin{theorem}
Let $n$ be a security parameter.  There exists a family of distributions $\mathcal{V}$ such that for each element $W\in \mathcal{V}$, $\Hfuzz(W)= \omega(\log n)$ and there is no $(\mathcal{M}, \mathcal{V}, \tilde{m}, t)$-secure sketch with error $\delta \le 1/2$ for $\tilde{m} =\omega(1)$.  %Furthermore, consider $V$ as the uniform distribution over $\mathcal{V}$.  There is no secure sketch for $\mathcal{
\bnote{what is $t$?}\bnote{finish proof}
\end{theorem}

\begin{proof}
\bnote{need to rearrange this proof.}
...
%\end{itemize}
\end{proof}
We now define the side information $Z = a_2, b_2, ..., a_\ell, b_\ell$.  By described above the view of $\sketch, \rec$ is a uniform distribution $V$.  Thus, the side information $Z$ is independent of $\sketch(V)$.
\begin{lemma}
$\Hav(V| P , Z) = \Theta(1)$
\end{lemma}
\begin{proof}
