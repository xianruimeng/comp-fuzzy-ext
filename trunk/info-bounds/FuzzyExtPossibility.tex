...
\bnote{should note that fuzzy min-entropy is the ideal security, measures how an adversary would do with oracle access, goal is to implement this oracle}
...
\bnote{introduce distributional uncertainty here}
...
\end{definition}
\noindent
\bnote{Should note this is the ideal security of a noisy source, both in the interactive/non-interactive settings}
...
Our goal is to provide a superlogarithmic length key when provided with superlogarithmic fuzzy min-entropy. 

\bnote{I feel we can provide a more general results than this one.  Helpful?}
...
\bnote{this should probably be in the body of the paper}
\end{proof}
...
&\ge -\log\left( 2^{-m} + (1-2^{-m})2^{-\Hav(W | \sketch(W) \wedge 1_{\text{low}} = 0)}\right)
\end{align*}
...
\]
We separate out this quantity into levels:\bnote{we need to talk about K here}\bnote{we were asked what $\Pr[W=w| \sketch(W)]$ means, to me that's well formed}\bnote{shouldn't there be a term here $\Pr[L_i = i]$?}
\begin{align*}
