...
If in the above definitions we take $\mathcal{W}$ to be a one-element set containing a single distribution $W$, then the fuzzy extractor/secure sketch is said to be constructed for a \emph{known distribution}.  In this case, we need to require correctness only for $w$ that have nonzero probability\footnote{If we wish to be pedantic and extend correctness to all of $\mathcal{M}$, we can define $\gen$/$\sketch$ to be the identity function on zero-probability inputs, which will ensure that $\rep$/$\rec$ can always be correct; this will not harm security}.
...
\end{lemma}
\begin{corollary}
Let $n = \log |\mathcal{M}|$.  If $|R| \ge |\beta_{t}|* n^{\omega(1)}$ then \consref{cons:universal hash} is correct with overwhelming probability.  That is, setting $\log |R| = \log \beta_{t} + \omega(\log n)$ suffices.
\end{corollary}
