...
We define \emph{fuzzy min-entropy} to quantify this property of a noisy source of secrets.  Fuzzy min-entropy measures the success of the adversary when provided with the functionality of the fuzzy extractor.  It measures the \emph{ideal} security of a distribution.  

High fuzzy min-entropy is necessary for the existence of a fuzzy extractor; moreover, there is evidence that it may be sufficient when only computational security is required (assuming the existence of semantically-secure graded encodings). Nevertheless, information-theoretic fuzzy extractors are not known for many practically relevant sources of high fuzzy min-entropy. The challenge is to design information-theoretic fuzzy extractors whose security (almost) matches the fuzzy min-entropy of a distribution.

In this work, we ask whether this challenge is inherent.
 We provide two main technical results.
\begin{itemize}
\item \textbf{Sufficiency of fuzzy min-entropy:}  It is possible to build a fuzzy extractor for any \emph{precisely known} physical source.  This result justifies fuzzy min-entropy as the right measure of the quality of a noisy distribution for key derivation.  Previous results considered the entropy and error tolerance of a separately and had not secured many sources with fuzzy min-entropy.
\item \textbf{Cost of distributional uncertainty:}  High entropy distributions can never be fully observed and must be modeled.  An adversary may have a better model than the fuzzy extractor designer.  To compensate, fuzzy extractors are typically designed to work for families of physical sources. We show that this uncertainty is an impediment to security by building a family of high fuzzy min-entropy sources for which no fuzzy extractor can exist.  This result justifies further study of physical sources to increase the fidelity of our models.
\end{itemize}
...
\end{definition}
\noindent
Fuzzy min-entropy is a necessary condition for security.  It measures the advantage provided to the adversary by allowing them to run the $\rep$ functionality.  Thus, the fuzzy min-entropy is the ideal security that any fuzzy extractor should be compared against.  All proofs for statements in this section can be found in \apref{sec:proof fuzz necessary}.
\begin{proposition}
...
Fuzzy min-entropy obeys some natural entropy properties:
\begin{lemma}
...
Before proving security of \consref{cons:leveling} we extend the notation to the maximum likelihood ball containing points of a given probability.  Define $\beta_{t,i}$ as the maximum number of points in a ball in level $i$.  That is,
\[
...
\end{align}
For the remainder of the proof, we seek a bound on $\Hav(W | \sketch(W) \wedge 1_{\text{low}} =0).$  Let $I$ be the random variable of the level information (in what follows, $I$ ranges over $m\le i \le m+n$, where possible we omit this for notational clarity). 

\begin{align}
\Hav(W | \sketch(W) \wedge 1_{\text{low}} =0) &=-\log \left( \expe_{i\in I} 2^{-\Hav(W | \sketch(W) \wedge 1_{\text{low}}=0 \wedge I=i}) \right)\nonumber\\
&=-\log \left( \expe_{i\in I} 2^{-\Hav(W | \mathcal{K}_i \wedge F_i(K_i, W) \wedge 1_{\text{low}}=0 \wedge I=i)}\right)\nonumber\\
&=-\log \left( \expe_{i\in I} 2^{-\Hav(W |  F_i(K_i, W) \wedge 1_{\text{low}}=0 \wedge I=i)}\right)\nonumber\\
&\ge-\log \left( \expe_{i\in I} 2^{-(\Hav(W |  1_{\text{low}}=0 \wedge I=i)-\log (\beta_{t, i}-1) +\log \delta)}\right)\nonumber\\
&\ge-\log \left(\frac{1}{\delta} \expe_{i\in I} 2^{-(\Hav(W |  1_{\text{low}}=0 \wedge I=i)-\log (\beta_{t, i}-1))}\right)\nonumber\\
&\ge-\log \left(\expe_{i\in I} 2^{-(\Hav(W |  1_{\text{low}}=0 \wedge I=i)-\log (\beta_{t, i}-1))}\right) - \log \frac{1}{\delta}\nonumber\\
&\ge-\log \left(\expe_{i\in I} 2^{-(\Hav(W |  1_{\text{low}}=0 \wedge I=i)-\log (\beta_{t, i}))}\right) - \log \frac{1}{\delta}\label{eq:fuzzy ball size}
\end{align}

\noindent
The third line follows because the only dependence between $\mathcal{K}_i$ and $W$ is in $i$.  The fourth line follows by \cite[Lemma 2.2]{DBLP:journals/siamcomp/DodisORS08}.  We now show that the fuzzy min-entropy conditioned on the level information is proportional to $\beta_{t, i}$.  
\begin{claim}
\label{cl:fuzz proportional beta}
\[\Hoo(W | 1_{\text{low}} =0 \wedge J=j ) -\log \beta_{t, i} \ge \Hfuzz(W | J=j \wedge 1_{\text{low}}=0)-1.\]
\end{claim}
\begin{proof}
For convenience denote by $J=j$ the random variable $1_{\text{low}}=0 \wedge I=i$. 
\begin{align*}
\Hfuzz(W|J=j) &= -\log \left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} \Pr[W=w | J=j]\right) \right)\\
&\le -\log \left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} \min_{w^*\in W=w| J=j}\Pr[W=w^* | J=j]\right) \right)\\
&\le -\log \left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} \max_{w^*\in W=w| J=j}\Pr[W=w^* | J=j]\right) \right)+1\\
&= -\log \left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} 2^{-\Hoo(W|J=j)}\right) \right)+1\\
&= \Hoo(W|J=j)- \log\left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} 1\right) \right)+1\\
&=\Hoo(W|J=j)-  \log \left(\max_{w' \in \mathcal{M}} \left|\{w | \dis(w, w')\le t \wedge \Pr[W=w]\in L_i\}\right|\right)+1\\
&=\Hoo(W|J=j)+1-\log \beta_{t, i}.
\end{align*}
Where the third line follows by \clref{cl:nearly flat after conditioning}, which follow the body of the proof.  This completes the proof of \clref{cl:fuzz proportional beta}.
\end{proof}
...
\begin{align*}
\Hav(W | \sketch(W) \wedge 1_{\text{low}} =0) &=-\log \left( \expe_{i\in I} 2^{-\Hfuzz(W | I=i \wedge 1_{\text{low}}=0)-1}\right) -\log \frac{1}{\delta} \\
&=-\log \left( \expe_{i\in I} 2^{-\Hfuzz(W | I=i \wedge 1_{\text{low}} =0)}\right) -1- \log \frac{1}{\delta} \\
...
\begin{claim}
\label{cl:nearly flat after conditioning}
...
&=\frac{\Pr[W=w_2]}{2}.
\end{align*}
Where the first and last equality follow by Bayes' rule.  The second and fourth equality follow by noting that $\Pr[W=w \wedge J=j] = \Pr[W=w \wedge 1_{\text{low}}=0 \wedge I=i] = \Pr[W=w]$ for all $w\in L_i$.  The inequality proceeds by assumption.  This completes the proof of \clref{cl:nearly flat after conditioning}.
\end{proof}
...
For the remainder of the proof we restrict ourselves to elements in $R_{small}$.  Only points that are distance $t$ from points outside of $Q_{p^*, r}$ are viable points in the metric space.  These are the interior of $Q_{p^*, r}$: \lnote{what's $r$? in this and previous lines? also, I can parse definition of interior below}
\begin{align*}
...
We will use the term deficient ball\footnote{In most statements of the isoperimetric inequality, this type of set is simply called a ball.  We use the term deficient ball for emphasis.}:\bnote{use a term other than deficient} \lnote{how about ``almost ball''}
\begin{definition}
...
$\dis(s, d) \ge t$~(that is, the distance between the sets is $t$).  Note that $S_{p^*, \key^*} \cup D^\complement$ is a deficient ball~(and its radius is $\eta+t$).\lnote{Why?}
We now bound the size of $S_{p^*, \key^*}$.

Recall that $|S_{p^*, \key^*} \cup D| = |Q_{p^*, \key^*} | \le 2^{n-\kappa}\leq |\mathcal{M}|/2$.  Since this set contains less than half the points in the metric space we know its radius at most $n/2$.  This means that $|S_{p^*, \key^*}|$ is a deficient sphere \lnote{ball} of radius at most $n/2-t$.  Let $X$ denote a uniform string on $\zo^n$.  We use Hoeffding's inequality~\cite{hoeffding1963probability}: \lnote{should the first $n$ in the line below be $n/2$?}
