...
Note that $\bnd(Q_{p^*, r}) \cap \inter(Q_{p,r}) =\emptyset$.  Furthermore, note that for any $r\in \zo^\kappa$,  $ | Q_{p^*, r}| \le (1+\epsilon)2^\kappa.$\bnote{need to fill in this distinguishing argument, note this doesn't need to be the case for all $p^*$ just on average. need another markov inequality.}\bnote{what can we say about the relative sizes of these spaces?}
