...
$\Hoo(W | I=i ) -\log \beta_{t, i} \ge \Hfuzz(W | I =i )-1$.\bnote{this is where I'm stuck right now.  We started with a nearly flat distribution which would allow us to prove this.  When we condition on $I=i$ it doesn't need to be nearly flat.}
\end{claim}
\begin{proof}
\begin{align*}
\Hfuzz(W|I=i) &= -\log \left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} \Pr[W=w | I=i]\right) \right)\\
&\le -\log \left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} 2^{-(m+i)}\right) \right)\\
&= (m+i)- \log\left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} 2^{-(m+i)}\right) \right)\\
&=(m+i)- \log\left(\max_{w' \in \mathcal{M}} \left( \sum_{w\in W | \Pr[W=w]\in L_i \wedge \dis(w, w')} 2^{-(m+i)}\right) \right)\\
&=(m+i)-  \log \left(\max_{w' \in \mathcal{M}} \left|\{w | \dis(w, w')\le t \wedge \Pr[W=w]\in L_i\}\right|\right)\\
&= m+i-\log \beta_{t, i}.
\end{align*}
\bnote{this proof doesn't follow.}
\end{proof}

\noindent
Together Equation \ref{eq:fuzzy ball size} and \clref{cl:fuzz proportional beta} yield:
\begin{align*}
\Hav(W | \sketch(W) \wedge 1_{\text{low}} =0) &=-\log \left( \expe_{i\in I} 2^{-\Hfuzz(W | I=i \wedge 1_{\text{low}})-1}\right) -\log 1/\delta \\
&=-\log \left( \expe_{i\in I} 2^{-\Hfuzz(W | I=i \wedge 1_{\text{low}} =0)}\right) -1- \log 1/\delta \\
&=\Hfav(W | I \wedge 1_{\text{low}}=0)-1- \log 1/\delta \\
&\ge\Hfuzz(W | 1_{\text{low}}=0)-1-\log n -\log 1/\delta.
\end{align*}
Where the last line follows by \lemref{lem:chain rule fuzz}.\bnote{need to get rid of low prob here}

%\bnote{old proof stuff}
%\noindent
