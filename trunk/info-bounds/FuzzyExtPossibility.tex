...
Let $n$ be a security parameter.  There exists a family of distributions $\mathcal{V}$ such that for each element $W\in \mathcal{V}$, $\Hfuzz(W)= \omega(\log n)$ and there is no $(\mathcal{M}, \mathcal{V}, \tilde{m}, t)$-secure sketch with error $\delta < 1/4$ for $\tilde{m} =\omega(1)$ for $t\ge 4$.  
\end{theorem}
...
The proof uses the same basic outline as the previous result.  However, we work over the binary space to exploit the isoperimetric inequality on Hamming space.
\begin{itemize}
...
\item For most values of possible $Z$, the intersection between the viable pre images of $V|P \wedge Z$ and the viable pre images of $R_{small}$ is empty.  Thus, the output of the fuzzy extractor can be distinguished from uniform.
\end{itemize}
...
\begin{lemma}
Fix some $(\gen, \rep)$ algorithm with $\kappa \ge 1$.  There exists an information-theoretic distinguisher between $(R, P, Z)$ and $(U_\kappa, P, Z)$ with advantage $\epsilon = 1/4-\Theta(2^{-n^{1/3}})$.
\end{lemma}
\begin{proof}
...
We have shown that $|\inter(Q_{p^*, r^*})| \le 2^{n-n^{3/5}}$.  %Define the set $K_{bad} = \cup_{r^*\in R_{small}} |Q_{p^*, r^*}|$ and note that $|K_{bad} | \le  |B_{n-\kappa -t+2}||R_{small}|$.
...
That is, 
\begin{align*}
\expe_z |Q_{small} \cap V | P=p^* \wedge Z=z | &\le \frac{2^{n-n^{1/3}}|R_{small}|}{2^{n - 3\nu}}\\
&=\frac{|R_{small}|}{2^{n^{1/3}-3n^{1/5}}}.
\end{align*}
Thus, at most $1/2^{n^{1/3}- 3n^{1/5}} = \Theta(2^{-n^{1/3}})$ fraction of keys in $R_{small}$ have any support  conditioned on $p^*$ and $Z$~(note this is an expectation across the values of $Z$).  We now present a distinguisher $D_{p^*}$ for a particular $p^*$:
\begin{enumerate}
\item On input $x, z$.
...
&\ge \sum_{p^* \in A_p} \Pr[P=p^*]\left(1/2- \Theta(2^{-n^{1/3}})\right) = 1/4-\Theta(2^{-n^{1/3}}).
\end{align*}
