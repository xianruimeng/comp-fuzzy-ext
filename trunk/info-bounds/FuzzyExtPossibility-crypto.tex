...
We ask: \emph{is high fuzzy min-entropy a
  sufficient condition for noise-tolerant extraction?}
...
In this paper,
show that fuzzy min-entropy is also sufficient for
information-theoretically secure fuzzy extraction. For every source
distribution $W$ for which security is possible in an ideal world with
access only to the extractor functionality, we give a secure fuzzy
extractor. 

Our construction relies on the extractor knowing the precise
