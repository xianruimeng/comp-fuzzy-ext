...
%\begin{definition}
%A binary $(\neigh_t, \delta_{code})$ code is output equiprobable\bnote{Find the right name for this} $\forall i, |\{C | C_i =1\}| =|C|/2$.
%\end{definition}
...
Let $C$ be a $(\neigh_t, \delta_{code})$-code.  Then when $\dis(w, w')\leq t$ and $(R, P)\leftarrow \gen(w)$, then
\[
...
Let $\mathcal{Z}$ be an alphabet, and let $W = W_1,..., W_\gamma$ be a source where each $W_i$ is over $\mathcal{Z}$ and $\gamma = \Omega(n)$.  Let $t$ be the desired error tolerance and let $C\subset \zo^\ell$ be a $(\neigh_t, \delta_{code})$ code.   Let $\eta = \omega(\log n)$ and let $\mathcal{O}$ be a $\ell$-composable obfuscator\bnote{right def?} for the family of point functions over $\zo^\eta\times \mathcal{Z}$.  Define $\gen, \rep$ as:
