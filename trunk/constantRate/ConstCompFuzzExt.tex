...
In the computational setting, a key may be expanded once it is long enough to serve as input to a pseudorandom generator.  Our focus is on expanding the class of distributions for which we can provide meaningful security.

\bnote{two constructions}
...
\end{itemize}
\bnote{compare contrast constructions}
...
\textbf{Connection with Noisy Point Obfuscation: } Ideally, our construction would be a noisy point obfuscation~(which is a stronger object than a computational fuzzy extractor).  Unfortunately, this is not the case.  Each block is individually obfuscated, so an adversary may ask equality queries on individual blocks.  If not all blocks have high entropy an adversary may learn significant information about the source.  Thus, our construction is not a obfuscation for any reasonable definition.  Dodis and Smith~\cite{DBLP:conf/stoc/DodisS05} construct noisy point obfuscation for $gap>>0$ using a information-reconciliation component that hides partial information.  Constructing VBB obfuscation for sources where $gap <0$ is an open problem.
\bnote{does second construction get closer to obfuscation?}

The remainder is organized as follows: we cover notation and background on obfuscation and error correcting codes in \secref{sec:preliminaries} , describe computational fuzzy extractors in \secref{sec:fuzzy extractors}, present our two constructions in Sections \ref{sec:construction} and \ref{sec:sampling} respectively.
...
\bnote{this needs to be rewritten}
Before describing our construction, we recall the problem we are trying address.  The goal is to derive strong keys from noisy sources.  As discussed in the introduction, our goal is to provide security when $gap = \Hoo(W) - \log|B_t|$ is  negative.
...
We show security of \consref{cons:first construction} for distributions satisfying \defref{def:block guessable}.  We discuss  block unguessable distributions in \apref{sec:characterize}.
To show security it is sufficient to show that there exists a distribution $C'$ with conditional min-entropy and $\delta^{\mathcal{D}_{s_{sec}}}((C, P), (C', P))$ are indistinguishable \onote{fix}.  Let $\mathcal{I}$ be the set of indices that exists in \defref{def:block guessable}, the distribution $C'$ is defined as a uniform codeword conditioned on the values of $C$ and $C'$ being equal on all indices outside of $\mathcal{I}$.  First note that $\Hav(C' |P) =\Hav(C' | C_{\mathcal{I}^c} = C'_{\mathcal{I}^c}) = |C| - |\mathcal{I}^c|$~(\clref{cl:many locations ent}).  It is left to show $\delta^{\mathcal{D}_{s_{sec}}}((C, P), (C', P))$ \onote{is small}.
...
\item Show that even an unbounded $S$ with oracle access to $X_1,..., X_\ell$ \onote{fix: making polynomial number of oracle queries to $\mathcal{O}_X$}cannot distinguish between $C$ and $C'$.  That is, $S^{\mathcal{O}_X(\cdot, \cdot)}(C) \approx S^{\mathcal{O}_X(\cdot, \cdot)}(C')$. \onote{sounds like VGB is enough}
...
\noindent Together these arguments allow us to conclude: \onote{Shouldn't this theorem have a proof? All of these parameter do not seem to follow trivially from the bullet points before.}
\begin{theorem}
\label{thm:security of cons}
Let $\mathcal{O}$ be an $\ell$-composable virtual black box obfuscator \bnote{grey box?} with auxiliary inputs for point programs over $\zo^n$ that is $(s_{obf}, \epsilon_{obf})$-hard.  Let $W$ be a $(s_{obf}, \alpha = \omega(\log n), \beta)$-unguessable block distribution.  Then $H^{\hill}_{\epsilon', s_{obf}}( C | P ) \geq |C| - \beta$ for $\epsilon' = 2\epsilon_{obf} + (\ell-\beta)2^{-(\alpha - 1)}$.
\end{theorem}
...
Let $C$ be a $(\neigh_t, \epsilon_{code})$-code.  Then when $\dis(w, w')\leq t$ and $(R, P)\leftarrow \gen(w)$, then
\[
\Pr[\rep( w', P) = R] \geq 1-\left(\frac{\ell}{2^n}+\epsilon_{code}\right).
\]
...
\begin{theorem}
Let $\mathcal{W}$ be a family of $(q,\alpha= \omega(\log n),  \beta)$-unguessable distributions over $\zo^{n\times \ell}$ for any $q = \poly(n)$.  Furthermore, let $C$ be a $(\neigh_t, \epsilon_{code})$-code.  Then for $s_{sec} = \poly(n)$ there exists some $\epsilon=\ngl(n)$ such that \consref{cons:first construction} is a $(\zo^{n\times \ell}, \mathcal{W}, \tilde{m}, t)$-computational fuzzy conductor that is $(s_{sec}, \epsilon_{sec})$ with error $\epsilon_{code} + \ell/2^n$ and resulting entropy $\tilde{m} =\log |C| - \beta$.
\end{theorem}
\begin{proof}
...
\label{sec:sampling}
\consref{cons:first construction} is a computational fuzzy extractor with $gap<0$.  In this section, we present a generalization where blocks are not individually obfuscated, instead several blocks are obfuscated together~(these blocks are selected using an oblivious sampler).  The main advantage of this approach is it expands the supported sources as no single block must have high entropy.  Unfortunately, obfuscating multiple blocks together decreases the effective error tolerance.  Thus, we have a tradeoff between individually required entropy and supported error tolerance.  In ordering for sampling to work properly, we need a source that is likely contribute some entropy from a randomly chosen block.
...
\defref{def:unordered source} is a generalization of block sources~(introduced by Chor and Goldreich~\cite{DBLP:journals/siamcomp/ChorG88}), an unordered block source is one where most blocks contribute some entropy conditioned on the value of the other blocks.  This is a strengthening of an unguessable block distribution~(\defref{def:block guessable}), however we achieve security for significantly lower entropy levels than were necessary for block unguessable distributions.  We use an algorithm that draws random samples from block sources.  Let $\sample_{\gamma, \eta}(\cdot)$ be an algorithm that outputs a random subset of $\{1,..., \gamma\}$ of size $\eta$ and let $r_{sam}$ the number of required coins for $\sample$.
\begin{construction}
\label{cons:sampling}
Let $n$ be a security parameter.
Let $\mathcal{Z}$ be an alphabet, and let $W = W_1,..., W_\gamma$ be a source where each $W_i$ is over $\mathcal{Z}$ and $\gamma = \Omega(n)$.  Let $t$ be the desired error tolerance and let $C\subset \zo^\ell$ be a $(\neigh_t, \epsilon_{code})$ code.   Let $\eta = \omega(\log n)$ and let $\mathcal{O}$ be a $\ell$-composable obfuscator\bnote{right def?} for the family of point functions over $\zo^\eta\times \mathcal{Z}$.  Define $\gen, \rep$ as:
...
The main change in \consref{cons:sampling} is that the obfuscated values are concatenated alphabet symbols of $W$.  This paradigm is similar to \emph{sample-then-extract} from the locally computable extractors literature~\cite{lu2002hyper,vadhan2003constructing}.  For this reason we call \consref{cons:sampling} \emph{sample-then-obfuscate}.  There is a crucial difference, our use of a computational primitive allows us to sample multiple times and only argue that each $v_i$ has sufficient has individual entropy.  In the information-theoretic setting, it would be necessary to argue about the joint entropy of the obfuscated values.  Our construction uses the na\"{i}ve sampler that takes truly random samples, but the public randomness can be substantially decreased by using more sophisticated samplers.  Goldreich provides an introduction to samplers~\cite{goldreich1997sample}.
...
\end{lemma}
\begin{proof}
By~\cite[Lemma 9]{vadhan2003constructing}, noting that the truly random sampler is optimum in necessary samples and we take $\eta = \Omega(\log n)$ samples.  \bnote{need someone to check parameters, in the abstract, I am fairly sure the statement makes sense, but I could easily have messed something up}
\end{proof}
...
\begin{lemma}
Let $n, W$ be as above and Let $C$ be a code that corrects $t$ $0\rightarrow 1$ bit errors where $t =\Theta(\eta)$.  Let $c = -\log(1-t'/(2\ell))/2 = \Theta(1)$, when $t \leq c(\gamma - \eta)/\eta$, then $\Pr[\dis(v, v')\leq t']\geq 1-O(2^{-\eta})$ where the probability is over the coins of $\gen$.
\end{lemma}
\begin{proof}
...
\begin{align*}
\Pr\left[\frac{1}{\eta} \sum_{i=1}^\eta X_i\geq 2\expe[X_i]\right]\leq 2e^{-2\expe[X_i]\eta} = O(e^{- \eta})
\end{align*}
\end{proof}

\begin{corollary}
Let $\eta = \omega(\log n)$ and let $C\subset\zo^\ell$ and  be a code that corrects $t' = \Theta(\ell)$ $0\rightarrow 1$ bit errors.  Then when
\[
\dis(w, w')= \frac{-\log(1-t'/(2\ell))}{2}\frac{\gamma-\eta}{\eta} = O(\frac{\gamma-\eta}{\eta}) = O(\gamma/\eta)
\]
and $(R, P)\leftarrow \gen(w)$, then
\[
\Pr[\rep( w', P) = R] \geq 1-\frac{\ell}{2^n} - O(2^{-\eta}) = 1-\ngl(n).
\]
...
&= \Theta(\gamma) - O(\gamma/\eta) \log |\mathcal{Z}|\\
\end{align*}
That is, \consref{cons:sampling} achieves $gap<0$ when the starting alphabet is super polynomial~(noting that for any fixed $\mathcal{Z}$ we can set $\eta = \omega(\log n)$ and $\eta = o(\log |\mathcal{Z}|)$).  If we are willing to accept $gap\geq 0$ then \consref{cons:sampling} works for polynomial size alphabet $\mathcal{Z}$.
