...
Let $W = W_1,..., W_\gamma$ be a $(\alpha = \Omega(1), \beta\le \gamma(1- \Theta(1)))$-unordered block source and let $\eta = \omega(\log n) \le  o(\gamma)$.  Consider some fixed $i$, there exists a value $\epsilon_{sam} = O(e^{-\eta}) = \ngl(n)$ such that 
\[
\Pr[\Hoo(V_i | \Lambda) \geq  \omega(\log n) = \alpha'] \geq 1- O(e^{-\eta}).
\]
\end{lemma}
\begin{proof}
Consider some fixed $i$.  
For each particular value of $w_1,..., w_\gamma$ there exists some set $\mathcal{I}$ of size $\gamma - \beta = \gamma(1/2+\Theta(1))$ such that each block $j\in \mathcal{I}, \Hoo(W_j | W_1 = w_1,..., W_{j-1}=w_{j-1}, W_{j+1}=w_{j+1},..., W_\gamma = w_\gamma) \geq \alpha$.  Since this is a worst case guarantee, 
\[
\Hoo(V_i |\Lambda ) \geq \alpha \times |\{\text{indices in }\mathcal{I}\text{ sampled in }V_i\}|.
\]

Denote by $X = |\{\text{indices in }\mathcal{I}\text{ sampled in }V_i\}|$.  We now calculate $\expe[X]$.  Let $j_1,.., j_\eta$ be the elements selected by sampling algorithm.  Note that the first element $j_1\in\mathcal{I}$ with probability $(\gamma-\beta)/\gamma$.  Consider some choice $j_k$ there are least $\gamma-\beta-k$ samples in $\mathcal{I}$ remaining, so it the $\Pr[j_k\in\mathcal{I}]>(\gamma-\beta-\eta)/\gamma$.  We can associate with each $j_k$ a Bernoulli random variable $X_i$ with success probability $(\gamma-\beta-\eta)/\gamma$ and note that 
\[
\expe[X] \geq \eta \expe[X_i] \geq \eta\frac{\gamma-\beta-\eta}{\gamma}.
\]

Using the Chernoff bound the probability that $X$ is rarely far from its expected value:
\[
...
\]
That is $\Pr[\Hoo(V_i ) \geq \alpha \times \frac{\eta(\gamma-\beta-\eta)}{2\gamma} = \omega(\log n)] \geq 1- O(e^{-\eta})$.
\end{proof}
...
\begin{corollary}
\label{cor:samp sec}
Let $W = W_1,..., W_\gamma$ be a $(\alpha = \Omega(1), \beta \leq \gamma(1-\Theta(1)))$-unordered block source.  Let $\eta = \omega(\log n) = o(\gamma)$ and let $\epsilon_{sam}, \alpha'$ be as in \lemref{lem:sampling works}.  Then $\Pr[\exists i, \Hoo(V_i | \Lambda) \not \ge \alpha'] \leq 1-\ell\epsilon_{sam}$.
...
Let $q = \poly(n)$, there exists some $\alpha'' = \omega(\log n)$ such that distribution $V , \Lambda$ is $\ell \epsilon_{sam}$ statistically close to a $(q, \alpha'', 0)$-block unguessable distribution.
\end{corollary}
