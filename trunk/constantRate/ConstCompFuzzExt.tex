...
We now show security of \consref{cons:first construction} for distributions satisfying \defref{def:block guessable}.
To show security it is sufficient to show that there exists a distribution $C'$ such that $\Hav(C' | P) \geq k$ and $\delta^{\mathcal{D}_{s_{sec}}}((C, P), (C', P))$ are indistinguishable.  Let $\mathcal{I}$ be the set of indices that exists in \defref{def:block guessable}, the distribution $C'$ is defined as a uniform codeword conditioned on the values of $C$ and $C'$ being equal on all indices outside of $\mathcal{I}$.  First note that $\Hav(C' |P) =\Hav(C' | \mathcal{I}^c) =  \Hav|C'| = |C| - |\mathcal{I}^c|$~(\clref{cl:many locations ent}).  It is left to show $\delta^{\mathcal{D}_{s_{sec}}}((C, P), (C', P))$.  
...
Let $\mathcal{O}$ be a virtual black box obfuscator \bnote{fill in parameters for auxiliary and composability} \bnote{need to link parameters for obfuscation and block unguessable} for point programs of hardness $s_{obf}, \epsilon_{obf}$.  Let $W$ be a $(s_{obf}, \alpha = \omega(\log n), \beta)$-unguessable block distribution.  Then $H^{\hill}_{\epsilon', s_{obf}}( C | P ) \geq |C| - \beta$ for $\epsilon' = 2\epsilon_{obf} + 2^{-(\alpha - 1)}$.
\end{theorem}
...
$\Delta(S^{\mathcal{O}_X(\cdot, \cdot)}(C), S^{\mathcal{O}_X(\cdot, \cdot)}(C')) < 2^{-(\alpha+1)}$.
\end{lemma}

\begin{proof}
\noindent It suffices to show that for any two codewords that agree on $\mathcal{I}^c$, the statistical distance is $2^{-(\alpha+1)}$.
\begin{lemma}
\label{lem:codewords in I close}
Let $c_1, c_2$ be two codewords that agree on all indices $\mathcal{I}^c$.  Then, 
\[
\Delta( S^{\mathcal{O}_X(\cdot, \cdot)}(c_1), S^{\mathcal{O}_X(\cdot, \cdot)}(c_2)) < 2^{-(\alpha+1)}.
\]
\end{lemma}
\begin{proof}
Recall that for all $i\in \mathcal{I}$, $\Hav(W_i | View(S))\geq \alpha$.  The only information about the correct value of the codeword is contained in the query responses.  Let $c^*\in C | C_{\mathcal{I}^c}$ be the value that was used to generate $X$.  It suffices to show that for any value of $c^*$ all queries on $i \in \mathcal{I}$ return $0$ with probability $1-2^{-\alpha+1}$.  Suppose not, that is suppose, the probability of at least one nonzero response is $> 2^{-(\alpha+1)}$.  Since $w, w'$ are independent of $p$, the probability of this happening when $c^*_i = 1$ is at most $q/2^n$.  Thus, it must occur with probability:
\begin{align*}
...
\end{claim}
\begin{proof}
Consider one particular $i \in \mathcal{I}$.  It suffices to show that there exists a simulator $S$ making $q$ queries such that the remaining entropy in that block is at most $n-\log (q+1)$.  Let $W_i$ be a distribution, consider $S$ that asks the $q$ most likely outcomes, the total probability of this block must be at least $2^{-n+\log q}$.  After these queries, Then, the remaining min-entropy is at most:
\begin{align*}
...
\begin{align*}
 ...
 \end{align*}
 When there is a $1$ response and $c_i=0$ this means that there is no remaining min-entropy.  If this occurs with over $2^{-\alpha}$ probability this violates the block unguessability of $W$~(\defref{def:block guessable}).  Thus, for all $c_1, c_2\in C| C_{\mathcal{I}^c}$ the statistical distance is at most $2^{-\alpha+1}$.  This concludes the proof of \lemref{lem:codewords in I close}
\end{proof}
Since point wise the two distributions are close we can conclude that $\Delta(S^{\mathcal{O}_X(\cdot, \cdot)}(C), S^{\mathcal{O}_X(\cdot, \cdot)}(C')) < 2^{-(\alpha+1)}$.  This completes the proof of \lemref{lem:sim cannot distinguish}.
\end{proof}
...
\end{theorem}
\begin{proof}
Security is result of \thref{thm:security of cons}.  Correctness follows from \lemref{lem:correct of cons} and the fact that $C$ is a $(\neigh_t, \epsilon_{code})$-code.
\end{proof}
