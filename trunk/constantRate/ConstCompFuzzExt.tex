...
Let $\mathcal{Z}$ be an alphabet, and let $W = W_1,..., W_\gamma$ be a source where each $W_i$ is over $\mathcal{Z}$ and $\gamma = \Omega(n)$.  Let $t$ be the desired error tolerance and let $C\subset \zo^\ell$ be a $(\neigh_t, \delta_{code})$ code.   Let $\eta = \omega(\log n)$ and let $\mathcal{O}$ be a $\ell$-composable virtual grey box obfuscator with auxiliary input for the family of point functions over $\zo^\eta\times \mathcal{Z}$.  Define $\gen, \rep$ as:
...
We first consider calculate $\expe[\dis(V, V')]$ for some desired error tolerance $t$.
Let $\mu$ be a parameter, let $t = \mu(\gamma-\eta)/\eta$ and assume that $\dis(w, w')\leq t$, for any  $i$ one has:
\begin{align*}
...
\end{align*}
Thus, by setting $\mu = -\log(1-t'/\ell)/2$ we obtain in $\expe[\dis(V, V')\leq t']$.  We need a stronger condition that this occurs with overwhelming probability.  \bnote{stopped here.}
