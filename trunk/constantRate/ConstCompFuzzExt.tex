...
The argument that $1\rightarrow 0$ flips are unlikely carries over from \consref{cons:first construction}.  Let $t'$ be the capacity of $C$.  To show correctness we must show that with overwhelming probability $\dis(v, v')\leq t'$.  Let $t$ be the number of supported errors between $w$ and $w'$.  
Let $\mu$ be a parameter and let $t = \mu(\gamma-\eta)/\eta$, for any  $i$ one has:
\begin{align*}
...
\end{align*}
Thus, by setting $\mu = -\log(1-t'/\ell)/2$ we obtain in $\expe[\dis(v, v')\leq t']$.  We need a stronger condition that this occurs with overwhelming probability.

\begin{lemma}
Let $n, W$ be as above and Let $C$ be a code that corrects $t$ $0\rightarrow 1$ bit errors where $t =\Theta(\eta)$.  Let $\mu = -\log(1-t'/(2\ell))/2 = \Theta(1)$, when $t \leq \mu(\gamma - \eta)/\eta$, then $\Pr[\dis(v, v')\leq t']\geq 1-O(2^{-\eta})$ where the probability is over the coins of $\gen$.
\end{lemma}
\begin{proof}
With each $i$ we can associate a Bernoulli random variable $X_i \overset{def}= V_i \overset{?}\neq V_i'$.  As stated above, we have that $\expe[X_i] \leq 1-\frac{1}{4}^\mu = t'/(2\eta)$. By the Chernoff bound, we have
\begin{align*}
...
\begin{corollary}
Let $\eta = \omega(\log n)$ and let $C\subset\zo^\ell$ be a $(\neigh_t, \delta_{code})$ for $t' = \Theta(\ell)$ and $\delta_{code} = \ngl(n)$.  Then when
\[
\dis(w, w')= \frac{-\log(1-t'/(2\ell))}{2}\frac{\gamma-\eta}{\eta} = O(\frac{\gamma-\eta}{\eta}) = O(\gamma/\eta)
\]
and $(R, P)\leftarrow \gen(w)$, then
\[
\Pr[\rep( w', P) = R] \geq 1-\frac{\ell}{2^n} - O(2^{-\eta}) -\delta_{code}= 1-\ngl(n).
\]
