...
a constant fraction of symbols in $w$ may have little entropy, as long as knowledge of their values does not reduce the entropy of the high-entropy symbols too much (see \defref{def:block guessable}).  This construction can correct a constant fraction~($\Theta(\ell)$) of errors.  However, this construction requires a large alphabet size~(super-polynomial in the security parameter).
...
However, this improvement comes at a price to error-tolerance: it tolerates a sub-constant fraction~($o(\ell)$) of errors.  This second construction comes with an additional feature: reusability.  Boyen~\cite{Boyen2004} defined reusable fuzzy extractors: multiple readings $w_1,..., w_q$ are enrolled and the adversary sees the corresponding public values~(and maybe some subset of secret keys) where $w_i$ and $w_j$ are close.  Boyen showed that with an unbounded number of reuses it is only possible to support limited correlations between $w_i$ and $w_j$~\cite[Theorem 11]{Boyen2004}.  We show that for computational adversaries it is possible to significantly expand the allowed correlation between $w_i$ and $w_j$.  The only condition we need for security is that each marginal $W_i$ is an admissible distribution.  We do not considering the joint distribution $W_1,..., W_q$.
...
The next two sections are dedicated to proving this theorem.
 For security we argue that each of the $v_i$ values is unguessable.  For correctness we show that with overwhelming probability at least of the $v_i$ and $v_i'$ match and allowing the digital locker to be opened and $r$ to be recovered.  For reusability, we show that \bnote{what?}
...
\]
That is, there exists a super-constant alphabet size for which \consref{cons:sampling} is secure with $\Huse\le 0$.  We note that \consref{cons:sampling} works for an arbitrary size alphabet; however, for a constant size alphabet, $\Huse>0$.
...
Instead of restricting the type of correlations, we limit the adversary to be computationally bounded~(and thus sees only a limited number of reuses).
We present a simplified non-adaptive definition and note that our results apply equally well to the adaptive definition.

\begin{definition}[Insider Reusable Fuzzy Extractors]
\label{def:reusable fuzz}
Let $\mathcal{W}$ be a family of distributions over $\mathcal{M}$.  Let $(\gen, \rep)$ be a $(\mathcal{M}, \mathcal{W}, \kappa, t)$-computational fuzzy extractor that is $(\epsilon, s)$-hard with error $\delta$.  Let $W_1,..., W_q$ be a joint distribution such that $\forall i, W_i\in\mathcal{W}$.  We say that $(\gen, \rep)$ is $q$-reusable if for all $i$
\begin{align*}
\delta^{\mathcal{D}_{s_{sec}}}(&(R_1, P_1), ..., (R_i, P_i),..., (R_q, P_q), \\
