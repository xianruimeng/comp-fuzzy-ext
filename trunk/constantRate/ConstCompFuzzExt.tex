...
Fuller, Meng, and Reyzin~\cite{fuller2013computational} present two approaches for constructing a computational fuzzy extractor: using a fuzzy conductor and a privacy amplification component or analyzing the information-reconciliation and privacy amplifications components together  .  We use each approach~(in \secref{sec:construction} and \secref{sec:sampling} respectively).
...
We use a computational version of a fuzzy conductor.
...
The main idea is to sample a random subset of blocks $W_{j_1},..., W_{j_\eta}$ and obfuscate the concatenation of these blocks.  Denote this concatenated value by $V_1$.  This process is repeated to produce $V_1,..., V_\ell$ where at least one $V_i$ should be correct.  We first present the necessary definitions and the construction and then compare it to \consref{cons:first construction}.  % and the construction proceeds by either obfuscating $V_i$ or a random point as before. 
...
show that with overwhelming probability, at each of the $\ell$ iterations, the sampler will choose enough coordinates of $W$ that have high entropy, making $V_i$ have sufficient entropy.   We can then argue that $V_1,..., V_\ell$ forms a block-unguessable distribution.  Then security follows very similarly to \lemref{lem:security of cons}~(used to show security of \consref{cons:first construction}).  Essentially, the argument is that an adversary will never be able to open a digital locker and thus they learn no information about the key.  %By the same argument of Then \consref{cons:sampling} is just \consref{cons:first construction} applied to $V_1,.., V_\ell$, and security follows by \lemref{lem:security of cons}.  
...
The main idea of this construction is to first condense entropy from each block of the source and then apply a standard secure sketch to the extracted information.  Intuitively, we are then able from a large alphabet while paying entropy loss on the smaller alphabet.  %Alternatively, this construction can simply be seen as replacing the computational point function obfuscation with an information-theoretic point-function obfuscation~(this is only possible for high enough entropy levels).
We first review some information-theoretic definitions.
...
Note that randomness condensers can be built significantly more efficiently than randomness extractors~(see for example~\cite{dodis2013key}).  
