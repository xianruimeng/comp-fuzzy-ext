...
\begin{abstract}
Fuzzy extractors convert a noisy source of entropy into a consistent uniformly distributed key.  In the process of eliminating noise, they necessarily lose some of the entropy of the original source. We call the difference between the starting entropy and the logarithm of the number of correctable error patterns the \emph{minimum usable entropy}.  Unfortunately, this quantity is negative for some sources that are important in practice.

We construct fuzzy extractors that work for distributions in which the minimum usable entropy is \emph{negative}. Such constructions must, necessarily, put additional limits on the distribution beyond its starting entropy. We show two constructions that have computational security for Hamming distance over large alphabets. The first requires high individual entropy of a constant fraction of symbols, but permits symbols to be dependent.  The second requires  a constant fraction  symbols to have a constant amount of entropy conditioned on prior symbols. Our constructions can be implemented efficiently based on number-theoretic assumptions or assumptions on cryptographic hash functions.
