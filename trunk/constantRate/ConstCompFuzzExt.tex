...
Let $W = W_1,..., W_\ell$ be a source.  Suppose that for all $i$, $\Hoo(W_i)\geq \alpha$, and that $q 2^{-\alpha}\leq1/4$ (this holds asymptotically, in particular, if $q$ is polynomial and $\alpha$ is superlogarithmic). Then  $W$ is a $(q, \alpha-1-\log(q+1), 0)$-block unguessable distribution.
\end{claim}
...
for any random variable $X$ and event $E$, $\Hoo(X|E)\ge \Hoo(X)+\log\Pr[E]$; and $(j-1) 2^{1-\alpha}\leq 2 q 2^{-\alpha} \leq 1/2$.  We prove the base case and the inductive step as follows:
\begin{align*}
\Pr[A_1\neq 0 \vee \dots \vee A_{j-1}\neq 0 \vee A_j\neq 0] & = 
\Pr[A_1\neq 0 \vee \dots \vee A_{j-1}\neq 0]+\Pr[A_1=\dots = A_{j-1}=0 \wedge A_j=1]\\
& \le  (j-1)2^{1-\alpha}+\Pr[A_j=1\,|\,A_1=\dots = A_{j-1}=0]\\
& \le  (j-1)2^{1-\alpha}+\max_k 2^{-\Hoo(W_k | A_1=\dots =A_{j-1}=0)}\\
& \le  (j-1)2^{1-\alpha}+ 2^{1-\alpha}\\
& = j 2^{1-\alpha}
\end{align*}
(where the third line follows by considering that to get $A_j=1$, the adversary needs to guess some $W_k$, and the fourth line follows by \eqref{eq:cond-entropy}).
Thus, using $j=q+1$ in \eqref{eq:cond-entropy},
 we know $\Hoo(W_i | A_1= \dots= A_q=0) \ge \alpha-1$.
\begin{align*}
...
& \ge -\log   (q+1) 2^{1-\alpha}) = \alpha-1-\log(q+1)\,.
\end{align*}
\end{proof}
