...
We then switch to computational security to obtain constructions with additional features. Our second construction, like the first, also requires that a constant fraction of symbols contribute fresh entropy. However, it provides reusability. The reusability we obtain is very strong:  security holds even if the multiple readings $w_i$ used in $\gen$ are \emph{arbitrarily correlated}, as long as each $w_i$ \emph{individually} satisfies the above conditions as a distribution.
We note that such strong reusability was shown to be impossible in information-theoretically secure fuzzy extractors~\cite[Theorem 11]{Boyen2004}.\lnote{really? I can't parse his theorem clearly enough} This construction, however, requires that the fraction of errors is subconstant.
