...
As shown by~\cite{DBLP:journals/siamcomp/DodisORS08}\lnote{specific theorem number} for the information-theoretic case and by \cite[Corollary 3.8, Theorem 3.10]{fuller2013computational} for the computational case, this is unlikely to work for sources with more errors than entropy. Thus, we give up on recovering $w$.
...
Our second construction, which is only computationally secure, is based on digital lockers~\cite{canetti2008obfuscating}. Digital lockers are obfuscated functions that output a secret value only when given the correct input to ``unlock'' the secret. The main idea of the construction is to pick a random $r$ and then lock it up in a digital locker that is unlocked by a random subset of the symbols of $w$. To tolerate errors in the input, this process is repeated several times, so that at least one digital locker can be unlocked using $w'$. Reusability follows from composability of obfuscation.

Finally, our third construction tolerates more errors than the second because it uses digital lockers that are unlocked by single symbols of $w$. Of course, hiding an entire $r$ in every locker then becomes too risky, since we do not assume that every symbol has high individual entropy. Instead, we hide a single bit per locker (this simplified version of a digital locker is basically point obfuscation). To tolerate errors, these bits come from an error correcting code. And to make sure that the adversary who learns some bits doesn't learn anything useful about $r$, we don't encode $r$ in the error-correcting code, but rather extract $r$ (using an information-theoretic~\cite{nisan1993randomness} or computational~\cite{krawczyk2010cryptographic} extractor~(\lemref{lem:cond and cext})) from the decoded string.
...
\bnote{everything below needs to be rewritten}
...
Information-theoretic fuzzy extractors are obtained by replacing computational distance by statistical distance.  We do make a second definitional modification.  The standard definition of information-theoretic fuzzy extractors considers $\mathcal{W}$  consisting of all distributions of a given entropy.  As described in the introduction, it is impossible to provide security for all distributions with more errors than entropy.  Thus, in both the computational and information-theoretic settings we consider arbitrary families of distributions.

\subsubsection{Reusable Fuzzy Extractors}
\label{sec:reusable}
\bnote{some of this should probably be moved to the introduction}

Ideally, a fuzzy extractor allows a user to authenticate to multiple different services/devices.  That is, instead of creating a single pair $(r, p)$ from an initial reading $w$, we read the noisy source multiple times~($w_1,..., w_q$) and produce $(r_1, p_1),..., (r_q, p_q)$.  This problem is known as a reusable fuzzy extractor~\cite{Boyen2004}.  Boyen considers two versions of the problem, first where the adversary sees $p_1,..., p_q$~(outsider security).  Second where the adversary controls some subset of the servers and sees key generation on arbitrary $p$~(insider security).  
Boyen constructs a outsider reusable cryptographic fuzzy extractor based on the code-offset or the syndrome sketches from~\cite{JW99, DBLP:journals/siamcomp/DodisORS08} instantiated with linear codes. Boyen also transforms this construction in an insider secure construction using random oracles. 
...
%\end{align*}
%\end{definition}
Our definition is a modified form of the definition of Boyen~\cite[Definition 7]{Boyen2004}.  We discuss definitional differences in \apref{sec:comparing reusability}.

\begin{definition}[Outsider Reusable Fuzzy Extractors]
\label{def:outsider fuzz ext}
Let $\mathcal{W}$ be a family of distributions over $\mathcal{M}$ and fix some $W_1 \in \mathcal{W}$.  Let $f_2,.., f_q , D$ be a split adversary.  Define the following game for all $i=1,..., q$:
...
The following theorem states the construction is a computational fuzzy conductor.  %After proving this theorem we discuss when \consref{cons:first construction} is useful on its own.
\begin{theorem}
...
\[
\text{\# errors} - \text{entropy} = \log |B_t| -  \Hoo(W)  <\left( \Theta(t\log |\mathcal{Z}|) + \log {\ell \choose t}\right) -  \ell \omega(\log n) .
\]
When $t =\Theta(\ell)$ and the entropy of each block is $o(\log |\mathcal{Z}|)$, then the construction supports more errors than entropy. Furthermore, the output entropy is $H_0(C) -\beta$~(if $C$ is a constant rate code, this is $\Theta(\ell)$).
...
\subsection{Adapting Reusability to the Computational Setting}
