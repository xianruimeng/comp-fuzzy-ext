...
the reusability we obtain is very strong:  security holds even if the multiple readings $w_i$ used in $\gen$ are \emph{arbitrarily correlated}, as long as each $w_i$ \emph{individually} satisfies the above conditions as a distribution. The switch to computational security is necessary here: as discussed in \secref{sec:reusable}, it is impossible to obtain such reusability information-theoretically for sources with more errors than entropy.
