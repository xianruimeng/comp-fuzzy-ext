...
We construct the first fuzzy extractors that work for a large class of distributions that have negative minimum usable entropy. Their security is computational.  In order to avoid the worst-case loss, they necessarily restrict distributions for which they work.  

Our first construction corrects a constant fraction of Hamming errors over a large alphabet. It requires high individual entropy of a constant fraction of symbols, but permits symbols to be dependent.  

Our second construction requires a constant fraction of symbols to have a constant amount of entropy conditioned on prior symbols. It corrects a sub-constant fraction of errors and supports super-constant size alphabets.  In addition, it is first construction of a reusable fuzzy extractor without assumption on the form of errors that occurs between reuse.  This is impossible in the information-theoretic setting for an arbitrary number of reuses~(Boyen, CCS 2004).

The constructions can be implemented efficiently based on number-theoretic assumptions or assumptions on cryptographic hash functions.
...
We provide the first constructions of computational fuzzy extractors that can be used for a large class of distributions with $\Huse\le 0$ over the Hamming metric on $\mathcal{Z}^\ell$ for an alphabet $\mathcal{Z}$. 
...
a constant fraction of symbols in $w$ may have little entropy, as long as knowledge of their values does not reduce the entropy of the high-entropy symbols too much (see \defref{def:block guessable}).  This construction can correct a constant fraction of errors~($\Theta(\ell)$).  However, this construction requires a large alphabet size~(super-polynomial in the security parameter).

We improve the entropy and alphabet size requirements in the second construction, which requires only a constant fraction of the symbols $w$ to have constant min-entropy conditioned on the previous symbols.  Our second construction supports an arbitrary alphabet size~(though it only achieves $\Huse\le 0$ when the alphabet size is super-constant).  
However, these improvements come at a price to error-tolerance: it tolerates a sub-constant fraction of errors~($o(\ell)$).  

The second construction comes with an additional feature: reusability.  Boyen~\cite{Boyen2004} defined reusable fuzzy extractors: multiple readings $w_1,..., w_q$ are enrolled and the adversary sees the corresponding public values $p_1,..., p_q$ and possibly some of the derived keys.  The enrolled values $w_i$ and $w_j$ are not assumed to be equal, and so information leakage may add up.  Indeed, Boyen showed that with an unbounded number of reuses it is only possible to support limited correlations between $w_i$ and $w_j$~\cite[Theorem 11]{Boyen2004}.  Our second construction allows for arbitrary correlation between $w_i$ and $w_j$ against computational adversaries\footnote{Our restriction to computational adversaries implies that the adversary only sees a polynomially bounded number of reuses.  It remains an open problem to construct a polynomially reusable fuzzy extractor against unbounded adversaries.} as long as each marginal $W_i$ is an admissible distribution.  %We do not considering the joint distribution $W_1,..., W_q$.

\begin{table}
\begin{tabular}{l | l | l | l }
Construction & Security & Features & Limitations \\
\hline
\consref{cons:info theoretic} & Info-theoretic & Exploits distribution & Most symbols must \\
&&structure & contribute fresh entropy \\\hline
\consref{cons:sampling} & Computational  & Reusable with arbitrary & Error tolerance is\\
&&correlation & sub-constant fraction\\\hline
\consref{cons:first construction} & Computational & Supports distributions  & Super-polynomial \\
& &  with dependent symbols & size symbols
\end{tabular}
\caption{Summary of new constructions.  All constructions support families of distributions with $\Huse \le 0$.}
...
Fuller, Meng, and Reyzin~\cite{fuller2013computational} present two approaches for constructing a computational fuzzy extractor: using a fuzzy conductor and a privacy amplification component or analyzing the information-reconciliation and privacy amplifications components together.  %We both approaches, \consref{use each approach~(in \secref{sec:construction} and \secref{sec:sampling} respectively).
%The lower bounds on entropy loss of fuzzy extractors shown by Dodis et al.~\cite[Section C]{DBLP:journals/siamcomp/DodisORS08} extend immediately to fuzzy conductors.  %In \apref{sec:conductors}, we show that information-theoretic fuzzy conductors are subject to the same lower bounds on entropy loss as information-theoretic  fuzzy extractors.  
We will use approaches: \consref{cons:info theoretic} and \consref{cons:first construction} use fuzzy conductors and privacy amplification and \consref{cons:sampling} achieves both tasks simultaneously.  %We use a computational version of a fuzzy conductor.
We use the notion of HILL entropy~\cite{DBLP:journals/siamcomp/HastadILL99} extended to the conditional case:
...
\end{itemize}
\end{definition}
\noindent \textbf{Note: } Replacing HILL entropy with average min-entropy gives an \emph{information-theoretic fuzzy conductor}.  We will use this notion for our information-theoretic construction \secref{sec:info theory cons}.
...
\subsubsection{Information-Theoretic Definitions}
In this section we review additional definitions that will be used for our information-theoretic construction in \secref{sec:info theory cons}.
\begin{definition}
...
\item The recovery procedure \rec takes an element $w'\in\mathcal{M}$ and a bit string $s\in\{0,1\}^*$.  The \emph{correctness} property of secure sketches guarantees that if $\dis(w,w')\leq t$, then $\Pr[\rec(w',\sketch(w))=w]\geq 1-\delta$ where the probability is taken over the coins of $\sketch$ and $\rec$.
\item The \emph{security} property guarantees that for any distribution $W$ over $\mathcal{M}$ with min-entropy $m$, $\Hav(W|\sketch(W))\geq \tilde{m}$.
\end{enumerate}
...
\begin{definition}
\label{def:conductor}
A function $\cond : \mathcal{Z}\times \zo^d\rightarrow \mathcal{Y}$ is a $(m, \tilde{m}, \epsilon)$-average case randomness condenser if whenever $\Hoo(W)\ge m$, then there exists a distribution $Y$ with $\Hoo(Y)\ge \tilde{m}$ and $(\cond(W, seed), seed) \approx_\epsilon (Y, seed)$.
\end{definition}

\noindent Randomness condensers can be built  more efficiently than randomness extractors~(for example~\cite{dodis2013key}).  

%\begin{definition}
%\label{def:fuzzy conductor}
%A pair of randomized procedures ``generate'' ($\gen$) and ``reproduce'' ($\rep$) is an $(\mathcal{M}, \mathcal{W}, \tilde{m}, t$)-fuzzy conductor that is $\epsilon$-hard with error $\delta$ if $\gen$ and $\rep$ satisfy \defref{def:comp fuzzy extractor}, except the last condition is replaced with the following condition:
%\begin{itemize}
%\item For any distribution $W\in \mathcal{W}$, the string $r$ has is close to a distribution with average min-entropy conditioned on $P$.  That is there exists a distribution $Y$ with $\Hav(Y | P)\ge \tilde{m}$ such that $(R ,P)\approx_\epsilon (Y, P)$.
%\end{itemize}
%\end{definition}

\subsubsection{Reusable Fuzzy Extractors}
\label{sec:reusable}
\bnote{some of this should probably be moved to the introduction}

Ideally, a fuzzy extractor allows a user to authenticate to multiple different services/devices.  That is, instead of creating a single pair $(r, p)$ from an initial reading $w$, we read the noisy source multiple times~($w_1,..., w_q$) and produce $(r_1, p_1),..., (r_q, p_q)$.  This problem is known as a reusable fuzzy extractor~\cite{Boyen2004}.  There are two basic versions of the problem, first where the adversary sees $p_1,..., p_q$~(outsider security), and second where the adversary also sees some subset of the derived keys as well~(insider security).  Ideally, we want $w_1,..., w_q$ and the unseen keys to be ``as good'' as in the single use case.  Boyen constructs a reusable cryptographic fuzzy extractor based on the code-offset or the syndrome sketches from~\cite{JW99, DBLP:journals/siamcomp/DodisORS08} instantiated with linear codes.  For simplicity here, we define the insider security and note any scheme that achieves insider security also achieves outsider security.  

\paragraph{Previous Results}

There are two primary questions about the reusability of a fuzzy extractor.  The first is how often it can be reused, the second is what correlation is allowed between $w_1,..., w_q$.  Boyen~\cite{Boyen2004} considers fuzzy extractors that are reusable an unbounded number of times.  Their construction is secure when the correlations between $w_1,.., w_q$ come from a subgroup of permutations used in the construction of the fuzzy extractor.  They show in the setting of unbounded reuse such a strong restriction on correlation is necessary~\cite[Theorem 11]{Boyen2004}.  In particular, they show how any information-theoretic fuzzy extractor secure with unbounded reuse can be transformed into their construction.  However, there is no reason to expect that noise between repeated readings comes from a well-behaved subgroup of permutations.
...
We limit the adversary to be computationally bounded~(and thus sees only a limited number of reuses).
...
\end{enumerate}
\end{definition}
For notational convenience, when we use point function obfuscation, we denote the oracle provided to the simulator as $I_w(\cdot, \cdot)$ where $w = w_1,..., w_\ell$ is the vector of obfuscated points.  When we use digital lockers we denote the oracle provided to the simulator as $I_{w, r}(\cdot, \cdot)$ where $w = w_1,..., w_\ell$ is the vector of obfuscated points and $r$ is the hidden value~(we will hide the same value in each obfuscation).
...
\section{An information-theoretic construction with $\Huse\le 0$}
\label{sec:info theory cons}
\bnote{needs to be rewritten depending on ordering}
In this section we show an information-theoretic analogue to \consref{cons:first construction}.  %This construction can essentially be seen as replacing the computational point obfuscation with information-theoretic randomness extractors.  
Using this weaker object requires us to make a stronger assumption on our starting distribution.  Furthermore, the important features of \consref{cons:sampling}, $\Huse \le 0$ for a small alphabet and reusability, crucially rely on computational assumptions.  Achieving these benefits information-theoretically is an open question.  We will construct an information-theoretic fuzzy conductor which can be converted to a fuzzy extractor using a randomness extractor~(using the information-theoretic analogue of \lemref{lem:cond and cext}).

The main idea of this construction is to first condense entropy from each block of the source and then apply a secure sketch to the extracted information.  Intuitively, we are then able from a large alphabet while paying entropy loss on the smaller alphabet.  %Alternatively, this construction can simply be seen as replacing the computational point function obfuscation with an information-theoretic point-function obfuscation~(this is only possible for high enough entropy levels).

\begin{construction}
\label{cons:info theoretic}
Let $\mathcal{Z}$ be an alphabet and let $W=W_1,..., W_\ell$ be a distribution over $\mathcal{Z}^\ell$.  We describe $\gen, \rep$ as follows:
\begin{center}
\begin{tabular}{c|c}
\begin{minipage}{3in}
\textbf{\gen}
...
\end{enumerate}
\item Set $ss \leftarrow \sketch(v_1,..., v_\ell)$.
\item Set $p = (ss, seed_1,..., seed_\ell)$.
\item Output $(v, p)$.
\end{enumerate}
 \end{minipage} &
\begin{minipage}{3in}
\textbf{\rep}
\begin{enumerate}
\item \underline{Input}: $(w', p = (ss, seed_1,..., seed_\ell))$
\item For $j=1,..., \ell$:
\begin{enumerate}[(i)]
\item Set $v_i = \cond(w_i, seed_i)$.
\end{enumerate}
\item Output $v = \rec(v', ss)$.
\end{enumerate}
\vspace{0.7in}
\end{minipage}
\end{tabular}
...
Let $\mathcal{W}$ be the family of $(\alpha, \beta)$-partial block sources over $\mathcal{Z}^\ell$ and let $\cond: \mathcal{Z} \times \zo^d\rightarrow \mathcal{Y}$ be a $(\alpha, \tilde{\alpha}, \epsilon)$-average case randomness conductor.  Let $(\sketch, \rec)$ be $(\mathcal{Y}^\ell, \tilde{\alpha}(\ell - \beta), \tilde{m}, t)$-secure sketch with error $\delta$.  Then the following is a $(\mathcal{Z}^\ell, \mathcal{W}, \tilde{m}, t)$-fuzzy conductor that is $\epsilon \ell$ hard with error $\delta$:

\end{lemma}

\subsection{Discussion of Information-Theoretic Construction}
\bnote{write this}

\section{Achieving Reusability \& $\Huse\le 0$}
\label{sec:sampling}
In the previous section, we showed it was possible to construct a fuzzy extractor for a family of distributions with $\Huse \le 0$.  In this section we show that the addition of computational techniques yields considerable extra power.  Using computational techniques we are able to retain many of the advantages of \consref{cons:info theoretic} and achieve a reusable fuzzy extractor.  This is known to be impossible in the information-theoretic setting.  
...
The main idea is to sample a random subset of blocks $W_{j_1},..., W_{j_\eta}$ and obfuscate the concatenation of these blocks.  Denote this concatenated value by $V_1$.  This process is repeated to produce $V_1,..., V_\ell$ where at least one $V_i$ should be correct to ``unlock'' the correct key.  We first present the necessary definitions and the construction.% and then compare it to \consref{cons:first construction}.  % and the construction proceeds by either obfuscating $V_i$ or a random point as before. 
For security each value $V_i$ needs to be unguessable.  
This will hold as long as enough blocks contribute some entropy:
...
%\end{itemize}
The use of a computational primitive~(obfuscation) allows us to sample multiple times, because we need to argue only about individual entropy of $V_i$, as opposed to the information-theoretic setting, where it would be necessary to argue about the entropy of the joint variable $V$.  Indeed, this is the property that allows this fuzzy extractor to be reusable~(we define reusability in \defref{def:reusable fuzz}).
...
Before we show \thref{thm:sampling} we present a simplified construction of \consref{cons:sampling}.  This construction essentially ignores the sampling step and asks when obfuscation provides security of the obfuscated values $V_1,..., \ell$.  Our security argument for \consref{cons:sampling} will reduce to this simplified construction.  After presenting this simplified construction we will return to showing \thref{thm:sampling}.

\subsection{Simplifying \consref{cons:sampling}}
In this section, we present a simplified version of \consref{cons:sampling}. The goal of this construction is to understand the type of sources that \consref{cons:sampling} supports.  Furthermore, this simplified construction has some advantages over \consref{cons:sampling}.\footnote{For the sake of clarity we defer discussion of \consref{cons:first construction} until \apref{sec:construction}.  In this section we focus on understand what information is leaked by point obfuscations and digital lockers.}
%Our goal is to derive strong keys for a large class of sources where $0\ge \Huse = \Hoo(W) - \log|B_t|$.
...
Let $\mathcal{Z}$ be an alphabet and let $V = V_1,..., V_\ell$ be a distribution over $\mathcal{Z}^\ell$.  Let $\mathcal{O}$ be an obfuscator for point functions with points from $\mathcal{Z}$.  Let  $C\subset \zo^\ell$ be an error-correcting code.
...
\textbf{\gen}
\begin{enumerate}
\item \underline{Input}: $v = v_1,..., v_\ell$
\item Sample $c\leftarrow C$.
\item For $j=1,..., \ell$:
\begin{enumerate}[(i)]
\item If $c_j = 0$: $p_j = \mathcal{O}(I_{v_j})$.
...
\textbf{\rep}
\begin{enumerate}
\item \underline{Input}: $(v', p)$
\item For $j=1,..., \ell$:
\begin{enumerate}[(i)]
\item If $p_j(v_j') = 1$: set $c_j' = 0$.
\item Else: set $c_j' = 1$.
\end{enumerate}
\item Set $c = \decode(c')$.
\item Output $c$.
...
The input $v$ is hidden in two different ways.  In locations where $c_j=1$, the block $v_j$ is information-theoretically unknown.
In locations where $c_j=0$, it is hard to find $v_j$ given access to the point obfuscation.
There are two possible reasons for a bit $c_j'$ to be $1$: because the true value was $1$ and because $v_j \neq v_j'$.  However, if a bit $c_j'$ is $0$, this likely means that $v_j=v_j'$ because collisions when $c_j=0$ are unlikely~(occurring with probability $1/|\mathcal{Z}|$).  This is the reason for the use of a code that only corrects $0\rightarrow 1$ flips.

\consref{cons:first construction} is secure if no distinguisher can tell whether it is working with random obfuscations or obfuscations of $V_j$.  By the security of point obfuscation, anything learnable from the obfuscation is learnable from oracle access to the function. Therefore, our construction is secure as long as enough blocks are unpredictable even after adaptive queries to equality oracles for individual symbols. This restriction on the distribution is captured in the following definition.
...
\end{cases}
\]
A source $V = V_1||...|V_\ell$ is a $(q, \alpha, \beta)$-\emph{unguessable block distribution} if there exists a set $J\subset\{1,..., \ell\}$ of size at least $\ell -\beta$ such that for any unbounded adversary $S$ with oracle access to $I_w$ making at most $q$ queries
\[
\forall j\in J, \Hav(V_j |View(S^{I_{V}(\cdot, \cdot)}))\geq \alpha.
\]
\end{definition}

We show some examples of unguessable block distributions in \apref{sec:characterize}.  In particular, any source $V$ where for all $j$, $\Hoo(V_j) \geq \omega(\log n)$~(but all blocks may arbitrarily correlated) is an unguessable block distribution~(\clref{cl:all blocks entropy}).

Intuitively, for a block unguessable distribution, the oracle provided by the obfuscation is not useful.  Indeed, we can show that \consref{cons:first construction} is a computational fuzzy conductor~(\apref{sec:construction}).  It is this observation that allows us to show the security of our \emph{sample-then-obfuscate construction}.  We will show that the output of the sampling part of the algorithm produces a unguessable-block distribution.
% For security we argue that each of the $v_i$ values is unguessable.  For correctness we show that with overwhelming probability at least of the $v_i$ and $v_i'$ match and allowing the digital locker to be opened and $r$ to be recovered.  Reusability comes essentially for free given a sufficiently composable obfuscation for digital lockers.  For the sake of clarity we state reusability as a separate theorem~(\thref{thm:reusability}).

\subsection{Security of \consref{cons:sampling}}
\label{ssec:sec cons sampling}
We now return to showing \thref{thm:sampling}.  
In order to show security \consref{cons:sampling}, we
show that with overwhelming probability, at each of the $\ell$ iterations, the sampler will choose enough coordinates of $W$ that have high entropy, making $V_i$ have sufficient entropy.   We can then argue that $V_1,..., V_\ell$ forms a block-unguessable distribution.  Then security essentially follows from the security of \consref{cons:first construction}. %follows very similarly to \lemref{lem:security of cons}~(used to show security of \consref{cons:first construction}).  
...
We begin by showing that each $V_i$ is statistically close to a high entropy distribution~(proof in \apref{sec:proof of sampling lemma}).   Let $\Lambda$ represent the random variable of all the coins used by $\sample$ and $\lambda=\lambda_1 \dots \lambda_\ell$
...
the inputs $V_1,..., V_\ell$ to \consref{cons:sampling} form a block unguessable distribution.  This is enough to show the probability that adversary sees a output other than $\perp$ from one of the obfuscations is negligible and that the adversary only has information about the key in this case.  
\begin{lemma}[Proof in \apref{sec:sampling unguess}]
...
\begin{lemma}[Proof in \apref{sec:sampling errors}]
...
The reusability of \consref{cons:sampling} essentially follows from the security of the VGB obfuscator with auxiliary input.  We consider a bounded $q = \poly(n)$ number of reuses.  For some fixed $i\in \{1,..., q\}$ we will treat the remaining keys as auxiliary input to the adversary, and the simulator must still be able to simulate.  Thus, given sufficiently strong reusability we achieve the following stronger result:

\begin{theorem}
\label{thm:reusability}
Let $q = \poly(n)$, and let all the variables be as in \thref{thm:sampling}, except that $\mathcal{O}$ be an $\ell\times q$-composable VGB obfuscator for digital lockers~(with $\kappa$ bit outputs) with auxiliary inputs.  Then \consref{cons:sampling} is $q$-reusable.
\end{theorem}
\begin{proof}
The only modification to the outline presented in \secref{ssec:sec cons sampling} is in \lemref{lem:samp unguess} with the other keys $R_1,..., R_{i-1}, R_{i+1}, ..., R_q$ treated as additional auxiliary input to the adversary/simulator.
\end{proof}

\subsection{Discussion of \consref{cons:sampling}}
\lnote{I need to check this}
We now show \consref{cons:sampling} can work for partial block sources when $\Huse\le 0$.  The required entropy of partial block source is $\alpha (\gamma-\beta ) = \Theta(\gamma)$.  We are able to correct $o(\gamma)$ errors.
This yields:
\[
\Huse = \Hoo(W) -\log |B_t| < \Theta(\gamma)- t \log |\mathcal{Z}|= \Theta(\gamma) - o(\gamma) \log |\mathcal{Z}|
\]
That is, there exists a super-constant alphabet size for which \consref{cons:sampling} is secure with $\Huse\le 0$.  We note that \consref{cons:sampling} works for an arbitrary size alphabet; however, for a constant size alphabet, $\Huse>0$.  However, we do achieve reusability for an arbitrary size alphabet, this has not been achieved in previous works.
...
\section{Proof of \lemref{lem:info theory sec}}
\begin{proof}
Let $W\in \mathcal{W}$.
It suffices to argue correctness and security.  We first argue correctness.  
When $w_i = w_i'$, then $\cond(w_i , seed_i) = \cond(w_i', seed_i)$ and thus $v_i = v_i'$.  Thus, for all $w, w'$ where $\dis(w, w')\le t$, then $\dis (v, v')\le t$.  Then by correctness of $(\sketch, \rep)$, $\Pr[\rec(v', \sketch(v)) = v]\ge 1-\delta$.  

We now argue security.  Denote by $seed$ the random variable consisting of all $\ell$ seeds and $V$ the entire string of generated $V_1,..., V_\ell$.
Our goal is to show that $\Hav(V | p)$ is close to a distribution with average min-entropy $\tilde{m}$.  We first argue that there exists a distribution $Y$ where $\Hav(Y | seed)\ge \tilde{\alpha}\beta$ and $(V, seed_1,..., seed_\ell)\approx (Y, seed_1,.., seed_\ell)$.  First note since $W$ is $(\alpha, \beta)$-partial block distribution that
there exists a set of indices $J$ where $|J| \geq \ell - \beta$ such that the following holds:
\[
\forall j\in J, \forall w_1,..., w_\gamma \in W_1,..., W_\gamma, \Hoo(W_j | W_1 = w_1,..., W_{j-1}=w_{j-1}) \geq \alpha.
\]
...
\[(\cond (W_{j_1}, seed_{j_1}), seed_{j_1}, W_1,..., W_{j_1-}) \approx_{\epsilon} (Y_{j_1}, seed_{j_1}, W_1,..., W_{j_1})\]
and since $(seed_1,..., seed_{j_1})$ are independent of these values
\[(\cond (W_{j_1},seed_{j_1}), W_{j_1-1},..., W_1, seed_{j_1}, ..., seed_{1}) \approx_{\epsilon} (Y_{j_1}, W_{j_1-1},..., W_1, seed_{j_1}, , ...,  seed_{1})\]
...
\begin{align*}
(\cond (W_{j_1}, seed_{j_1}), \cond(W_{j_1-1}, seed_{j_1-1}),..., \cond(W_1, seed_1), seed_{j_1},..., seed_{1}) \\\approx_{\epsilon} (Z_{j_1}, seed_{j_1},..., seed_1)
\end{align*}

\noindent
By a hybrid argument there exists a distribution $Z$ with $\Hav(Z | seed) \ge \alpha(\ell -\beta)$ where 
\[
(\cond(W_\ell, seed_\ell), ..., \cond(W_1, seed_1), seed_\ell,..., seed_1) \approx_{\epsilon\ell} (Z, seed_\ell,...,  seed_1).\]
By the security of $(\sketch, \rec)$ we know that $\Hav(Z | seed, ss) \ge \tilde{m}$.  %Note that $U_{\mathcal{Y}}^\ell$ is independent of $seed_1,..., seed_\ell$ and so $\Hav(U_{\mathcal{Y}}^\ell, seed_1,..., seed_\ell, p)\ge \tilde{m}$. 
That is, 
\[
(V, seed_1,..., seed_\ell, p) \approx_{\epsilon\ell} (U_{\mathcal{Y}}^\ell, seed_1,..., seed_\ell, p).\]  This completes the proof.
\end{proof}

\section{Analysis of \consref{cons:first construction}}
\label{sec:construction}
In this section we present analysis of \consref{cons:first construction}.  The following theorem states the construction is both correct and secure.  After proving this theorem we discuss when \consref{cons:first construction} is useful on its own.
\begin{theorem}
...
\end{itemize}
\end{proof}
\subsection{Proof of \lemref{lem:security of cons}}
...
\section{Proof of \lemref{lem:samp unguess}}
\label{sec:sampling unguess}
The proof of \lemref{lem:samp unguess} is similar to the proof of \lemref{lem:security of cons}.  The main difference is the oracle provided to the unbounded simulator $S$.  Instead repeating the entire proof, we show the following lemma~(analogous to \lemref{lem:codewords in I close}).  The ``computational'' part of the proof follows similarly by the definition of VGB obfuscation.
\begin{lemma}
Let $S$ be an unbounded simulator.  Let $V$ be a $(q, \alpha = \omega(\log n), 0)$-block unguessable distribution and let $I_{(V, R)}(i, x_i')=\begin{cases} R & v_i = x_i \\ \perp & \text{otherwise}\end{cases}$ be an oracle.  Let $r^*$ be the true output value in the oracle, then for any $u\in\zo^\kappa$,
\[
\Delta(S^{I_{V, R}(\cdot, \cdot)}(r, 1^{\ell \log |Z|}), S^{I_{V, R}(\cdot, \cdot)}(u, 1^{\ell \log |Z|}))\le \ell 2^{-\alpha}.
\]
\end{lemma}
\begin{proof}
Recall that for all $i, \Hav(V_i | View(S))\geq \alpha$.  The only information about the value of $R$ is contained in query responses. When all responses are $\perp$ the view of $S$ is identical when presented with $r^*$ or $u$.  We now show that for any value of $r^*$ all queries return $\perp$ with probability $1-2^{-\alpha}$.  Suppose not, that is suppose, the probability of at least one nonzero response for some index $i$ is at least $2^{-a}$.  When there is a response other than $\perp$ this means that $V_i$ has no remaining entropy.  If this occurs with over $2^{-\alpha}$ probability this violates the block unguessability of $F$.  By union bound over the indices $i$, the total probability of a non-$\perp$ response is at most $\ell 2^{-\alpha}$.  This concludes the proof.
\end{proof}
