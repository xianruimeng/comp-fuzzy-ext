...
The correctness guarantee is that $r$ will be correctly reproduced by \rep as long as $w'$ is no farther than $t$ from $w$ in some metric space (in this paper, we focus on the Hamming metric on length $\gamma$ strings over some alphabet $\mathcal{Z}$). \onote{parameters $\gamma$ and $\mathcal{Z}$ are not used until they are reintroduced in our contribution paragraph. Consider ignoring them here.}
...
We provide the first constructions of fuzzy extractors that can be used for large classes of sources that have more errors than entropy.  Our constructions work for Hamming errors for strings $w$ of length $\gamma$ over some alphabet $\mathcal{Z}$. Naturally, as argued above, these constructions cannot work for all sources of a given entropy; each construction comes with a constraint on the sources for which it is secure\onote{consider adding here that the class of sources we can deal with (in all constructions) contains many natural sources (or something along these lines).}.  Table~\ref{tab:upper bounds} summarizes our constructions. \lnote{Note sure about the first line of this table. What does ``exploit distribution structure'' mean? Also, can we make it look a bit better? Perhaps center. Perhaps also remove some lines. Perhaps make "main feature" column the same part of speech in each entry.}
\onote{The table is confusing to me: all the constructions exploits distribution structure. In the second construction most symbols must contribute fresh entropy as well. Third construction is also reusable. Perhaps there is a better way to organize this information.}
...
a constant fraction of symbols in $w$ may have little or no entropy, as long as knowledge of their values does not reduce the entropy of the high-entropy symbols too much (see \defref{def:block guessable}).  %\bnote{rewrite this!}
...
Construction & Security & Main Feature & Main Limitation \\
\hline
...
\paragraph{The Required Notion of Obfuscation} \onote{How about this paragraph instead of the one above?} Our constructions are base on obfuscation that is simulation-secure, however, we do not require full-fledged virtual black-box obfuscation \cite{barak2001possibility}. Instead, we rely on the relaxed notion of \emph{virtual grey-box} obfuscation\cite{bitansky2010strong}. We also require that the obfuscation remains secure even when several digital lockers of correlated points are composed.
Composable digital lockers with virtual grey-box security where constructed by Bitanski and Canetti under particular number-theoretic assumptions~\cite{bitansky2010strong}.
...
We note that fuzzy extractors for sources with errors than entropy can be trivially constructed from virtual grey-box obfuscation for the class of {\em proximity point programs}. A proximity point program $I_w(x)$ tests if $x$ is within Hamming distance $t$ of $w$. 
Recently, such obfuscation was constructed based on the strong assumption of {\em semantically secure graded encodings} \cite{PassTS13,BitanskyCKP14}.
This construction however, is still highly impractical, while the obfuscation used in this work can be instantiated quite efficiently under appropriate assumptions.
...
\lnote{why do we need this outsider/insider distinction here? push everything in this paragraph to the discussion of the definition in the appendix}
...
%\end{align*}
%\end{definition}
We modify the definition of Boyen~\cite[Definition 7]{Boyen2004} for the computational setting.  We discuss the differences between our definition and the definitions in Boyen's work \apref{sec:comparing reusability}. \lnote{why do we call it ``outsider''?}
