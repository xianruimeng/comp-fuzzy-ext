...
\label{subsec:LWE}
The $\LWE$ problem was introduced by Regev \cite{regev2005LWE, regevLWEsurvey} as a generalization of ``learning parity with noise." We now recall the definition of the decisional version of the problem. 
...
Let $\bar{\Psi}_\rho$ be the discretized Gaussian distribution with variance $(\rho q)^2/2\pi$, where $\rho \in (0,1)$ with $\rho q > 2\sqrt{n}$.  If GAPSVP and SIVP are hard to approximate~(on lattices of dimension $n$) within polynomial factors for quantum algorithms, then $\distLWE_{n, m, q, \bar{\Psi}_\rho^m}$ is secure. 
...
\begin{theorem}
\label{thm:blockLWE}
Let $n$ be a security parameter, $\alpha$ be polynomial in $n$, and $\beta=\beta(n)=\omega(1)$. \lnote{rephrase so that $q=q(n)$ and$q^{-\beta}$ is negl in $n$}
...
Let $n$ be a security parameter and let $t = c\log n$ for some positive constant $c$.  Let $d\le c$ be a positive constant and consider the Hamming metric over the alphabet $Z=[-2^{b-1},2^{b-1}]$, where $b \approx \log 2(c/d+2)n^2 = O(\log n)$.  Let $\mathcal{M} = Z^{m+\alpha}$ where $m= (c/d+2)n=O(n)$ and $\alpha \leq n/3$. 
Let $\mathcal{W}$ be the class of all $(m+\alpha, m, |Z|)$-symbol fixing sources.  If GAPSVP and SIVP are hard to approximate within polynomial factors 
...
\begin{proof} Security follows by Lemmas~\ref{lem:uniform LWE decision} and~\ref{lem:many hardcore bits} and  \thref{thm:blockLWE}.  Efficiency follows by \lemref{lem:i t poly time}.  For a more detailed explanation of parameters see \secref{ssec:block params}. \lnote{to satsify \thref{thm:blockLWE} condition on $\beta$ is easy here, because $q$ is at least $n^2$, so superconstant $\beta$ is enough}
\end{proof}

%\textbf{Comparison with \thref{thm:lossless secure extractor log}}
%%\thref{thm:lossless block sketch log} is a direct consequence of \thref{thm:blockLWE} and parameter settings~(discussed in  \secref{ssec:block params}).  
%The weaker entropy source means we are able to extract fewer variables~($n/3$ instead of $n/2$) and that we must work over a slightly larger field.  We could set $\alpha$ larger than $n/3$.  This would yield fewer hardcore dimensions and force us to use a larger field.  To extract enough bits and keep the field size polynomial in $n$, $n-\alpha$ must be at least a constant fraction of $n$.  We consider the setting where the number of fixed errors, hardcore dimensions, and dimensions used for randomness are all $n/3$.
%The parameter $\beta$ does not significantly impact the asymptotic behavior but is important for small values of $n$.

\ignore{
\subsection{\consref{cons:informal construction} with slightly deficient sources}
\label{sec:DP general error LWE}
In this section, we discuss how to generalize \consref{cons:informal construction} to an arbitrary high min-entropy distribution.  We will show security using a result of Micciancio and Peikert~\cite{micciancio2013hardness}.  This result allows us to argue security for an arbitrary distribution that is nearly high entropy.  This result should be seen as a complement to the proceeding sections.  It allows for a more general error distribution, but with worse parameters than \thref{thm:lossless secure extractor log}.  Also \thref{thm:lossless block sketch log} allows for more deficient sources but of a more restricted form~(symbol fixing sources).
\begin{theorem}
...
%\begin{lemma}
%Assume $(\sketch, \rec)$ is an $(\mathcal{M}, m, \tilde{m}, t)$-computational fuzzy extractor that is $(\epsilon, s_{sec})$ hard with error $\delta$ and denote the range $\rec$ by $\chi$.  Let $\ext:\chi \times \{0,1\}^d\rightarrow \{0,1\}^\ell \times \{0, 1\}^d$ be an $(\tilde{m}, \epsilon_{ext})$ extractor.  Then the following $(\gen , \rep)$ is an $(\mathcal{M}, m, \ell, t)$-computational fuzzy extractor that is $(\epsilon + \epsilon_{ext}, s_{sec}- s_{ext})$-hard with error $\delta$.
%\end{lemma}
...
\begin{proof}
We assume that all of the fixed blocks are located at the end and their fixed value is $0$, that is, $W_{m+1}, ..., W_{m+\alpha} =0$ \lnote{are we using $W$ or $\chi$ notation?}.  If the blocks are fixed to some other value, the reduction is essentially the same\lnote{isn't the point of a block fixing source that we don't know which values are fixed and to what? At least in the extractor literature, that would seem to be crucial---else, extraction is trivial}.  For a matrix $\vA$ we will denote the $i$-th row by $\va_i$.  For a set $T$ of column indices, we denote by $\vA_T$ the restriction of the matrix $\vA$ to the columns contained in $T$.  Similarly, for a vector $\vx$ we denote by $\vx_T$ the restriction of $\vx$ to the variables contained in $T$.  We use similar notation for the complement of $T$, denoted $T^c$.
...
Let $n$ be a security parameter, $m ,q , \alpha= \poly(n)$.  Let $\beta = \omega(1)$.  All operations are computed modulo $q$, and we omit $``\bmod q$'' notation.
Let $D$ be a distinguisher that breaks $\distLWE_{(m+\alpha), (n+\alpha+\beta), q, \chi'}$ with advantage $\epsilon>1/\poly(n)$m
where $\chi'_i = \chi_i$ for $i\leq m$ \lnote{what's $\chi_i$?} and $\chi_i \overset{d}= 0$ otherwise. \lnote{we should probably swap $\chi$ and $\chi'$ to be consistent about the use of primes} Let  $\vA$ denote the uniform distribution over $\Fq^{(m+\alpha)\times(n+\alpha+\beta)}$, $X$ denote the uniform distribution over $\Fq^{(n+\alpha+\beta)}$, and $U$ denote the uniform distribution over $\Fq^{m+\alpha}$ . Then
\[
...
\item For all $j\notin T$, $1\le j \le \alpha+\beta$:\lnote{swapped $i$, $j$ and shifted}\label{step:fill in matrix}
...
\end{claim}
\begin{proof}
Direct result of \clref{cl:full rank matrix} when $\beta = \omega(1)$ in $n$. \lnote{rephrase to say that it follows from the assumption that $q^{-\beta}$ is negligible.}
\end{proof}
\begin{corollary}
$
...
$
\end{corollary}
\lnote{why is this a corollary?  A corollary should follow from something above it, usually}

We are interested in the case where $\rank(\vS) = \alpha$, and it suffices to show that $D'$ prepares a good instance for $D$ in this case. We show this in the following three claims: 
\begin{enumerate}
...
\item $x_i = x_i'$ for $1\leq i \leq n$,
\item and $x_i$ is uniform and independent of $\vA$ and $\ve'$ for $n<i\le n+\alpha+\beta$,
\end{itemize}
\end{claim}
\begin{proof}
Partially define $\vx$ as $x_i = x_i'$ if $1\leq i \leq n$ and $x_i$ as the value generated in step~\ref{step:fill in matrix} for $i>n $ and $i\not\in T$.  Define the remaining variables $\vx_T$ as the solution to the following system of equations.
\[
\vS_T  \vx_T = \begin{pmatrix} b_{m+1}^*  \\ \vdots \\b_{m+\alpha}^* \end{pmatrix}  - \vR \vx' - \vS_{T^c} \vx_{T^c}  
\]
A solution $\vx_T$ exists as $\vS_T$ is full rank. Moreover, it is uniform and independent of $\vA$ and $\ve'$, because $b^*_{m+1}, \dots, b^*_{m+\alpha}$ are uniform and independent of $\vA$ and $\ve'$. Note that $\vb^*=\vA^*\vx+\ve$ before step~\ref{step:randomization} \lnote{this requires careful attention to the algorithm and the definition of $\vx$, not much different from the steps below. Why do the steps below get the attention and this one doesn't? Perhaps for the above stuff say "by inspection, particularly Step 6, and for below part say "by inspection, we show it formally below"}.
...
\lnote{I would delete this paragraph and the next one, and replace with the one below} We will now show that $\vb\,|\,(\vA, \vA^*)$ is uniformly distributed, therefore, showing that $\vb\,|\,\vA$ is uniform.  Indeed, let $\vgamma$ represent the matrix formed by $\gamma_{i}$.  
 
 The matrix $\vgamma$ is a deterministic function of $(\vA, \vA^*)$ \lnote{why?  And does it depend on the choice of the set $T$ in step 5 being deterministic? and do you need this? Can't you just write $\vb\,|\,\vA=\vb\,|\,(\vA^*,\Gamma)$, because $\vA$ is a function of $\vA^*, \Gamma$?}.  Thus, $\vb\,|\,(\vA, \vA^*)$ has the same distribution as $\vb\, |\,(\vA, \vA^*, \vgamma)$.  The only dependence of $\vb$ on $(\vA, \vA^*, \vgamma)$ is in the matrix $\vgamma$\lnote{not sure what the previous sentence means}.  Thus, $ \vb | (\vA, \vA^*, \vgamma) \overset{d}= \vb | \vgamma$. Recall that 
 \[
...
\lnote{I got confused by the two paragraphs above --- see comments --- so I wrote it differently.  If you agree, delete the two paragraphs above.}
...
&\, \geq (\epsilon - \ngl(n))(1-\ngl(n)) \approx \epsilon
\end{align*}
\lnote{change notation above to match notation at the beginning of this section; also, note that the first line follows from teh coinflip if rank is not right} Thus, Equation~(\ref{eq:block LWE dist}) is satisfied, this completes the proof.
\end{proof}
