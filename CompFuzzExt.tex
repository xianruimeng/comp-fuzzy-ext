...
Sketch takes $O(n^3)$ operations over $\Fq$, and recover takes expected time $O(n^{4d+3})$ operations over $\Fq$.
...
\consref{cons:informal construction} is a $(\M, bm, bm, t)$-computational fuzzy extractor that is $(\epsilon, s_{sec})$-hard with error $\delta = e^{-\Omega(n)}$. Sketch takes $O(n^3)$ operations over $\Fq$, and recover takes expected time $O(n^{4d+3} \log n)$ operations over $\Fq$.
\end{theorem}
