...
We first study whether improvements are possible by relaxing the definition of the main building block of fuzzy extractors, called \emph{secure sketch}.  A secure sketch provides information reconciliation: it  produces a public value $s$ that allows recovery of $w$ from any close value $w'$ (The key $r$ is formed by the fuzzy extractor by applying a randomness extractor~\cite{nisan1993randomness} to $w$.)  The traditional secrecy requirement of a secure sketch is that $w$ has high min-entropy conditioned on $s$. 
