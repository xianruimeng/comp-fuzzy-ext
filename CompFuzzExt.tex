...
\end{lemma}
Thus, using a sketch that retained unpredictability entropy and a reconstructive extractor we could build a computational fuzzy extractor~(in a similar fashion to \lemref{lem:fuzzy ext construction}).  Unfortunately, we show that the unpredictability entropy must decrease with the $O(\log B_t(\cdot))$.  
\begin{theorem}
...
\end{proof}
This result has a weaker condition than \corref{cor:rec yields sketch} and yields a weaker conclusion.  The two statements are incomparable.
