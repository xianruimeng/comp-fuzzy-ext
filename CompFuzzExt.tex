...
\newcommand{\hillrlx}{\ensuremath{\mathtt{HILL-rlx}}\xspace}
...
In \secref{sec:imp HILL sketch} we show that a sketch that retains HILL entropy implies a sketch that retains nearly the same amount of min-entropy.  In \secref{sec:imp unp sketch}, we show that the unpredictability of a sketch is at most $n - |B_t(\cdot)|$.  For comparison, random linear codes achieve unpredictability $n(1-o(1)) - B_t(\cdot) $.
...
denoted $H^{\hill-rlx}_{\epsilon, s_{sec}}(W|X)\geq k$ if there exists a joint distribution $(Y, Z)$, such that $\tilde{H}_\infty(Y|Z)\geq k$ and $\delta^{\mathcal{D}_{s_{sec}}} ((W, X),(Y,Z))\leq \epsilon$.
\end{definition}
...
We can now formalize the intuition above.  A sketch that retains $\tilde{m}$-bits of relaxed HILL entropy implies a good error correcting code with nearly $2^{\tilde{m}}$ points~(proof in \secref{sec:proof of thm sketch implies code}).
...
\]  
where  $s_{sec} \geq t(s_{neigh}+s_{rec})$.  One such $(Y, Z)$ must exist by the definition of conditional HILL entropy.
...
$\delta^D((W, \sketch(W)), (Y, Z))<\epsilon$, we know $\Pr[D(Y, Z) = 1]>1-\epsilon-t\delta$.  By Markov's inequality,  there exists a set $S_Z$ such that $\Pr[Z\in S_Z]\ge 1/2$ and for all $z\in S_Z$, $\Pr[ D(Y|z, z) =1]> 1- 2(\epsilon + t\delta)$.  Moreover, by \cite[Lemma 2.2a]{DBLP:journals/siamcomp/DodisORS08} (which is derived simply via Markov's inequality applied to the positive random variable $\max_y \Pr[Y_s=y]$), there exists a set $S'_Z$ such that $\Pr[z\in S'_Z]> 1/2$ and for all $z\in S'_Z$, $\Hoo(Y | z)\ge k-1$. \lnote{simplified this derivation}\bnote{I don't believe this fix}  Fix one value $z \in S_Z\cap S'_Z$ (which exists because the sum of probabilities of $S_Z$ and $S'_Z$ is greater than 1).  
Thus, for all such that $t', 1\leq t'\leq t$, 
\[ \Pr_{y\leftarrow Y|z}[y'\leftarrow \sample(y, t') \wedge \rec(y',z) = y]> 1-2(\epsilon+t\delta).\]  
...
Thus,  $Y|z$ is a $(t, 2(\epsilon+t\delta))$-average error Shannon code with recovery $\rec(\cdot,z)$ and $2^{k-1}$ points.  The statement of the theorem follows by application of \lemref{lem:averageToMaximalError}.  
\end{proof}
