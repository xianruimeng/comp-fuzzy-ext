...
\begin{construction}
Let $n$ be a security parameter and let $m> n$\bnote{should we just switch to $m\ge 3n$ here?}.  Let $q$ be a prime. %Let $W = W_1,..., W_m$ and let $b$ be a constant and let $W_i \in \zo^b$ for all $i$.  
...
\label{subsec:LWE}
The $\LWE$ problem was introduced by Regev \cite{regev2005LWE, regevLWEsurvey} as a generalization of ``learning parity with noise." For a positive integers $n, m$,  a prime $q$, a vector $\vx \in \Fq^n$, and a probability distribution $\chi$ on $\Fq$, let $A_{\vect{x}, \chi}$ \bnote{only two places we use this notation.}be the distribution obtained by choosing $\vA \overset{\$}\leftarrow \Fq^{m\times n}$ uniformly at random and a noise term $\ve \overset{\$}\leftarrow \chi^m$, and outputting $(\vA, \vA\vx+\ve)$. 
...
Let $m = m(n) = \poly(n)$ be an integer and $q = q(n) = \poly(n)$ be a prime (unlike in common formulations of LWE, where $q$ can be any integer, we need $q$ to be prime for decoding)\bnote{does this belong in a definition?}. The decisional version of the $\LWE$ problem, denoted \class{dist}-$\LWE_{n, m, q, \chi}$, is to distinguish between $m$ samples chosen according to $A_{\vx, \chi}$ \bnote{only two places we use this notation.} for a uniformly random $\vx$ from samples chosen according to the uniform distribution over $(\Fq^{m\times n}, \Fq^m)$. 
...
\begin{proposition} 
\bnote{Why is this not a theorem?  For example Theorem 1 from Reg05?}
...
To extract pseudorandom bits, we use a result of Akavia, Goldwasser, and Vaikuntanathan~\cite{akavia2009} to show that $X$ has many hardcore bits\bnote{Do we need to cite anything about hardcore?}.  The result says that if $\distLWE_{(n-k, m, q, \chi)}$ is secure then any $k$ variables of $X$ in a $\distLWE_{(n, m, q, \chi)}$ instance are hardcore.  We state their result for a general error distribution~(noting that their proof does not depend on the error distribution):
...
Let $d$ be a positive constant and assume that $\dis(W, W')\leq t$ where $t\leq d(\frac{m}{n}-2)\log n$.  Then $\decode_t$ runs in expected time $O(n^{4d+3})$ operations in $\Fq$ (the expectation is taken over the input distribution\bnote{this needs to be more precise} $\vA, \vA X+W - W'$).  It outputs $X$ with probability $1-e^{-\Omega(n)}$ (this probability is over the choice of the random matrix  $\vA$).
\end{lemma}
\textbf{Note:}
...
%\lnote{removed (on lattices of dimension $n/6$)} 
