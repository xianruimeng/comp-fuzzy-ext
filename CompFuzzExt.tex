...
\label{lem:many hardcore bits}
 If $\distLWE_{(n-k, m, q, \chi)}$ is $(\epsilon, s_{sec})$ secure then \lnote{notation doesn't exactly match def 4.2}
...
Let $n$ be a security parameter.  Let $q=q(n)=\poly(n)$ be a prime and  $m = m(n)=\poly(n)$ be an integer.  Let $W = W_1||...||W_{m+\alpha}$ be an $(m+
\alpha, m, |Z|)$ symbol fixing source over  $Z^{m+\alpha}$ for the alphabet $Z=[-2^{b-1},2^{b-1}]$. For a vector $X \leftarrow \Fq^n$, let $A_{X, W}$ be the distribution obtained by choosing a uniform $\vA\in\Fq^{(m+\alpha)\times (n+\alpha)}$ and outputting $\vA X + W$. The decisional learning with errors problem $\distLWE_{n+\alpha, m+\alpha, q, W}$ withAt symbol fixing errors is to distinguish samples chosen according $A_{X, W}$ \lnote{why is $X$ a subscript in this notation by A is uniform? why not both?}   \lnote{notation doesn't exactly match def 4.2}
for a uniformly random $X \in \Fq^{n+\alpha}$ from samples chosen according to the uniform distribution over $\Fq^{n+\alpha} \times \Fq$.
\end{problem}
...
\begin{theorem}
\label{thm:blockLWE}
 \lnote{notation doesn't exactly match def 4.2}
...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in\mathbb{Z}^+$ be a constant and consider the Hamming metric over the alphabet $Z=[-2^{b-1},2^{b-1}]$, where $b \approx \log (2n^3 (c+2d)/d) = O(\log n)$ \lnote{this may need to be modified after App D.1 calculation}.  Let $\alpha \leq n/3$ and $m=(c+2d)n/d = O(n)$, and let $\mathcal{W}$ be the class of all $m$-symbol fixing sources over $Z^{m+\alpha}$.  If GAPSVP and SIVP are hard to approximate within polynomial factors \lnote{removed (on lattices of dimension $n/6$)} using quantum algorithms, then  there is a setting of $q = \poly(n)$ such that for any polynomial $s_{sec} = \poly(n)$
