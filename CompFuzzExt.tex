...
This approach (specifically, its analysis via \lemref{lem:fuzzy ext construction}) works as long as the amount of entropy $\tilde{m}$ of $w$ conditioned on the sketch $s$ remains high enough to run a computational extractor.  However, $\tilde{m}$ decreases with the error parameter $t$ due to coding bounds, and it is conceivable that, if $W$ has just barely enough entropy to begin with, it will have too little entropy left to run a computational extractor once $s$ is known.

In contrast, in our approach, the entropy of $w$ conditioned on $p=(\vA, \vA X+w)$ is not relevant to security, because we do not extract from $w$. Instead, we require that $w$ is not recoverable computationally given $p$.  Note that this requirement does not imply that $w$ can be extracted from, because computational extractors may require a longer minimum input length than the computational entropy of $w$.   Our approach works in such a setting because instead of extracting from $w$, we hide secret randomness using $w$.

The main advantage of our analysis (instead of sketch-and-comp-extract) is that security need not depend on the decoding radius $t$.  Our codes (namely, random linear codes) can tolerate very high $t$, in fact.  In the case of secure sketches, codes tolerating such high $t$ would leave little entropy in $w$.  In our construction, the decoding radius depends only on the best available decoding algorithm for random linear codes.  

\lnote{use any of the below?}
 utilize secret randomness to obtain the key $r$, and hide that randomness using $w$.  

The code defined by $\vA$ can correct very high 
