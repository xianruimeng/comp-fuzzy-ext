...
\end{itemize}
\end{lemma}
The main parameter we will be concerned with is the entropy loss of the construction.  We relax the security requirement of \defref{def:fuzzy extractor} to require a pseudorandom output instead of a truly random output.
...
It seems natural to construct a computational fuzzy extractor using a sketch that retains computational entropy and then a randomness extractor.  In the next section, we show this paradigm is difficult to realize. 
...
In \secref{sec:imp HILL sketch} we show that a sketch that retains HILL entropy implies a sketch that retains nearly the same amount of min-entropy.  In \secref{sec:imp unp sketch}, we show that the unpredictability of a sketch must decrease with the number of neighbors of a point in the metric space.  These results are strongest for high entropy sources.  The results get weaker as we consider sources with lower starting entropy.  If a source contains points from a good error code, then the sketch procedure does not need to output any public value so no drop occurs in entropy.  
...
If $s_{sec}=\Omega(t(s_{rec} + n))$, then there exists a   $(Z^n, n\log |Z|, \tilde{m}-2,t)$ (information-theoretic) secure sketch with error
$4(\epsilon+\delta)$. 
...
.\]
\end{definition}
We will say a pair of procedures $(\sketch, \rec)$ is a \emph{unpredictability-entropy}$(\mathcal{M}, m, \tilde{m}, t)$ secure sketch that is $(\epsilon, s_{sec})$-hard with error $\delta$ if it satisfied \defref{def:secure sketch} with the security requirement replaces with $H^{\unp}_{\epsilon, s_{sec}}(W| \sketch(W))\geq \tilde{m}$.  
...
\end{lemma}
Thus, using a sketch that retained unpredictability entropy and a reconstructive extractor we could build a computational fuzzy extractor~(in a similar fashion to \lemref{lem:fuzzy ext construction}).  

Unfortunately, unpredictability entropy must also depend on $t$.  An adversaries ability to predict the output of the \rec algorithm increases with as $t$ grows.  %That is, $\forall Y$ such that $\delta^{\mathcal{D}_{s_{sec}}}((W, \sketch(W)) , (Y, \sketch(W))$ our ability to predict $Y$ increases as $|B_t(\cdot)|$ increases.  
The formal statement is below:
\begin{theorem}
\label{thm:imp of unp entropy}
Let $W$ be a distribution over $\zo^b$ and let $(\sketch, \rec)$ be an Unpredictability-entropy $(\zo^b, \Hoo(W), \tilde{m}, t)$-secure sketch that is $(\epsilon, s_{sec})$-secure with error $\delta$.  If $s_{sec} = \Omega(|\rec|+b)$, then $\tilde{m}\leq b - \log |B_t(\cdot)| + \log(1-\epsilon -\delta)$.
\end{theorem}
...
The lower bounds of \corref{cor:rec yields sketch} and \thref{thm:imp of unp entropy} are strongest for high entropy sources.  The results get weaker as we consider sources with lower starter entropy.  This is necessary, if a source contains only codewords (of an error correcting code), no sketch is necessary.  This means we cannot rule out computational sketches for other arbitrary lower entropy distributions.  

Both of the lower bound results arise because the reduction can ensure that \rec functions as an error-correcting code for many points of the output distribution.  It may be possible to avoid this problem if \rec outputs a fresh random variable\footnote{If some efficient algorithm can take the output of $\rec$ and efficiently transform it back to the source $W$, the bounds of \corref{cor:rec yields sketch} and \thref{thm:imp of unp entropy} both apply.  This means that we need to consider constructions that are hard to invert~(either information-theoretically or computationally).}.  An algorithm that outputs a fresh random variable would be a computational fuzzy conductor~(see~\cite{KanukurthiR09} for the definition of a fuzzy conductor).  Our construction~(in \secref{sec:fuzzyCompExt}) has pseudorandom output and immediately satisfies definition of a computational fuzzy extractor~(\defref{def:comp fuzzy extractor}).  It is unknown if a more efficient construction could be achieved using a computational fuzzy conductor that is not also a computational fuzzy extractor.
\bnote{Not happy with this wording, revise}
...
In this section we describe our main construction.  Our construction uses the code-offset construction~\cite{JW99},\cite[Section 5]{DBLP:journals/siamcomp/DodisORS08} instantiated with a random linear code over a finite field $\Fq$.   Let $\mathcal{I}_t$ be an algorithm that decodes a random linear code with at most $t$ errors~(not necessarily efficient).  We provide an informal description below.
...
We describe conditions for efficient decoding in \secref{sec:time main construction}.  In \secref{sec:lossless extractor} we describe parameter settings where our construction yields a lossless secure sketch.  In \secref{sec:prg based comparison} we compare \consref{cons:informal construction} to using a information-theoretic fuzzy extractor and running a pseudorandom generator on the output.\lnote{revise after going through the rest}.
...
Denote by $\bar{\Psi}_\alpha$ the discretized Gaussian distribution over $\Fq$ with variance $(\alpha q)^2/(2\pi)$, Regev\cite{regev2005LWE} and Peikert \cite{peikert2009latticereduction} shows that, when $\chi = \bar{\Psi}_{\alpha}$, $\class{dist}$-$\lwe_{n, m, q, \chi}$ is secure if the worst-case lattice problems are hard.
...
Let $n$ be a security parameter and let $t = c\log n$ for some constant $c$.  Let $d\in\Fq^+$ be a constant and consider the Hamming metric with block length $b = \log (2n^3 (c+d)/d)$.  Let $W$ be $\alpha$-symbol fixing source over $\zo^{((c+d)n/d+\alpha)b}$.  Assume GAPSVP and SIVP are hard to approximate within polynomial factors using quantum algorithms.   There is a setting of $q = poly(n)$ such that \consref{cons:informal construction} is a lossless conductor.  That is, $H^{\hill}_{\epsilon, s_{sec}}(X_{1,..., n/2} |\sketch(W)) = H_\infty(W)$ for %the following parameters:
%\[
...
\item Output $1$.
\end{enumerate}
  By correctness of the sketch $ \Pr[D(W, \sketch(W)) =1]\ge 1-\delta$.  Since 
$\delta^D((Y, \sketch(W)), (Y, \sketch(W)))<\epsilon.$ then $\Pr[D(Y, SS(W)) = 1]>1-\epsilon-\delta$.  By Markov inequality there exists a set $S_W$ such that $\Pr[S_W]\geq 1/2$ and $\Pr[ D(Y, s) =1 | \sketch(W) = s \wedge s\in S_W]> 1- 2(\epsilon + \delta)$.  Consider the random variable $1_{S_W}$ that is $1$ when $s\in S_X$ and otherwise.  Then 
\[
...
\[ \Pr_{y\leftarrow Y_s}[y'\leftarrow \sample(y, t') \wedge \rec(y',s) = y]\geq 1-2(\epsilon+\delta).\]  
...
Thus,  $Y$ is a $(t, \epsilon)$-average error Shannon code with recovery $\rec(\cdot,s)$.  The statement of the theorem follows by application of \lemref{lem:averageToMaximalError}.  
\end{proof}
...
For a distribution $(X, Z)$, we say that $X$ has \emph{relaxed unpredictability entropy} at least $k$ conditioned on $Z,$ denoted by $H^{\unp}_{\epsilon, s_{sec}} (X|Z) \geq k$, if there exists a joint distribution $Y, Z'$ such that $\delta^{\mathcal{D}_{s_{sec}}}((X, Z),(Y, Z'))\leq \epsilon$, and for all circuits $C$ of size $s_{sec}$,
\[
\Pr[C(Z) = Y ] \leq 2^{-k}
.\]
\end{definition}
...
\begin{theorem}
Let $W$ be a distribution over $\zo^b$ and let $(\sketch, \rec)$ be a relaxed unpredictability-entropy $(\zo^b, \Hoo(W), \tilde{m}, t)$-secure sketch that is $(\epsilon, s_{sec})$-secure with error $\delta$.  If $s_{sec} = \Omega(|\rec|+b)$, then $\tilde{m}\leq b - \log |B_t(\cdot)| + \log(1-\epsilon -\delta)$.

\end{theorem}
\begin{proof}
Let $W$ be a distribution over $\zo^b$ and let $(\sketch, \rec)$ be an relaxed unpredictability-entropy $(\zo^b, \Hoo(W), \tilde{m}, t)$-secure sketch that is $(\epsilon, s_{sec})$-secure with error $\delta$ where $s_{sec} = \Omega(|\rec|+b)$.  Let $Y, Z'$ be a joint distribution such that $\delta^{\mathcal{D}_{s_{sec}}}((W, \sketch(W)), (Y, Z'))\leq \epsilon$.  It suffices to show that $\exists \mathcal{I}$ of size $s_{sec}$ such that $\Pr[\mathcal{I}(Z') = Y]\geq 2^{\tilde{m}}$ where $\tilde{m} = n - \log |B_t(\cdot)| + \log(1-\epsilon -\delta)$.  We begin by showing that \rec must recover points of $Y$.
\begin{claim}
...
\[
 \Pr[(y, z\leftarrow (Y, Z')] \sum_{y'\in \zo^b} \Pr[B_t(y) =y' | Y = y] P_{y',  z, y} \geq 1-\delta -\epsilon.
\]
...
&\geq \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in \zo^b} \Pr[X = y' ]\Pr[\mathcal{I}(z) = y | X = y']\\
&\geq \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in \zo^b} \Pr[X = y' ] P_{y', z, y}\\
&\geq \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in \zo^b} \frac{\Pr[X = y']}{\Pr[B_t(y) = y' | Y = y]}\Pr[B_t(y) = y' | Y=y ] P_{y', z, y}\\
&\geq  \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in \zo^b} \frac{2^{-b}}{1/|B_t(\cdot)|} \Pr[B_t(y) = y' | Y=y ] P_{y', z, y}\\
&\geq \frac{|B_t(\cdot)|}{2^b} \Pr[(y, z)\leftarrow (Y, Z')] \sum_{y'\in \zo^b} \Pr[B_t(y) = y' | Y=y ] P_{y', z, y}\\
...
\end{align*}
Furthermore, we get decoding in time $O(n^{2d}(n^3\log n+s_{ver}))$.  We can output a $(g-1)/g$ fraction of $X$ and the bits will be pseudorandom.  In \thref{thm:lossless secure conductor log}, we set $g=2$ and output the first half of $X$.  By setting $g>2$, \consref{cons:informal construction} achieves a linear stretch in output size.
