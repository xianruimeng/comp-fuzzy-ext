...
In order to decode a meaningful number of errors we need $m>n>k$.  However, dimensions of $w$ are sampled with fewer bits than dimensions of $X$.  That is, $\log \rho q < \log q$.  Thus, by increasing the size of $q$ we can set $k\log q = m\log \rho q$, yielding a key of the same size as our source.    The formal statement is below. 
