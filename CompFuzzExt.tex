...
\consref{cons:informal construction} is a $(\M, bm, bm, t)$-computational fuzzy extractor for any symbol-fixing distribution symbol $W$ such that it is $(\epsilon, s_{sec})$-hard with error $\delta = e^{-\Omega(n)}$. The generate procedure $\gen$ takes $O(n^2)$ operations over $\Fq$, and the reproduce procedure $\rep$ takes expected time $O(n^{4d+3} \log n)$ operations over $\Fq$.
\end{theorem}
