...
superlogarithmic fuzzy min-entropy  is \emph{necessary} for nontrivial key extraction~(we show this in \propref{prop:fuzz necessary}, which formalizes the above intuition). 
...
We show comparable, but stronger, results for secure sketches.  Namely, we show in \thref{cons:leveling} that secure sketches are possible if the distribution $W$ is precisely known. (In fact, we obtain our fuzzy extractors for the case of a known distribution from this result by applying a randomness extractor.) On the other hand, there is a family of sources with superlogarithmic $\Hfuzz(W)=\Hoo(W)$ for which no secure sketch correcting even a few errors is possible~(\thref{thm:imposs sketch}). The impossibility result applies even when $\rec$ is allowed to be incorrect with probability up to $1/4$ (as opposed to our fuzzy extractor impossibility result).
...
The result also relies on the majority of points lying near boundaries of a set. Indeed, fuzzy extractors for arbitrary distributions are known in metric spaces where the boundary of a set does not consistent the majority of its area.\bnote{cite cite cite!!!}
...
Our goal is to provide a superlogarithmic length key when provided with superlogarithmic fuzzy min-entropy.  We now describe distribution-aware algorithms.
...
\section{Sufficiency of $\Hfuzz(W)$ for Distribution-Aware Algorithms}
...
$
For a flat distribution this is within a superlogarithmic factor of optimal~(see Equation~\eqref{eq:fuzz for flat}). Thus, for flat distributions, we can construct a secure sketch that is correct with overwhelming probability while retaining a superlogarithmic amount of entropy~(by choosing $\delta$ based on $\Hfuzz(W)$).  

\subsection{Secure sketches for all distributions with $\Hfuzz(W)$}
The hashing approach just described does not work for arbitrary sources.  The reason is that some balls may have significantly more points but low overall probability.  Let $W$ be a distribution, denote by $B^1_t$ a ball with $2^{\Hoo(W)}$ points but with probability $\Pr[W\in B^1_t] =2^{-\Hoo(W)}$ and let $B^2_t,..., B^{2^{-\Hoo(W)}}_t$ be balls with one point each with probability $\Pr[W\in B^i_t] = 2^{-\Hoo(W)}$.  Then the hashing algorithm needs to write down $\Hoo(W)$ bits to achieve correctness.  However, with probability $1-2^{-\Hoo(W)}$ the point $w_0$ lies outside of $B^1_t$ and the hash completely reveals the point.  
...
\section{Impossibility of $\Hfuzz(W)$ for Distribution-Oblivious Secure Sketches}
...
\item For most values of possible $Z$, the intersection between the viable pre images of $V|Z$ and $V_\key$ is empty.  Thus, for most values of $Z$, any key lying in $R_{small}$ can be labeled as ``random''.
\end{itemize}
...
\]
Note that $D$ only provides an input and looks at the output, thus it works in the interactive setting.  Also, $D$ is of size $\max |\mathcal{M}|+ |\rep|$ where $\max |\mathcal{M}|$ is the longest description of an item in the metric space.  Thus, $D$ is also a distinguisher in the computational setting.
\end{proof}
...
Let $C$ be a set over space $\mathcal{M}$.  We say that $C$ is an $(t,\delta)$-\emph{Shannon code} if there exists an efficient procedure $\rec$ such that for all $t'\le t$ and for all $c\in C$, $\Pr[c'\leftarrow B_t(c) \wedge \rec(c') \neq c]\le \delta$. %To distinguish it from the average-error Shannon code defined below, we will sometimes call it a \emph{maximal-error} Shannon code.
\end{definition}
...
Recall that $Z$ consists of $2\gamma$ coefficients and there are $(|\mathbb{F}|-1)^{\gamma-1} |\mathbb{F}|^{\gamma-1}$ equally likely values for $Z$.
 As described above, the view of $\sketch, \rec$ is a uniform distribution $V$.  We show having seen $V$ there are many possible values for $Z$~(and the distributions $V|Z$ have disjoint support outside of the observed point).  The only information seen by $\sketch$ algorithm is in the point $V=v$.  The length of this point is $|\mathbb{F}|^\gamma$.  Conditioned on this information there are still many possible values for $Z$.  That is, 
 \[
...
We now show for any sketch algorithm, for possible values of $Z$ conditioned on $V=v$ there are few points included in the support of $V |\sketch(V)$.  The distributions $V | Z=z_1$ and $V| Z=z_2$ for possible $z_1, z_2$~(having seen $v$) overlap only at the point $v$.  This means for any $v^*\in V| \sketch(V)$ (other than the true $v$) there is at most one $z$ such that $v^*\in V | \sketch(V), Z=z$.  Thus, the optimum strategy is to include these values uniformly from different $Z$ values.
...
\end{proposition}
\begin{proof}
Consider some $w\in V$ over $\zo^n$.  Then $w$ was drawn from some intermediate distribution $W$ with coefficients $a_2, b_2, ..., a_\gamma , b_\gamma$.  The value $w_{1,...,\nu} =x $ is uniformly random and $w_{i\nu+1,...,(i+1)\nu}$ are uniformly random since $b_2,..., b_\gamma$ are uniformly random.
\end{proof}
...
\end{lemma}
\begin{proof}
We assume that $\rep$ is deterministic in our analysis.  Any randomness necessary for the \rep algorithm can be provided by $\gen$.  We do not consider these coins in our security analysis~(and thus, they can be considered private).  Denote by $(\Key, P) \leftarrow \gen(V)$.
...
Many of these sets must be small.  That is, there exists a set $R_{small}$  where $|R_{small} | \ge 2^{\kappa-1}$ such that for all $\key\in R_{small}$,  $|Q_{p^*, r}|\le \mathcal{M}/2^{\kappa-1} = 2^{n-\kappa +1}$.  If not, there exists a set of $\key$ of size $2^{\kappa-1}$ that all have at least $\mathcal{M}/2^{\kappa-1}$ points.\footnote{In the case where $\kappa=1$, $R_{small}$ still has at least one element.}  This is more points than exist in the metric space.  
...
We now show that for $\key \in R_{small}$ there are few points in $\inter(Q_{p^*, \key })$.  Consider some $\key^*\in R_{small}$.  We will use the term deficient ball:\bnote{use a term other than deficient}
\begin{definition}
