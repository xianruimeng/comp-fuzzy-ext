...
The security requirement for fuzzy extractors that is $\key$ uniform even to a (computationally unbounded) adversary who has observed $p$.   This requirement is  harder to satisfy as the allowed error tolerance $t$ increases, because it becomes easier for the adversary to guess $\key$ by guessing a $w'$ within distance $t$ of $w$ and running $\rep(w',p)$.
...
In the secure sketch case, $\sketch$ can be used to find fixed points of $\rec(\cdot, \sketch)$ which, by the correctness requirement of the sketch, must be separated by minimum distance $t$. This means there aren't too many of them, so few can lie on an average line, permitting the adversary to guess one easily.
...
The result for secure sketches is strong, it only requires the secure sketch to correct a constant, $t=4$, errors and allows $\rec$ to be wrong almost a quarter of the time.  The result for fuzzy extractors is more delicate.  It requires the fuzzy extractor to correct $t = \omega(n^{1/2}\log n)$ errors~(where $n$ is the length of the string) and have perfect correctness.  Extending this result to imperfect correctness seems challenging and is an interesting open problem.
This result also relies on the majority of points lying near boundaries of a set. Indeed, fuzzy extractors for arbitrary distributions are known in metric spaces where the boundary of a set does not consistent the majority of its area.\bnote{cite cite cite!!!}
...
We discuss distribution-aware algorithms in \secref{sec:distribution aware def}.  Let $\mathcal{M}$ be a metric space with distance function $\dis$.
...
A function $\ext: \mathcal{M}\times \{0,1\}^d \rightarrow \{0,1\}^\kappa$ a \emph{$(m, \epsilon)$-average-case extractor} if for all pairs
...
In an ideal world, we could provide a reproduce functionality as an oracle.  In this world, the key is output if a close point is provided, and no response is provided otherwise.  We call the performance of an adversary in this ideal world fuzzy min-entropy:
...
Fuzzy min-entropy is a necessary condition for security~(proof in \apref{sec:proof fuzz necessary}):%If an adversary can always obtain the output of a fuzzy extractor by running the reproduce on a fixed point security is impossible~(independent of the implementation of the fuzzy extractor).  We make this intuition formal:
...
Our goal is to provide a super-logarithmic length key when provided with super-logarithmic fuzzy min-entropy.  We now describe distribution-aware algorithms.

\subsection{Distribution-aware algorithms}
\label{sec:distribution aware def}
...
We call an algorithm with complete information on the distribution $W$ \emph{distribution-aware}.  We provide a definition for secure sketches~(extendible to fuzzy extractors):

\begin{definition}
Let $W$ denote a distribution.  A pair of algorithms, $\sketch, \rec$ is a $(\sketch_W, \rec_W)$ is called a $(\mathcal{M}, W, \tilde{m}, t)$-\emph{distribution-aware secure sketch} with error $\delta$ if the correctness and security properties hold for the distribution $W$.  That is, 
\begin{enumerate}
...
\end{enumerate}
\end{definition}
$(\sketch, \rec)$ know the description of $W$ and security only holds for $W$.  We assume distribution-aware algorithms are \emph{inefficient}.  Finding a natural model of specifying distributions that allows for efficient~(yet generic) distribution-aware constructions of sketches and extractors is an interesting problem.

\section{Sufficiency of $\Hfuzz$ for Distribution-Aware Algorithms}
\label{sec:distribution aware}
In this section, we show it is possible to build distribution-aware secure sketches~(and thus fuzzy extractors through \lemref{lem:fuzzy ext construction}) whenever $\Hfuzz(W)= \omega(\log n)$.
...
Fuzzy min-entropy is a lower bound of the adversaries success probability for any scheme.  Fuzzy min-entropy is the weight of the maximum probability ball~(\defref{def:fuzzy min-ent}).  For flat distributions, this is proportional to the largest number of points in any ball.  More precisely, 
\begin{align*}
...
\end{align*}
For notational convenience, we denote the ball with the largest number of points as $B_{t, max}$.  That is, 
\begin{align}
...
\subsection{Secure sketches for all distributions with $\Hfuzz$}
The hashing approach just described does not work for arbitrary sources.  The reason is that some balls may have significantly more points but low overall probability.  Let $W$ be a distribution, denote by $B_1$ a ball with $2^{\Hoo(W)}$ points but with probability $\Pr[W\in B_1] =2^{-\Hoo(W)}$ and let $B_2,..., B_{2^{-\Hoo(W)}}$ be balls with one point each with probability $\Pr[W\in B_i] = 2^{-\Hoo(W)}$.  Then the hashing algorithm needs to write down $\Hoo(W)$ bits to achieve correctness.  However, with probability $1-2^{-\Hoo(W)}$ the point $w_0$ lies outside of $B_1$ and the hash completely reveals the point.  
...
We address this by describing the probability of the point $w_0\in W$ seen in sketch.  Then the hash is applied only considering the maximum weight of the ball containing points of similar probability.  We can think of this construction as dividing the distribution $W$ into probability levels, and each component level is nearly flat.  As long as levels are large enough, they do not reveal too much information about the stored point $W$.  Naturally, there is a tradeoff between the number of levels and how useful the level information is to both \rep and the adversary.

\begin{construction}
\label{cons:leveling}
Let $\mathcal{M}$ be a metric space and let $n =\log |\mathcal{M}|$. Let $W$ be a distribution with $\Hfuzz(W)\ge m$.  Let $\ell\in\mathbb{Z}^+$ be a parameter.  Let $L_i = (2^{-(i+1)}, 2^{-i}]$ for $i=m,..., m+\ell$.  Let $F_i :\mathcal{K}_i\times \mathcal{M}\rightarrow R_i$ be a parameterized family of universal independent hash functions.  We describe $\sketch_W, \rec_W$ as follows: 
\begin{center}
\begin{tabular}{c|c}
\begin{minipage}{3in}
\textbf{$\sketch_W$}
...
In the previous section, we showed the sufficient of $\Hfuzz$ for distribution-aware algorithms.  It is infeasible to assume that $W$ is completely known.  Traditionally, algorithms only assume that $W$ comes from a family $\mathcal{W}$.  In this section, we show that uncertainty of the distribution $W$ comes at a real cost. 
...
\label{sec:def equiv}
Standard definition requires a secure sketch to work for all members $W$ of a family $\mathcal{W}$.  We refer to $\mathcal{W}$ as the \emph{family}, the distribution $W$ as a \emph{source} and the particular outcome taken $w\in W$ as the \emph{sample}.   

The security game of a fuzzy extractor can be thought of as a two stage process: 1) the challenger specifies $(\sketch, \rec)$, 2) the adversary specifies a source $W\in \mathcal{W}$.  The challenger wins if the adversary can guess the outcome of $W$ with low probability, and the adversary wins if they can guess $w\in W$ with noticeable probability.  The minimax theorem says we can reverse which of these actions is announced first~\cite{neumann1928theorie}.  That is, the following two player games are equivalent:
...
\textbf{Experiment} $\Exp^{\mathcal{W}}_1(\mathcal{A}, \mathcal{C}, k)$: \\
...
\begin{tabbing}
123\=123\=123\=123\=123\=\kill
\textbf{Experiment} $\Exp^{\mathcal{W}}_2(\mathcal{A}, \mathcal{C}, k)$: \\
...
Our negative results will consider an adversary that specifies a distribution $V$ over $\mathcal{W}$.  Importantly, the adversary sees the distribution $W$ and $\sketch(W)$ when they are trying to recover the particular outcome.  That is, $\sketch, \rec$ sees a distribution $V$ and the adversary seeing some side information $Z$ corresponding to the distribution $W$.
...
Let $n$ be a security parameter.  There exists a family of distributions $\mathcal{W}$ such that for each element $W\in \mathcal{W}$, $\Hfuzz(W)= \omega(\log n)$ and there is no $(\mathcal{M}, \mathcal{W}, \tilde{m}, t)$-secure sketch with error $\delta < 1/4$ for $\tilde{m} \ge 1$ for $t\ge 4$.  
\end{theorem}
...
This type of distribution is an affine line in space $\mathbb{F}^\gamma$.  Define $V$ as the uniform distribution over $\mathcal{W}$.  The adversary sees $V|Z$.  $Z$ is the description of the line $Z = a_2, b_2, ..., a_\gamma, b_\gamma$. 
...
\item \lemref{lem:side info determines sketch}:  for most lines $Z$, the intersection of the support of $V|Z$ and $V | \sketch(V)$ is small.  \\That is, $\tilde{H}_0(V | \sketch(V), Z) < 1$.
\item By \corref{cor:no fuzz for dist}, since no secure sketch can support the uniform distribution $V$ over $\mathcal{W}$, there is distribution oblivious secure sketch for the elements of $\mathcal{W}$.\end{itemize}
\end{proof}
...
Let $n$ be a security parameter.  There exists a family of distributions $\mathcal{W}$ over $\zo^n$ such that for each element $W\in \mathcal{W}$, $\Hfuzz(W)= \omega(\log n)$ and there is no $(\mathcal{M}, \mathcal{W}, \kappa, t, \epsilon)$-fuzzy extractor with error $\delta = 0$, for $\kappa \ge 1$ such that $\epsilon \le 1/4 - \ngl(n)$ for $t = \omega(n^{1/2}\log n)$. \end{theorem}

\begin{proof}[Proof Outline]
Our counterexample uses a slightly different family of distributions $\mathcal{W}$ than the counterexample for secure sketches.  In particular, we need to assume a larger error tolerance.  Let $\nu = \omega(\log n)$ and $\nu = o(n^{1/2}/\log n)$.  Let $t=4\nu n^{1/2}$ and note that $n/\nu >t$.  We work over the binary alphabet so we squish affine lines to the binary realm.  Let $x_1,..., x_\nu \in \zo^\nu$.  Let $\mathbb{F}$ denote the field of size $2^{\nu}$.  Let $a_2,..., a_{n/\nu}\in\mathbb{F}$ such that $a_i\neq 0$ and let $b_2,..., b_{n/\nu}\in\mathbb{F}$.  
...
We now present of the proof~(proofs in \apref{sec:fuzz ext proof}):
\begin{itemize}
\item \propref{prop:dist fuzzy ent fuzz}: for all $W\in \mathcal{W}$, $\Hfuzz(W) = \omega (\log n)$. Alternatively, $\forall z, \Hfuzz(V | Z=z) = \omega(\log n)$.
\item \propref{prop:dist uniform fuzz}: the distribution $V$ is uniform.
\item \lemref{lem:fuzz can't get key}: for many values of $Z$ and for half the keys $r\in\zo^\kappa$, there are no viable $v\in\mathcal{M}$.  Specifically:
\begin{itemize}
\item Show that for half the keys have a bounded number of pre images in the metric space.  Denote this set as $R_{small}$.  
\item Consider some $r\in R_{small}$.  %Our goal is to show there are few values of $v \in V|Z$ that could produce this key.  
Consider the set of $V_{r} = \{w | \rep(w, p) = r\}$.  The only values of $V_r$ that could be the real value of $w$ are distance $t$ from a boundary of $V_r$~(the functionality of $\rep$ guarantees that for the true $w$ all nearby points map to the same $r$).  We show that most of $V_r$ is near a boundary.  A result of Frankel and F{\"u}redi says that the boundary of a region is minimized by a ball containing the same number of points~\cite{frankl1981short} .  Hoeffding's inequality says that most of a ball lies near its boundary~\cite{hoeffding1963probability}.  Together these two results imply that $V_r$ is small.
...
\item For most values of possible $Z$, the intersection between the viable pre images of $V|Z$ and $V_r$ is empty.  Thus, for most values of $Z$, any key lying in $R_{small}$ can be labeled as ``random''.
\end{itemize}
\item By \corref{cor:no fuzz for dist}, since no fuzzy extractor can support the uniform distribution over $\mathcal{W}$, there is distribution oblivious fuzzy extractor for the elements of $\mathcal{W}$.
\end{itemize}
...
%
%%In the flat case, we used a hash whose length was proportional to the number of points in the largest ball.  The residual entropy in this case was within a super-logarithmic factor of $\Hfuzz(W)$.  However, in the setting where $W$ is a convex combination of flat sources this does not hold.
%
...
\section{Proofs}
\subsection{Proof of \propref{prop:fuzz necessary}}
...
\item Else output $0$.
\end{itemize}
Clearly, $\Pr[D(R, P) = 1]\geq 1/\poly(n) - \delta$, while $\Pr[D(U_\kappa, P)=1 ]= 1/2^{-\kappa}$.  Thus, when $\kappa = \omega(\log n)$:
\[
\delta^D((R, P), (U_\kappa, P))\geq \frac{1}{\poly(n)} -\delta -  \frac{1}{2^{-\kappa}} = 1/\poly(n).
\]
Note that $D$ only provides an input and looks at the output, thus this distinguisher extends to the interactive setting).  Also note that $D$ is of size $\max |\mathcal{M}|+ |\rep|$ where $\max |\mathcal{M}|$ is the longest description of an item in the metric space.  Thus, $D$ is also a distinguisher in the computational setting.
\end{proof}
...
\textbf{Correctness:}  We begin by arguing correctness.  We set $n = \ell$ to be the number of levels.\footnote{This proof can be carried out for an arbitrary $\ell$ but it leads to a more complicated theorem statement.}  Fix some $w, w'$.  If $\Pr[W=w]\le 2^{-(m+\ell)} = 2^{-(m+n)}$, then $w$ is simply transmitted to $\rec$ and correctness is clear.  Now consider the case when $\Pr[W=w]> 2^{-(m+n)}$, let $L_i^*$ be the level of $\Pr[W=w]$.
...
An adversary that receives $i$ as input the best strategy is to guess a point that has the most nearby weight in that level.  That is, $w^*\leftarrow A(i)$ such that $\max_{w^* \in \mathcal{M}}\Pr_{w\in W | 2^{-(i+1)}\le \Pr[W=w]\le 2^{-i}}[\dis(w, w^*)]$. The success of this adversary is at least $2^{-(i+1)}|B_{t,i, max}|$ as there at $B_{t, i, max}$ nearby points in that layer each with probability at least $2^{-(i+1)}$.  Note there are $n$ outcomes for $i$ so the overall success of such an adversary is at most $n$ better than an adversary without such input~(by~\cite[Lemma 2.2]{DBLP:journals/siamcomp/DodisORS08}).  That is, 
\begin{align}
\expe_{i | m\le i \le m+n}2^{-(i+1)}|B_{t, i, max}|&\le \expe_{i | m\le i \le n+m}\left( \max_{w^*\in W}\sum_{w\in W| 2^{-(i+1)}\le \Pr[W=w]\le 2^{-i} \wedge \dis(w, w^*) \le t}\Pr [W=w]\right) \label{eq:link fuzz 1}\\
&\le n \left(\max_{w^*\in W} \sum_{w\in W | \dis(w, w^*)\le t} \Pr[W=w]\right)\label{eq:link fuzz 2}\\
&= n 2^{-\Hfuzz(W)}\label{eq:link fuzz 3}
\end{align}
\textbf{Security:}
We now argue security.  First note that the total weight of points whose probability is less than $2^{-(n+m)}$ is at most $2^{-m}$~(there are at most $2^n$ points in the distribution).  That is, $\sum_{w | \Pr[W=w]\le 2^{-(n+m)}}\Pr[W = w] \le 2^{-m}$.  Let $1_{\text{low}}$ be the indicator random variable for $\Pr[W=w]\le 2^{-n}$.  Then 
\begin{align*}
...
\[
2^{-\Hav(W | \sketch(W) \wedge 1_{\text{low}} =0} = \max_{w\in W | \Pr[W=w]>2^{-(n+m)}}\Pr[W=w | \sketch(W)].
\]
...
\subsection{Proof of \thref{thm:imposs sketch}}
...
We now prove item in the outline of \thref{thm:imposs sketch} in turn.

\begin{proposition} 
\label{prop:each element good} For each $W\in\mathcal{W}$, $\Hfuzz(W) = \omega(\log n)$.
\end{proposition}
\begin{proof}
Consider some $W\in\mathcal{W}$.  The value $w_1$ is uniform in a field of size $\omega(\poly(n))$, so $\Hoo(W) =\omega(\log n)$.  Recall that $t< \gamma$.  We now show that for any $w, w'\in W$, $\dis(w, w') = \gamma$ and thus $\Hfuzz(W) = \Hoo(W)$.  Fix some $w, w'\in W$.  Clearly, $w_1 \neq w_1'$, for any $i$, $w_i = a_i w_1 + b_i$ and $w_i' = a_i w_1' + b_i$.  Since $a_i\neq 0$, $a_iw_1 \neq a_iw_1'$ and thus $a_iw_1+b_i \neq a_iw_1'+b_i$.  That is, $\dis (w, w')  =\gamma$.
\end{proof}
...
\subsection{Proof of \thref{thm:imposs fuzz ext}}
...
We now show that for $r\in R_{small}$ there are few points in $\inter(Q_{p^*, r})$.  Consider some $r^*\in R_{small}$.  We will use the term deficient ball:\bnote{use a term other than deficient}
\begin{definition}
