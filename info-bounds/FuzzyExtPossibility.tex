...
\end{theorem}
\bnote{right now this is stated without epsilon.  I found the expression to be really gross with $\epsilon$ included.  Thoughts?  I know $\epsilon$ needs to be there because the construction uses it.}
\begin{proof}
...
\end{align*}
We first consider the optimum strategy for an adversary that receives just the information about the probability of $w$.  An adversary that receives $i$ as input the best strategy is to guess a point that has the most nearby weight in that layer.  That is, $w^*\leftarrow A(i)$ such that $\max_{w^* \in \mathcal{M}}\Pr_{w\in W | 2^{-(i+1)}\le \Pr[W=w]\le 2^{-i}}[\dis(w, w^*)]$.The success of this adversary is at least $2^{-(i+1)}|B_{t,i, max}|$ as there at $B_{t, i, max}$ nearby points in that layer each with probability at least $2^{-(i+1)}$.  Note there are $n-k+\log 1/\epsilon$ outcomes for $i$ so the overall success of such an adversary is at most $n-k+\log 1/\epsilon$ better than an adversary without such input~(by~\cite[Lemma 2.2]{DBLP:journals/siamcomp/DodisORS08}).  That is, 
\begin{align}
\expe_{i | k\le i \le n+\log 1/\epsilon}2^{-(i+1)}|B_{t, i, max}|&\le \expe_{i | k\le i \le n+\log 1/\epsilon}\left( \max_{w^*\in W}\sum_{w\in W| 2^{-(i+1)}\le \Pr[W=w]\le 2^{-i} \wedge \dis(w, w^*) \le t}\Pr [W=w]\right) \label{eq:link fuzz 1}\\
&\le \left(n-k+\log 1/\epsilon\right)\left(\max_{w^*\in W} \sum_{w\in W | \dis(w, w^*)\le t} \Pr[W=w]\right)\label{eq:link fuzz 2}\\
&= \left(n-k+\log 1/\epsilon\right)\Hfuzz(W)\label{eq:link fuzz 3}
\end{align}
...
&\le \expe_{i | k\le i \le n+\log 1/\epsilon} \left(\max_{w\in W | \Pr[W=w]\in L_i} \Pr[W=w]*2^{H_0(\sketch(W) | i)}\right)\\
&\le \expe_{i | k\le i \le n+\log 1/\epsilon} \left(2^{-i}*|B_{t, i, max}|/\delta\right)\\
&\le\frac{ \expe_{i | k\le i \le n+\log 1/\epsilon} \left(2^{-(i+1)}*|B_{t, i, max}|\right)}{2\delta}\\
...
&= \frac{(n-k+\log 1/\epsilon) 2^{-\Hfuzz(W)}}{2\delta}.
\end{align*}
Where the last line follows by Equations~\eqref{eq:link fuzz 1}-\eqref{eq:link fuzz 3}.
Combining both cases we have:
\begin{align*}
\Hav(W | \sketch(W)) = -\log \left(\epsilon+\frac{(1-\epsilon)(n-k+\log1/\epsilon)2^{-\Hfuzz(W)}}{2\delta}\right).
\end{align*}
Setting $\epsilon = 2^{-k}$ yields
\begin{align*}
\Hav(W | \sketch(W)) &= -\log \left(k+\frac{(1-2^{-k})(n)2^{-\Hfuzz(W)}}{2\delta}\right)\\
&\ge -\log \min\{k, \frac{(1-2^{-k}) n2^{-\Hfuzz(W)}}{2\delta}\})-1\\
&\ge \Hfuzz(W) - \log n + \log \delta - \log (1-2^{-k}) - 2\\
&\ge \Hfuzz(W) - \log n + \log \delta - 3\\
\end{align*}
Where the third line follows from the third because $\Hfuzz(W)\le \Hoo(W) = k$.\bnote{what condition do we need to get rid of $\log (1-2^{-k})$?}
\end{proof}
