...
\begin{abstract}
Fuzzy extractors (Dodis et al., Eurocrypt 2004) convert repeated noisy readings of a high-entropy secret into the same uniformly distributed key. To help eliminate noise, they produce a non secret helper string. This helper value allows the key to be regenerated from a nearby point.  

The nonsecret helper string allows an adversary to learn the generated key if they can guess a point nearby to the original reading.  A necessary condition for key derivation is hardness of guessing nearby points.  We call the maximum probability mass of nearby points \emph{fuzzy min-entropy}.  Many practical sources have no known construction of fuzzy extractors but have fuzzy min-entropy.  There is evidence fuzzy min-entropy is sufficient to construct computational fuzzy extractors.\bnote{how to work in the interactive case?}
In this work, we ask: \emph{is fuzzy min-entropy sufficient to build an information-theoretic fuzzy extractor?}  

We first consider a setting where the fuzzy extractor knows the precise distribution of the physical source.  In this setting, we construct a fuzzy extractor for any distribution with fuzzy min-entropy.  

However, high-entropy distributions can never be fully observed and must be modeled. It is imprudent to assume that the designer's model of a distribution is completely accurate---the adversary may have a better model.  Thus, fuzzy extractors are normally built for families of distributions.  This uncertainty is an impediment to security.  We show a family of distributions such that no fuzzy extractor can extract a single key bit out for an average member of the family.
...
\begin{itemize}
\item Half the keys have at most $2^{n- (\kappa-1)}$ pre images in the metric space~(this is at most half the metric space).  Denote this set as $R_{small}$.  
