...
\newcommand{\bnd}{\ensuremath{\class{Bnd}}\xspace}
...
Ideally, key derivation should be possible whenever a negligible portion of $W$ lies within a ball~(arbitrarily centered) of radius $t$.  We call the maximum weight that lies within any ball of radius $t$ the fuzzy min-entropy of a distribution.  In the interactive setting, fuzzy min-entropy is both a necessary and sufficient condition for security.  
...
\textbf{Our Contribution} In the \emph{distribution-aware} setting, for every source with super-logarithmic fuzzy min-entropy it is possible to build a secure sketch with super-logarithmic output entropy~(\thref{cons:leveling}).  Using standard techniques we can convert this secure sketch to a fuzzy extractor with super-logarithmic key output length.  (A key with super-logarithmic entropy can be expanded to arbitrary length using computational techniques.)  Thus, in the distribution-aware setting, fuzzy min-entropy is both a necessary and sufficient condition for security.  

\textbf{Note:} Our constructions are information-theoretic and do not run in polynomial time.  They should be regarded as feasibility results.  It is unclear how to define distribution-aware efficient algorithms as most distributions have exponential description size.  The power of the distribution-aware setting is that the algorithms can access this description.  It seems unlikely that algorithms with access to a polynomial size part of the distribution description can replicate these results.

We show that distributional uncertainty is a real drawback to providing security.  In the standard \emph{distribution-aware} setting, we show a family of distributions~(each with super-logarithmic fuzzy min-entropy) such that any secure sketch for the family is insecure~(\thref{thm:imposs sketch}).  This negative result extends to computational secure sketches~\bnote{cite a corollary}.


%The situation is significantly more complicated for distribution-oblivious algorithms.  Indeed, we show a natural families to distributions where every distribution has fuzzy min-entropy such that no secure sketch can provide meaningful security.  However, it is possible to build a meaningful fuzzy extractor for this same class of distributions.


\textbf{Open Problem:} The main open problem is whether fuzzy min-entropy is a sufficient condition for constructing a fuzzy extractor in the distribution-oblivious setting.  We note that it is known how to build a fuzzy extractor for the family of distributions used in our negative result~\cite[Theorem 5.7]{canetti2014key}.  Furthermore, are computational assumptions necessary?
\bnote{talk about obfuscation.}
...
\hline
Fuzzy Extractor & Yes~(\corref{cor:extension to fuzz ext}) & Unknown
...
\subsection{Fuzzy Extractors}\label{sec:fuzz extractor}

We recall definitions and lemmas from the work of Dodis et. al.~\cite[Sections 2.5--4.1]{DBLP:journals/siamcomp/DodisORS08} with two changes.  First we allow for error, as discussed in \cite[Sections 8]{DBLP:journals/siamcomp/DodisORS08} and we consider an arbitrary family $\mathcal{W}$ of distributions instead of families containing all distributions of a given min-entropy.  See~\cite{canetti2014key} for discussion.  Let $\mathcal{M}$ be a metric space with distance function $\dis$.
...
\item The recovery procedure \rec takes an element $w'\in\mathcal{M}$ and a bit string $s\in\{0,1\}^*$.  The \emph{correctness} property of secure sketches guarantees that 
\[
...
\textbf{Notes:} In the above definition of secure sketches (resp., fuzzy extractors), the errors are chosen before $s$ (resp., $P$) is known: if the error pattern between $w$ and $w'$ depends on the output of $\sketch$ (resp., $\gen$), then there is no guarantee about the probability of correctness.  
...
\]
\end{proof}
\lemref{lem:fuzz necessary} generalizes to interactive protocols, $D$ only provides an input to the protocol and looks at the output.  This means that fuzzy min-entropy is also a necessary condition for interactive solution.  As stated above, fuzzy min-entropy is also a sufficient condition for security in the interactive setting.  However, in the fuzzy extractor~(non-interactive) setting there are many distributions for which no construction is known to be secure and for which no impossibility result is known.  We attempt to bridge this gap in this work.  Our goal is to provide a super-logarithmic length key when provided with super-logarithmic fuzzy min-entropy.  

We separate between two types of fuzzy extractors and secure sketches.  The standard definition requires a fuzzy extractor/secure sketch to work for a family of distributions.  This is because we rarely have complete distributions of a physical probability distribution. We call this type of algorithm distribution-oblivious.  We define a new type of algorithm called distribution-aware that is allowed to know the precise distribution to be supported.  We provide a definition for secure sketches~(extendible to fuzzy extractors):
...
\forall w\in W, \forall w' \in\mathcal{M}| \dis(w',w')\le t, \Pr[\rec(w',\sketch(w))=w]\ge 1-\delta\]
 where the probability is taken over the coins of $\sketch$ and $\rec$.  Note correctness is guaranteed only for the support of $W$.
\item \emph{Security} The distribution $W$ is unknown conditioned on $s$: $\Hav(W|\sketch(W))\geq \tilde{m}$.
\end{enumerate}
\end{definition}
Note (\sketch, \rec) know the description of $W$ and security only holds for $W$.  When we consider distribution-aware algorithms, we generally assume they are \emph{inefficient}, this seems inherent to use the (possibly)~exponential size distributions.  One possible approach is to allow $(\sketch, \rec)$ an oracle to the probability mass function of $W$.  Our constructions do not work in this setting.\bnote{why am I talking about this?}
...
\end{tabular}
\caption{Summary of results for distribution aware setting.  Residual entropy of distribution-aware secure sketches with error $\delta$.}
...
Let $F :\mathcal{K}\times \mathcal{M}\rightarrow R$ be a universal hash function.  We describe $\sketch, \rec$ as follows:
\begin{itemize}
\item $\sketch(w)$:
...
\begin{construction}
\label{cons:leveling}
Let $\mathcal{M}$ be a metric space and let $n =\log |\mathcal{M}|$. Let $\ell\in\mathbb{Z}^+$ be a parameter.  Let $L_i = (2^{-(i+1)}, 2^{-i}]$ for $i=m,..., n+\ell$.  Let $F_i :\mathcal{K}_i\times \mathcal{M}\rightarrow R_i$ be a parameterized family of universal independent hash functions.  We describe $\sketch, \rec$ as follows: 
\begin{itemize}
\item $\sketch(w)$:
...
\item For any secure sketch on $V$, $\tilde{H}_0(V | \sketch(V)) \le  (\gamma-t+1)\log |\mathbb{F}|+1$. 
...
&= \log \left(\frac{2^{h_s}|\mathbb{F}|}{(|\mathbb{F}|-1)^{\gamma-1}} \right)
\end{align*}
...
&\le (\gamma-t+2)\log |\mathbb{F}| - (\gamma-2) \log |\mathbb{F}|+1\\
&\le \Theta(1)
\end{align*}
...
\subsubsection{Computational secure sketches}
...
The definitional of Fuller et al. is uses HILL entropy.

HILL entropy is a commonly used computational notion of entropy \cite{DBLP:journals/siamcomp/HastadILL99}.  Fuller et al.~use a weak version due to Gentry and Wichs~\cite{gentry2011separating}.
...
A \emph{HILL secure sketch} is obtained by replacing the security condition in \defref{def:secure sketch} with HILL entropy~(adding parameters $s_{sec}, \epsilon_{sec}$ parameters to the definition, representing the distinguishing circuit).  By applying the contrapositive of~\cite[Corollary 3.8]{fuller2013computational}, no sketch can retain HILL entropy for the same family of distributions:
...
\textbf{Note:} The above result does not rule out a computational secure sketch based on unpredictability~(where $W$ is hard to guess given the string $S$).  Unpredictable secure sketches is implied by virtual-grey box obfuscation of point functions~\cite{BitanskyCKP14}.  

\section{Distribution-Oblivious Fuzzy Extractors}
In the previous section, we showed a family of distributions that does not admit a secure sketch.  In this section, we extend this result to fuzzy extractors.  The main difficulty is showing that fuzzy extractors must cause a significantly entropy drop in $V$.  This is not true for all metric spaces.  We begin with the case of perfect correctness in the Hamming metric.  

\begin{theorem}
\label{thm:imposs fuzz ext}
Let $n$ be a security parameter.  There exists a family of distributions $\mathcal{V}$ such that for each element $W\in \mathcal{V}$, $\Hfuzz(W)= \omega(\log n)$ and there is no $(\mathcal{M}, \mathcal{V}, \kappa, t, \epsilon)$-secure sketch with error $\delta = 0$ for $\tilde{m} =\omega(1)$ for $t\ge 4$. \bnote{what is $\epsilon$? what are the other parameters?}
\end{theorem}
 \bnote{write a proof!}
\begin{proof}
The only difference between \thref{thm:imposs fuzz ext} and \thref{thm:imposs sketch} is the functionality required of the algorithms $(\gen, \rep)$.  Thus, we must prove that a fuzzy extractor must still cause a significant drop in the entropy of $V$:

\begin{lemma}
Fix some $(\gen, \rep)$ algorithm with $\kappa = \omega(\log n)$ and $\epsilon=$ \bnote{what is this?}.  Then $\tilde{H}_0(V | \gen(V)) \le (\gamma-t+1)\log |\mathbb{F}|+1$.
\end{lemma}
\begin{proof}
...
Consider some $p^*\in A_{ss}$.  We now show that $H_0(V | p^*) \le XXXX$\bnote{fill in}.  Since $(R | P=p^*, p^*)\approx_{2\epsilon} (U, p^*)$ this means that $R|P=p$ is at least $(1-2\epsilon)2^\kappa$.  Note that, $R|P=p$ is the set of possible keys.

The set of possible keys induces a partition on the metric space.  That is, for every $w\in\mathcal{M}$, there exists a unique $Q_{p^*,r}$ such that $\rep(w, p^*) =r$.  Define two sets, called the boundary and interior respectively: \begin{align*}
\bnd(Q_{p^*, r}) = \{w | \rep(w, p^*) = r \wedge \exists w', \dis(w, w')\le t \wedge \rep(w', p^*) \neq r\},\\
\inter(Q_{p^*, r}) = \{w | \rep(w, p^*) = r \wedge \forall w', \dis(w, w') \le t \wedge \rep(w', p^*) =r\}.
\end{align*}

Note that $\bnd(Q_{p^*, r}) \cap \inter(Q_{p,r}) =\emptyset$, for any $r_1, r_2$, $\inter(Q_{p^*, r_1}) \cap \inter(Q_{p^*, r_2}) =\emptyset$ and the union of all boundaries and interiors is the entire space.  Furthermore, note that for any $r\in \zo^\kappa$,  $\Pr[V\in Q_{p^*, r}] = \ngl(n).$\bnote{need to fill in this distinguishing argument, note this doesn't need to be the case for all $p^*$ just on average. need another markov inequality.}\bnote{what can we say about the relative sizes of these spaces?}

We now consider the distribution of $V | P=p^*$.  By the functionality of $(\gen, \rep)$, for every $v\in V| P=p^*$, $v\in Int(Q_{p, r})$.  Furthermore, if $v$ is not in some interior then $v\not \in V | P=p^*$.  Thus, it suffices to show that $\frac{\sum_{r} |\inter(Q_{p^*, r})|}{\mathcal{M}} = XXX$\bnote{fill this in.}

To show this we any such partitioning of the metric space must have the majority of points lying on the boundary.  This is true for the Hamming metric.
\begin{lemma}[Isoparametric Inequality]
\bnote{XXXX}
\end{lemma}
\end{proof}
\end{proof}
