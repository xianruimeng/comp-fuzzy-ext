...
In the remainder of this work, we consider increasingly complex types of distributions and consider both the minimum number of bits a secure sketch/fuzzy extractor must write down and the minimum entropy loss.  We begin by considering secure sketches.  We consider the setting where the algorithms have a complete description of the distribution $W$.  In the next setting we consider the setting where the construction must work for a family of distributions~(we show there is a large gap between these two settings).

\subsection{Well-Spread Distributions}
We call a distribution well-spread if the distance between any two original readings $w_0, w_1$ is large.  Well-spread distributions should be the easiest type of distribution to handle as given a particular $w'$ there is a unique $w$ that could be the original reading.  Error-correction should be easy when all points of $W$ are far apart.  Intuitively, the fuzzy extractor/secure sketch does not need to disambiguate points and thus error-correction should be easy.  Indeed for any error-correcting code, a fuzzy extractor can be designed~(where the design of the fuzzy extractor depends on the supported distribution).  However, this intuition becomes more difficult when a fuzzy extractor should work for all well-spread distributions.  We begin by defining a well-spread distribution.
...
In the previous section we showed that info-theoretically it is easy to produce a secure sketch and fuzzy extractor for a well-spread distribution when the algorithms are allowed to encode the input distribution $W$.  
...
In this section, we consider information-theoretic bounds on distributions that contain multiple points in each ball.  
...
Dealing with non-flat distributions requires a new strategy.  Ideally, this strategy should be aware of the density of the current ball and aware that when an original reading comes from a sparse ball it is not necessary to write down many bits.  We first consider a mechanism that tries to reduce the arbitrary case to the flat case by selecting a candidate flat distribution.  Unfortunately, this mechanism does not work as a distribution with high fuzzy min-entropy can be formed as a convex combination of distributions with almost no fuzzy min-entropy~(\secref{sec:convex comb}). 

We then show a solution that comes within a super-logarithmic factor of optimal of optimal by partitioning the probability space~(\secref{sec:layered hash}).

\subsubsection{Leveraging convex combinations?}
...
\subsubsection{Layered Hashing}
\label{sec:layered hash}
In the previous section, we saw that sketching non-flat distributions is difficult~(while retaining entropy close to the fuzzy min-entropy).  The problem is distinguishing between the setting where a ball has a few high probability points and a large number of low probability points.  We will deal with this problem by describing the probability of the point $w_0\in W$ seen in sketch.  Then the hash is applied only considering the maximum weight of the ball containing points at this level.  Roughly, at each layer we can think of the distribution as being flat, and the only additional information is the layer of the original reading.  Naturally, revealing the probability of the original point leaks some information and there is a balance between how much about the probability is revealed to the adversary and how useful it is.
...
\begin{proof}
\textbf{Correctness:}  We begin by arguing correctness.  Let $\ell$ be a parameter describing the number of levels.\footnote{later we will set $\ell = m$ to simplify expressions but this is not necessary.}  Fix some $w, w'$.  If $\Pr[W=w]\le 2^{-(n+\ell)}$, then $w$ is simply transmitted to $\rec$ and correctness is trivial.  Now consider the case when $\Pr[W=w]> 2^{-(n+\ell)}$, let $L_i^*$ be the level of $\Pr[W=w]$.
...
\textbf{Aside:} Before arguing security of the actual construction, we consider the optimum performance of an adversary that receives the level information.  %We first consider the optimum strategy for an adversary that receives just the information about the probability of $w$.  
...
\end{align*}
Where the third line follows from the second because $\Hfuzz(W)\le \Hoo(W) = m$. The last line follows from the fourth because if $m\ge 1$ then $\log (1-2^{-m})\le 1$ and if $m< 1$ the entire bound is vacuous as $\Hfuzz(W)< 1$ and thus the whole quantity is negative~(and entropy is non-negative).
\end{proof}

\section{Distribution-Agnostic Sketches}
...
\subsection{Min-Max to switch quantifiers}

\subsection{Impossibility of info-theoretic sketches for distribution}
\begin{lemma}
