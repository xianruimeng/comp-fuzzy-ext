...
Denote the ball of radius $t$ with the largest number of points in the support of $W$ as $B_{t, max}$.   For flat distributions, the weight of this maximum-probability ball (which determines $\Hfuzz(W)$ by \defref{def:fuzzy min-ent}) is proportional to the number of points in it.  More precisely, 
\begin{align}
...
&=\Hoo(W) -\log\left( \max_{w' \in \mathcal{M}}| \{w | w\in \supp(W) \wedge \dis(w, w')\le t\} |\right)\nonumber \\
&= \Hoo(W) -\log |B_{t, max}\cap \supp(W)|.\label{eq:fuzz for flat}
\end{align}
...
\end{lemma}
\begin{corollary}
Let $n = \log |\mathcal{M}|$.  If $|R| \ge |B_{t, max}|* n^{\omega(1)}$ then \consref{cons:universal hash} is correct with overwhelming probability.  That is, setting $\log |R| = \log |B_{t, max}\cap\supp(W)| + \omega(\log n)$ suffices.
\end{corollary}
...
$
\Hav(W |\sketch(W)) = \Hoo(W) - \log |B_{t, max}\cap\supp(W)| - \omega(\log n).
$
...
\item $W^* = \{w \in \supp(W) | \dis(w, w')\le t\}$.
...
\[
B_{t, i, max} = \max_{w' \in \mathcal{M}} \{w | w\in \supp(W) \wedge \dis(w, w')\le t \wedge \Pr[W=w]\in L_i\}.
\]
...
\section{Impossibility of Secure Sketches for all Families with $\Hfuzz$}
...
Furthermore, this is true on average.  Let $V$ be process of uniformly sampling $W\leftarrow \mathcal{W}$ and sampling $w\leftarrow W$, and let $Z$ specify which $W$ is sampled.  Then
\[
\Hav(V|\sketch(V), Z)< 2.
\]
\end{theorem}
...
\section{Impossibility of Fuzzy Extractors for all Families with $\Hfuzz$}
...
Let $n$ be a security parameter.  There exists a family of distributions $\mathcal{W}$ over $\zo^n$ such that for each element $W\in \mathcal{W}$, $\Hfuzz(W)= \omega(\log n)$.  Let $\kappa \ge 2$, $t = \omega(n^{1/2}\log n)$.  Any $(\mathcal{M}, \mathcal{W}, \kappa, t, \epsilon)$-fuzzy extractor with error $\delta = 0$ has $\epsilon > 1/8 - \ngl(n)$.

Furthermore, this is true on average.  Let $V$ be process of uniformly sampling $W\leftarrow \mathcal{W}$ and sampling $w\leftarrow W$ and let $Z$ be the description of which $W$ is sampled.  Let $(\Key, P)\leftarrow \gen(V)$.  Then, 
\[
...
Let $\mathcal{W}$ be the set of all such $W$.  
 ...
 Define $V$ as the process of uniformly choosing $W\leftarrow \mathcal{W}$ and then sampling from $w\leftarrow W$.  The adversary sees $\sketch(V)$ and which distribution was sampled, call this information $Z$. $Z$ is the description of the line $Z = a_2,..., a_{n/\nu}, b_2, ..., b_{n/\nu}$.
...
\begin{align*}
\Pr[\exists w^* \in W^* |w^* \neq w \wedge F(K, w^*) = F(K, w)] &\le \sum_{w^*\in W^* | w^*\neq w} \Pr[F(K, w^*) = F(K, w)] \\
 &= \sum_{w^*\in W^* | w^*\neq w} \frac{1}{|R_i|} \le \frac{|B_{t, i, max}\cap \supp(W)|-1}{|R_i|} = \delta
\end{align*}
...
The best strategy for an adversary that receives $i$ as is to guess a point that has the most nearby weight in that level.  The adversary chooses \[w^*= \argmax_{w' \in \mathcal{M}}\Pr_{w\in W | 2^{-(i+1)}< \Pr[W=w]\le 2^{-i}\wedge \dis(w, w^*)}[W=w].\] The success of this adversary is at least $2^{-(i+1)}|B_{t,i, max}\cap\supp(W)|$ as there at $|B_{t, i, max}\cap\supp(W)|$ nearby points in that layer each with probability at least $2^{-(i+1)}$.  There are $n$ outcomes for $i$. The overall success of such an adversary is at most $n$ better than an adversary without such input~(by~\cite[Lemma 2.2]{DBLP:journals/siamcomp/DodisORS08}).  That is, 
\begin{align}
\expe_{i | m\le i \le m+n}2^{-(i+1)}|B_{t, i, max}\cap\supp(W)|&\le \expe_{i | m\le i \le n+m}\left( \max_{w^*\in W}\sum_{w\in W| 2^{-(i+1)}< \Pr[W=w]\le 2^{-i} \wedge \dis(w, w^*) \le t}\Pr [W=w]\right) \nonumber\\
...
&\le \expe_{i | m\le i \le m+n} \left(2^{-i}*|B_{t, i, max}\cap\supp(W)|/\delta\right)\\
&\le\frac{ \expe_{i | m\le i \le m+n} \left(2^{-(i+1)}\cdot |B_{t, i, max}\cap \supp(W)|\right)}{2\delta}\\
