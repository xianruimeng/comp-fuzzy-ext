...
The security requirement for fuzzy extractors is that they should produce a $key$ should looks uniform to an adversary who has observed $p$.   Naturally, satisfying this requirement gets harder the easier it is for the adversary to guess some $w'$ within distance $t$ of $w$, because an adversary who is successful at doing so will just obtain $\key$ by running $\rep$.

\paragraph{A New Entropy Notion}
We introduce a new entropy notion that measures precisely how hard it is for the adversary to guess a value within distance $t$ of the original reading $w$. Suppose $w$ is sampled from a distribution $W$. To have the maximum chance of success with such a guess $w'$, the adversary would want to maximize the total probability mass of $W$ within a ball $B_t(w')$ of radius $t$ around $w'$. We thus define \emph{fuzzy minentropy} $\Hfuzz(W) \eqdef -\log \max_w' \Pr[W\in B_t(w')]$. Observe that, trivially,
$\Hfuzz(W) \ge \Hoo(w)-\log |B_t(\cdot)|$; this bound is useless (negative) in important practically relevant settings (e.g., iris scans~\cite[Section 5]{blanton2009biometric} and some physical sources~\cite{koeberl2014entropy}) in which entropy is at a premium and the tolerated distance must be high.


Since an adversary who can guess $w'$ can trivially guess $\key$,  $\Hav(\key|p) \le \Hfuzz(W)$, and so at least superlogarithmic fuzzy minentropy is necessary for nontrivial key extraction. We now ask whether it also sufficient.

\paragraph{A New Characterization}
We show that for every source $W$ with superlogarithmic $\Hfuzz(W)$, it is possible to construct a fuzzy extractor with a superlogarithmic length $\key$ that is at most negligibly far from uniform (\corref{cor:extension to fuzz ext}). We thus characterize when fuzzy extractors are possible in terms of our new entropy notion $\Hfuzz$. While our constructions are not polynomial-time (because they are dependent on having complete information about $W$), we observe in Section \lnote{ref} that polynomial-time constructions that provide only computational (rather information-theoretic) security are also possible. 

This characterization justifies paying attention to $\Hfuzz$ rather than relying on cruder measures such as $\Hoo(w)-\log |B_t(\cdot)|$.
It is important to note that this characterization allows a fuzzy extractor to be designed with $W$ in mind. We call such fuzzy extractors \emph{distribution aware}.


\paragraph{A New Impossibility Result}

However, fuzzy extractor designs are typically not tailored to a particular source, but rather for a family sources (for example, all sources of minentropy at least $k$ with at most $t$ errors). Making minimal assumptions about the distribution $W$ is desirable for two reasons: 1) for high entropy distributions, only a small part of the distribution has been observed 2) the adversary may have some side information about the distribution $W$ and thus the adversaries view of the distribution is different than that of the algorithm designer.  

We show (\thref{thm:imposs fuzz ext}) that this extra adversarial knowledge can be devastating. 
Specifically, we show a family of distributions $\mathcal{W}$ such that no fuzzy extractor can be secure for most elements of $\mathcal{W}$.  We emphasize that each distribution $W\in \mathcal{W}$ has fuzzy min-entropy and its points are far apart~(intuitively, this should make the task easier). \lnote{can we say this a bit better and more precisely, by relating $\Hfuzz$ and $Hoo$?}
...
A secure sketch performs non-interactive information-reconciliation.  We show comparable results for secure sketches.  secure sketches are possible if the distribution $W$ is precisely known~(\thref{cons:leveling}), and yet there is a family of distributions for which no secure sketch in possible.  \lnote{something about how for sec sketches results are stronger?}

\paragraph{Related Settings}
We have evidence that fuzzy min-entropy is a sufficient condition in several related settings.  We first consider the interactive setting.  
...
In this world, secure two-party computation can produce a key whenever the distribution $W$ has fuzzy min-entropy.  The minimal assumption necessary for two party computation is \bnote{what?} \lnote{OT, right?; we can ask Ran about their paper with Tal R. or read the paper}
