...
\begin{enumerate}
\item \emph{Correctness} For all $w\in W$ and for all $w'\in\mathcal{M}$,  if $\dis(w,w')\leq t$, then $\Pr[\rec(w',\sketch(w))=w]\geq 1-\delta$ where the probability is taken over the coins of $\sketch$ and $\rec$.  If $\dis(w,w')>t$, then no guarantee is provided about the output of \rec.
\item \emph{Security} $W$ is unknown conditioned on $s$: $\Hav(W|\sketch(W))\geq \tilde{m}$.
\end{enumerate}
\end{definition}
...
We consider increasingly complex types of distributions and end with an arbitrary distribution over a metric space.  We construct secure sketches but all constructions can be transformed to fuzzy extractors using \lemref{lem:fuzzy ext construction}.  In the next section \bnote{what section?} we consider distribution-oblivious algorithms.  
