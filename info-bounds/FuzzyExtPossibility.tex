...
\subsubsection{Extending to Computational secure sketches}
...
Note that $\bnd(Q_{p^*, r}) \cap \inter(Q_{p,r}) =\emptyset$.  We now show $Q$ are small.

\begin{claim} For any $r\in \zo^\kappa$,  $ | Q_{p^*, r}| \le (1+\epsilon)2^\kappa.$
\end{claim}
\begin{proof} 
Recall that for any $r$, $\Pr[U = r] = 1/2^\kappa$.  Since the total statistical distance is $2\epsilon$, 
\[
\Pr[R=r] \le 2( 2\epsilon + \Pr[U=r]) = 4\epsilon + 2^{-\kappa+1}.
\] 
The distribution $R | P=p^*  \overset{def}= \rep(V | P=p^*, p^*)$.  Thus, the overall probability $\Pr[V|P=p^* \in Q_{r, P^*}] \le 4\epsilon+2^{-\kappa +1}$.  \bnote{how to continue from here?  this doesn't talk about the size of $V|P=p^*$.}
\end{proof}
