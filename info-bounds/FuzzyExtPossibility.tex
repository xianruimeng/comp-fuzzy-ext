...
The basic hashing approach outlined above does not extend to arbitrary sources.  The reason is that some balls may have significantly more points but occur with low probability.  Let $W$ be a distribution, denote by $B_1$ a ball with $2^{\Hoo(W)}$ points but with total probability $\Pr[W\in B_1] =2^{-1/2\Hoo(W)}$ and let $B_2$ be a ball with one point with probability $\Pr[W\in B_2] = 2^{-\Hoo(W)}$.  Then the hashing algorithm outlined above it is necessary to write down $\Hoo(W)$ bits in the case when $W$ lands in $B_1$.  However, this may completely reveal the point in $B_2$.  

Dealing with non-flat distributions requires a new strategy.  Ideally, this strategy should be aware of the density of the current ball, aware that when an original reading comes from a sparse ball it is not necessary to write down many bits.  We present two strategies: leveraging convex combinations, and partitioning the probability space.

\subsection{Leveraging convex combinations?}
In many situations solving a problem for min-entropy distributions can be reduced to solving the problem for flat distributions and then using the fact that any min-entropy distribution is a convex combination of flat distributions.  For example, consider the following extension to \consref{cons:pairwise hash}.  Let $W$ be some distribution that is a finite convex combination of flat sources $W_1,..., W_n$ where each $\forall i, \Hoo(W_i)\ge k$~(a distribution may be an infinite convex combination, we will show this idea does not work even for the case of finite convex combinations).  For convenience we assume that the coefficients on all $W_i$ are the same and distributions may be repeated. 
...
%\begin{centering}
%\begin{figure}[t]
%\includegraphics[scale.5*text_width]{convexCombExample}
%\end{figure}
%\end{centering}
