...
\end{align}
For the remainder of the proof, we seek a bound on $\Hav(W | \sketch(W) \wedge 1_{\text{low}} =0).$  Let $I$ be the random variable of the level information (in what follows, $I$ ranges over $m\le i \le m+n$, where possible we omit this for notation clarity). 
...
\noindent
We now show that the fuzzy min-entropy conditioned on the level information is proportional to $\beta_{t, i}$.  
\begin{claim}
...
\end{align*}
Where the third line follows by \clref{cl:nearly flat after conditioning}, which follows.  This completes the proof of \clref{cl:fuzz proportional beta}.
\end{proof}
...
&=&\frac{\Pr[W=w_2]}{2}.
\end{align*}
Where the first and last equality follow by Bayes' rule.  The second and fourth equality follow by noting that $\Pr[W=w \wedge J=j] = \Pr[W=w]$ for all $w\in L_i$.  The inequality proceeds by assumption.  This completes the proof of \clref{cl:nearly flat after conditioning}.
\end{proof}

\noindent 
We now return to the proof of \lemref{lem:flat hashing}. Together Equation \ref{eq:fuzzy ball size} and \clref{cl:fuzz proportional beta} yield:
\begin{align*}
...
&\ge \Hfuzz(W) - 3- \log n - \log 1/\delta
\end{align*}
