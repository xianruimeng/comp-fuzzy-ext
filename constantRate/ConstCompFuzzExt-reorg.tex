...
We show an information-theoretic construction and two computational constructions with additional features.  Our constructions are for the Hamming metric.
...
The construction of Bitansky et al. is based on multilinear encoding and is  highly impractical.  Our constructions use obfuscated digital lockers.  Obfuscated digital lockers are instantiable under significant weaker assumptions and can be implemented quite efficiently.
...
Furthermore, we showed that \consref{cons:sampling} is secure on partial block sources.  In fact, it is secure for any source where sampling produces a high entropy string~($\omega(\log n)$) with overwhelming probability.  For comparison with \consref{cons:info theoretic} we restrict our attention to partial block sources.\bnote{I just added this.  Okay?}
