...
Construction & Security & Source Structure & Error-Tolerance & Alphabet Size\\
\hline
\consref{cons:info theoretic} & Info-theoretic  & Partial block source& $\Theta(\gamma)$ & $\omega(1)$ \\
\consref{cons:sampling} & Reusable Computational & Partial block source& $o(\gamma)$ & $\omega(1)$ \\
\consref{cons:first construction} & Computational & Unguessable block source & $\Theta(\gamma)$ & $n^{\omega(1)}$
\end{tabular}
\caption{Summary of new constructions.  All constructions support families of distributions with more errors than entropy.  Partial blocks sources are defined in \defref{def:partial source} and unguessable block sources are defined in \defref{def:block guessable}.}
...
\item \textbf{Perturbation} For $i=2,..., q$: the challenger computes $(r_i, p_i)\leftarrow \gen(w_i)$.  Set $w_{i+1} = f_i(w_1, p_1,..., p_i)$.% The output $p_i$ is provided to $\mathcal{A}$, who selects $\delta_i:\mathcal{M}\rightarrow \mathcal{M}$.  The challenger sets $w_{i+1} = \delta_i(w_1)$.
...
Insider security strengthens outsider security in two ways.  First, it allows the adversary to see some subset of keys, second it allows the adversary to perform regeneration on arbitrary $p_i$.  This mixes two properties of a fuzzy extractor:  reusability and robustness~\cite{dkrs2006}.  Robust fuzzy extractors provide security against modified $p$.  
In this work, we show reusability when some $r_i$ are observed but do not handle the issue of robustness.  That is, we assume passive servers.  Our definition lies between outsider and insider security.
