...
Let $\mathcal{W}$ be the family of $(\alpha, \beta)$-partial block sources over $\mathcal{Z}^\ell$ and let $\cond: \mathcal{Z} \times \zo^d\rightarrow \mathcal{Y}$ be a $(\alpha, \tilde{\alpha}, \epsilon)$-average case randomness conductor.  Let $(\sketch, \rec)$ be $(\mathcal{Y}^\ell, \tilde{\alpha}(\ell - \beta), \tilde{m}, t)$-secure sketch with error $\delta$.~(Convertible to a fuzzy extractor using~\cite[Lemma 4.1]{DBLP:journals/siamcomp/DodisORS08}).
...
We now show \consref{cons:sampling} supports partial block sources with $\Huse\le 0$.  The required entropy of partial block source is $\alpha (\gamma-\beta ) = \Theta(\gamma)$.  We are able to correct $o(\gamma)$ errors.
This yields:
\[
