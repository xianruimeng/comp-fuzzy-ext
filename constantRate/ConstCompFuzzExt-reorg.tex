...
We construct the first fuzzy extractors that work for a large class of distributions that have negative minimum usable entropy. We present both information-theoretic and computational constructions.In order to avoid the worst-case loss, they necessarily restrict distributions for which they work.  

Our first construction is information-theoretic and corrects a constant fraction of Hamming errors over a large alphabet.  In this construction each symbol must contribute fresh entropy.  We then use computational techniques to obtain two constructions with additional properties.
...
Our third construction corrects a constant fraction of Hamming errors over a large alphabet. It requires high individual entropy of a constant fraction of symbols, but permits symbols to be dependent.  
...
The correctness guarantee is that $r$ will be correctly reproduced by \rep as long as $w'$ is no farther than $t$ from $w$ in some metric space (in this paper, we focus on the Hamming metric on length $\ell$ strings over some alphabet $\mathcal{Z}$).
...
Thus, if the security guarantee of a given fuzzy extractor holds for \emph{any} source of a given min-entropy $m$ and the correctness guarantees holds for any $t$ errors, then $m$ must   be greater than $\log B_t$, where $B_t$ denotes the number of points in a ball of radius $t$.  If a source fails this condition, we will says that it has \emph{more errors than entropy}.
...
\paragraph{Our Contributions: Handling Sources with More Errors than Entropy}
We provide the first constructions of fuzzy extractors that can be used for large classes of sources that have more errors than entropy.  Our constructions work for Hamming errors for strings $w$ of length $\ell$ over some alphabet $\mathcal{Z}$. Naturally, as argued above, these constructions cannot work for all sources of a given entropy; each construction comes with a constraint on the sources for which it is secure.
...
We then switch to computational security to obtain constructions with additional features. Our second construction works for the same class of distributions as the first, but also provides reusability. The reusability we obtain is very strong:  security holds even if the multiple readings $w_i$ used in $\gen$ are \emph{arbitrarily correlated}, as long as each $w_i$ \emph{individually} satisfies the above conditions as a distribution.
We note that such strong reusability was shown to be impossible in information-theoretically secure fuzzy extractors~\cite[Theorem 11]{Boyen2004}.\lnote{really? I can't parse his theorem clearly enough} This construction, however, requires that the fraction of errors is sub constant.
...
\noindent Randomness condensers can be built  more efficiently than randomness extractors~(for example~\cite{dodis2014key}).  
...
%\end{align*}
%\end{definition}
Our definition is a modified form of the definition of Boyen~\cite[Definition 7]{Boyen2004}.  We first present the modified definition and then discuss the differences between our definition and that of Boyen.
\begin{definition}[Outsider Reusable Fuzzy Extractors]
\label{def:outsider fuzz ext}
Let $\mathcal{W}$ be a family of distributions over $\mathcal{M}$.  Let $f_2,.., f_q , D$ be a split adversary.  Define the following game~(for each $W_1\in \mathcal{W}$ and for all $i=1,..., q$):
\begin{itemize}
...
\[
\epsilon_{sec}\overset{def}= \Pr[D(r_1,..., r_{i-1}, r_i, r_{i+1},..., r_q, p_1,..., p_q)=1] - \Pr[D(r_1,..., r_{i-1}, u, r_{i+1},..., r_q, p_1,..., p_q)=1].
\]
where $u$ is a uniformly random string.
\end{itemize}
Let $(\gen, \rep)$ be a $(\mathcal{M}, \mathcal{W}, \kappa, t)$-computational fuzzy extractor that is $(\epsilon, s)$-hard with error $\delta$.  We say that $(\gen, \rep)$ is $(q, s_{sec}, \epsilon_{sec}, f_2,..., f_q)$-reusable if for all $D$ of size at most $s_{sec}$ they win the game with advantage at most $\epsilon_{sec}$.
\end{definition}

\paragraph{Adapting the definition of Boyen to the computational setting:} 
...
In this section we show an information-theoretic fuzzy conductor that supports more errors than entropy.
...
The main idea of this construction is to condense entropy from each block of the source and then apply a secure sketch to the condensed blocks.  The main idea is to correct errors on a large alphabet~(before condensing) while paying for the error correction on a smaller alphabet~(after condensing).%Alternatively, this construction can simply be seen as replacing the computational point function obfuscation with an information-theoretic point-function obfuscation~(this is only possible for high enough entropy levels).
...
\end{enumerate}
\item Set $ss \leftarrow \sketch(v_1,..., v_\gamma)$.
\item Set $p = (ss, seed_1,..., seed_\gamma)$.
\item Output $(v, p)$.
\end{enumerate}
 \end{minipage} &
\begin{minipage}{3in}
\textbf{\rep}
\begin{enumerate}
\item \underline{Input}: $(w', p = (ss, seed_1,..., seed_\gamma))$
\item For $j=1,..., \gamma$:
\begin{enumerate}[(i)]
\item Set $v_i = \cond(w_i, seed_i)$.
\end{enumerate}
...
Let $\mathcal{W}$ be the family of $(\alpha = \Omega(1), \beta\leq \gamma(1-\Theta(1)))$-partial block sources over $\mathcal{Z}^\gamma$ wand let $\cond: \mathcal{Z} \times \zo^d\rightarrow \mathcal{Y}$ be a $(\alpha, \tilde{\alpha}, \epsilon)$-average case randomness conductor.  Let $(\sketch, \rec)$ be $(\mathcal{Y}^\gamma, \tilde{\alpha}(\gamma - \beta), \tilde{m}, t)$-secure sketch with error $\delta$.~(Convertible to a fuzzy extractor using~\cite[Lemma 4.1]{DBLP:journals/siamcomp/DodisORS08}).

\end{lemma}

\subsection{When is $\Huse\le 0$?}

\consref{cons:info theoretic} supports partial block sources with $\Huse\le 0$.  The required entropy of partial block source is $\alpha (\gamma-\beta ) = \Theta(\gamma)$.  The condenser of Dodis et al~\cite{dodis2014key} has a constant entropy loss, so $\alpha-\tilde{\alpha} = \Theta(1)$.  This means that the input entropy to the secure sketch is $\Theta(\gamma)$.  Standard secure sketches correct a constant fraction of errors at a entropy loss, yielding $\tilde{m} = \Theta(\gamma)$.  Thus, our construction is secure for distributions with more errors than entropy whenever $|\mathcal{Z}| = \omega(1)$.
More formally:
\[
\Huse(W) = \Hoo(W) -\log |B_t| < \Theta(\gamma)- t \log |\mathcal{Z}|= \Theta(\gamma) - \Theta(\gamma) \log |\mathcal{Z}| <0
\]
That is, there exists a super-constant alphabet size for which \consref{cons:info theoretic} is secure with $\Huse\le 0$.  Furthermore, the construction has super-logarithmic output entropy since $\tilde{m} = \Theta(\gamma) = \omega(\log n)$.

\section{Achieving Reusability \& $\Huse\le 0$}
\label{sec:sampling}
In the previous section, we showed it was possible to construct a fuzzy extractor for a family of distributions with $\Huse \le 0$.  In this section we show that computational techniques allow for additional desirable properties.  Using computational techniques we are able to retain many of the advantages of \consref{cons:info theoretic} and achieve a reusable fuzzy extractor.  This is known to be impossible in the information-theoretic setting.  
...
The main idea is to sample a random subset of blocks $W_{j_1},..., W_{j_\eta}$ and obfuscate the concatenation of these blocks.  Denote this concatenated value by $V_1$.  This process is repeated to produce $V_1,..., V_\ell$ where at least one $V_i$ should be correct to ``unlock'' the correct key.  
...
In this section we briefly describe why \consref{cons:sampling} is secure.  A proof of security appears \apref{sec:analysis sampling}.  
In order to show security \consref{cons:sampling}, we
show that with overwhelming probability, at each of the $\ell$ iterations, the sampler will choose enough coordinates of $W$ that have high entropy, making $V_i$ have sufficient entropy.   Once each of the $V_1,..., V_\ell$ have high entropy the obfuscations are unlikely to return a value other than $\perp$ to an adversary.
...
Once all $V_i$ all simultaneously have good entropy, the adversary will never see anything but $\perp$ as an output from the obfuscations~(with overwhelming probability).  If the adversary only sees $\perp$ from the obfuscations, they have no information about the key.  
...
\bnote{almost certainly need to say more here.  expand}
\end{proof}

\subsection{When is $\Huse\le 0$?}
\consref{cons:sampling} supports partial block sources with $\Huse\le 0$.  The required entropy of partial block source is $\alpha (\gamma-\beta ) = \Theta(\gamma)$.  We are able to correct $o(\gamma)$ errors.
This yields:
\[
...
In the previous two sections, we presented constructions that supported $\Huse\le 0$ for restricted classes of distributions.  Unfortunately, both Constructions~\ref{cons:info theoretic} and \ref{cons:sampling} required each symbol to contribute ``fresh'' entropy.  In this section, we present a computational construction that allows for correlation between symbols while still supporting $\Huse \le 0$ and correcting a constant fraction of errors.
This construction is inspired by the construction of digital lockers from point obfuscation by Canetti and Dakdouk~\cite{canetti2008obfuscating}.  Before presenting the construction we provide some definitions from error correcting codes.
We use slightly nonstandard error-correct codes over $\{0,1\}^\ell$, which correct up to $t$ bit flips from $0$ to $1$ but no bit flips from $1$ to $0$ (this is the Hamming analog of the $Z$-channel~\cite{tallini2002capacity}).\footnote{Any code that corrects $t$ Hamming errors also corrects $t$ $0\rightarrow 1$ errors, but more efficient codes  exist for this type of error~\cite{tallini2002capacity}.
...
The following theorem states the construction is both correct and secure.  %After proving this theorem we discuss when \consref{cons:first construction} is useful on its own.
\begin{theorem}
...
\]
The simplest type of unguessable block distribution is where each block is independent and has super-logarithmic entropy~(\clref{cl:independent high ent}).  For this type of source the required entropy is $\Hoo(W) = \ell\omega(\log n)$.  This yields:
\[
\Huse (W) = \Hoo(W) - \log |B_t| < \ell \omega(\log n) -\left( \Theta(t\log |\mathcal{Z}|) + \log {\ell \choose t}\right).
\]
...
We now argue security.  Denote by $seed$ the random variable consisting of all $\gamma$ seeds and $V$ the entire string of generated $V_1,..., V_\gamma$.
Our goal is to show that $\Hav(V | p)$ is close to a distribution with average min-entropy $\tilde{m}$.  We first argue that there exists a distribution $Y$ where $\Hav(Y | seed)\ge \tilde{\alpha}\beta$ and $(V, seed_1,..., seed_\gamma)\approx (Y, seed_1,.., seed_\gamma)$.  First note since $W$ is $(\alpha, \beta)$-partial block distribution that
...
\noindent
By a hybrid argument there exists a distribution $Z$ with $\Hav(Z | seed) \ge \alpha(\gamma -\beta)$ where 
\[
(\cond(W_\gamma, seed_\gamma), ..., \cond(W_1, seed_1), seed_\gamma,..., seed_1) \approx_{\epsilon\gamma} (Z, seed_\gamma,...,  seed_1).\]
...
That is, 
\[
(V, seed_1,..., seed_\gamma, p) \approx_{\epsilon\gamma} (U_{\mathcal{Y}}^\gamma, seed_1,..., seed_\gamma, p).\]  This completes the proof.
\end{proof}
...
%be some particular outcome.
%
%Intuitive once all $V_i$ all simultaneously have good entropy, then the obfuscations will never be unlocked.
%%In ~\clref{cl:all blocks entropy} we show that any distribution where each individual block has super-logarithmic min-entropy forms a unguessable block distribution.  
