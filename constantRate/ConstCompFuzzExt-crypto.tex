...
\label{sec:sampling}
Let $w$ be a string of symbols (over an arbitrary alphabet) $w=w_1\dots w_\gamma$. The construction first chooses a random fuzzy extractor output $r$. It then samples a random subset of blocks $v_1=w_{j_1},..., w_{j_\eta}$ and creates a digital locker that hides $r$ using $v_1$.  This process is repeated to produce $\ell$ digital lockers all containing $r$,  each unlockable with $v_1,..., v_\ell$, respectively. Let $\sample_{\gamma, \eta}(\cdot)$ denote an algorithm that  outputs a random subset of $\{1,..., \gamma\}$ of size $\eta$ given let $r_{sam}$ bits of randomness.
...
%\end{itemize}
The use of the composable digital lockers allows us to sample multiple times, because we need to argue only about individual entropy of $V_i$.  Composability also allows reusability.

This construction can use the na\"{i}ve sampler that takes truly random samples. If desired, public randomness may be substantially decreased by using more sophisticated samplers. (See Goldreich~\cite{goldreich2011sample} for an introduction to samplers.)
...
\section{Construction for Sources with High-Entropy Marginals}
