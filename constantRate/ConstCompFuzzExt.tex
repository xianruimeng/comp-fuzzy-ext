...
\lemref{lem:fuzz necessary} generalizes to interactive protocols, $D$ only provides an input to the protocol and looks at the output.  This means that fuzzy min-entropy is a necessary condition for an interactive solution as well.  The original fuzzy extractors paper of Dodis et al.~\cite{DBLP:journals/siamcomp/DodisORS08} separated the starting entropy of $W$ and the desired error tolerance.  When $gap<0$ there is some distribution such that $\Hfuzz(W)=0$.  This means we inherently must talk about the security and errors together.  
