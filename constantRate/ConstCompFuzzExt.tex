...
\[
\expe_{View(A)} \Pr[C=c | C_{\mathcal{I}^c} \wedge View(A)] \leq \Pr[C=c | C_{\mathcal{I}^c}]+\ngl(n).
\]
\end{lemma}
\begin{proof}
...
Let $c^*$ be an arbitrary $c^*$ in the support of $C|C_{\mathcal{I}^c}$.  Suppose that there exists some polynomial $1/\poly(n)$ such that 
\begin{align*}
\expe_{view \leftarrow View(A^{\mathcal{O}_{(X_1,..., X_\ell)}})}\Pr[C=c^* | C_{\mathcal{I}^c} \wedge view ] &\geq \Pr[C=c^* | C_{\mathcal{I}^c} ]+ \frac{1}{\poly(n)} \\
\end{align*}
This means that the difference between the expected probability of $c_{min}$ and $c^*$ is at least $1/\poly(n)$.  Thus, there is an unbounded distinguisher that can tell between the two cases, violating the statistical distance property of   \lemref{lem:codewords in I close}.  This is a contradiction and completes the proof.
\end{proof}
\begin{corollary}
\label{cor:avg min after view}
$\Hav(C| View(A))\geq \Hav(C | View (A) \wedge C_{\mathcal{I}^c}) \geq \min \{ \Hav(C | C_{\mathcal{I}^c}), \omega(1) \log n\}= \min\{ |C| - |\mathcal{I}^c|,  \omega(1) \log n\}$.
\end{corollary}
\begin{proof}
...
\Hav(C_i |  View(A^{\mathcal{O}_{(X_1,..., X_\ell)}(\cdot)}))
 ...
 &\approx \min \{-\log P_c, -\log \ngl(n)\} \\
 &= \min \{ \Hav(C | C_{\mathcal{I}^c}), \omega(1) \log n\}
...
\end{align}
\bnote{If we are more precise with the analysis above we can probably make this $\ngl$ more precise and get better bounds. For example if each block has linear entropy this should be quite good.  Also we are doing point wise analysis, can we do better by considering each part of the code?}
\end{proof}
%\begin{lemma}
...
\begin{corollary}
Let $\mathcal{W}$ be the family of block distributions where $W_i$ are independent and $\Hoo(W_i) \geq \omega(\log n)$.  Then for $s_{sec} = \poly(n)$ there exists some $\epsilon=\ngl(n)$ such that \consref{cons:first construction} is a $(\zo^{n\times \ell}, \mathcal{W}, \tilde{m}, t)$-computational fuzzy conductor that is $(\epsilon, s_{sec})$ with error $\epsilon_{code} + \ell/2^n$ and resulting entropy $\tilde{m} =\min\{ |C|, \omega(1)\log n\}$.
\end{corollary}
\begin{proof}
...
\begin{theorem}
Let $\mathcal{W}$ be a family of $(q, \omega(\log n),  \beta)$-unguessable distributions over $\zo^{n\times \ell}$ for any $q = \poly(n)$.  Furthermore, let $C$ be a $(\neigh_t, \epsilon_{code})$-code.  Then for $s_{sec} = \poly(n)$ there exists some $\epsilon=\ngl(n)$ such that \consref{cons:first construction} is a $(\zo^{n\times \ell}, \mathcal{W}, \tilde{m}, t)$-computational fuzzy conductor that is $(\epsilon, s_{sec})$ with error $\epsilon_{code} + \ell/2^n$ and resulting entropy $\tilde{m} = \min\{\log |C| - \beta, \omega(1)\log n\}$.
\end{theorem}
