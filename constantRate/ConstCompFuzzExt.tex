...
We discuss unguessable block distributions in \apref{sec:characterize}.  
...
\label{sec:sampling}
\bnote{Leo didn't like this linking.  How about now?}
...
Unfortunately, obfuscating multiple blocks together decreases the effective error tolerance.  Thus, we have a tradeoff between entropy and error tolerance.  The main idea is to sample a random subset of blocks $W_{i_1},..., W_{i_\eta}$ and obfuscate the concatenation of these blocks.  Denote this concatenated value by $V_1$.  This process is repeated to produce $V_1,..., V_\ell$ and the construction proceeds by either obfuscating $V_i$ or a random point as before. For security each value $V_i$ needs to be unguessable.  This will hold as long as enough blocks we sample contribute some entropy:
...
\label{thm:sampling}
Let $n$ be a security parameter.
Let $\mathcal{W}$ be the family of $(\alpha = \Omega(1), \beta\leq \gamma(1-\Theta(1)))$-unordered block sources over $\mathcal{Z}^\gamma$ where $\gamma = \Omega(n)$.   Let $\eta$ be such that $\eta = \omega(\log n)$ and $\eta = o(\gamma)$,  and $\ell$ be such that $\ell = O(\poly(n))$ and
...
\end{claim}
\begin{proof}
We begin by noting that $\Hoo(V_i |\Lambda = \lambda ) = -\log \max_{v\in V_i} \Pr[ V_i =v | \Lambda =\lambda]$.  Denote by $j_1,..., j_\eta$ the indices selected by the randomness $\lambda_i$.  Then 
\begin{align*}
\max_{v\in V_i}\Pr[V_i = v | \Lambda = \lambda ]&= \max_{w_{j_1},..., w_{j_\eta}} \Pr[ W_{j_1}=w_{j_1} \wedge \dots \wedge W_{j_\eta} = w_{j_\eta}]\\
%&=\max_{w_{j_1},..., w_{j_\eta}} \Pr[W_{j_\eta} = w_{j_\eta} | W_{j_{\eta-1}} =w _{j_{\eta-1}} \wedge ... \wedge W_{j_1} =w_{j_1}]\\
%&\times \Pr[W_{j_{\eta-1}} = w_{j_{\eta-1}} \wedge ... \wedge W_{j_1} = w_{j_1}]\\
