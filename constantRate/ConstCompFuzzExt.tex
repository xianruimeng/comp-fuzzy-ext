...
In order to show security \consref{cons:sampling}, we show that with overwhelming probability each of the selected $v_i$ has entropy on its own.  This means that  $V_1,..., V_\ell$ is a block-unguessable distribution and security follows by \thref{thm:security of cons}.  Consider a source with entropy $\lambda$.  The expected number of selected blocks that have entropy at least $\alpha$ is $\frac{\gamma-\beta}{\gamma} \times \beta$.  A good sampler selects a within a constant fraction of the expected number of these blocks.

\begin{lemma}
\bnote{write a lemma about this}
\end{lemma}
%\begin{align*}
...
\] 
then for all $i$ and for all values of $j_{1,1},..., j_{\ell, \eta}$ the distribution $\delta(V_i | j_{1,1},..., j_{\ell, \eta} , V'_i ) <\epsilon_{sam}$ where $\Hoo(V_i') = \omega(\log n)$.\bnote{I think $\epsilon_{sam}$ will be a negligible for resultant entropy of $O(\beta \times \alpha \times (\mathcal{I}-\beta)/\mathcal{I})$.}
\end{theorem}
