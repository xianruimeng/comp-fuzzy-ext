...
\section{Reducing Alphabet Size}
\consref{cons:first construction} is a computational fuzzy extractor with $gap<0$.  Unfortunately, each block is individually obfuscated causing the blocks to be of super-polynomial size~(our analysis was in the case where the blocks where exponential in the security parameter).  In this section, we present a generalization of \consref{cons:first construction} where the values being obfuscated are sampled from the blocks of the source, but blocks are not individually obfuscated.  Unfortunately, obfuscated multiple blocks together decreases the effective error tolerance.  
\begin{construction}
\label{cons:sampling}
Let $n$ be a security parameter.
Let $Z$ be an alphabet where $|Z| = \Omega(n)$, and let $W = W_1,..., W_\gamma$ be a source where each $W_i$ is over $Z$.  Let $t$ be the desired error tolerance and let $C\subset \zo^\ell$ be a $(\neigh_t, \epsilon_{code})$ code.  Let $\eta = \omega(\log n)$ be some function such that $\eta \log (Z) = \omega(\log n)$.  Let $\mathcal{O}$ be a $\ell$-composable obfuscator for the family of point functions over $\zo^\eta$.  Define $\gen, \rep$ as:
...
\item For $i=1,..., \ell$:
\begin{itemize}
\item Sample $j_{i, 1},..., j_{i, \eta}\overset{\$}\leftarrow \{1,..., \gamma\}$
\item If $c_i = 0$: 
...
The main change in \consref{cons:sampling} is that the obfuscated values are concatenated alphabet symbols of $W$.  In the setting where each symbol of $W$ is long enough for secure obfuscation, \consref{cons:sampling} is essentially the same as \consref{cons:first construction}.  As before we must show security and correctness of \consref{cons:sampling}.  For security, the main argument is that each of the $v_i$ created has enough entropy to be unguessable.  We show this in the setting of block fixing sources~\cite{KZ07}.  This argument comes from the sampling literature~(used to create locally computable randomness extractors~\cite{lu2002hyper,vadhan2003constructing}).  For correctness we must show that the probability for each symbol $v_i \neq v_i'$ is a small constant so that $c'$ will be corrected to $c$.

\subsection{Security of \consref{cons:sampling}}
In order to show security \consref{cons:sampling}, we show that with overwhelming probability each of the selected $v_i$ has entropy on its own.  Consider a source with entropy $\lambda$.  Then the expected entropy of 
\begin{align*}
...
\end{align*}
This value is super-logarithmic when $\lambda = \omega (\frac{\gamma \log n}{\eta})$.
