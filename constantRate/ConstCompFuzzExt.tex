...
&\le 1\times 2^{-n+\log q} + 1\times  \Pr[\text{non zero response location }j \wedge c_j^*=0] \label{eq:ways to remove ent}
\end{align}
...
\end{claim}
\begin{proof}
It suffices to show that there exists a simulator $S$ making $q$ queries such that the remaining entropy in any block is at most $n-\log q$.  Let $W_j$ be a distribution. Consider $S$ that asks the $q$ most likely outcomes. The total probability\lnote{what does total probability mean?} of this block\lnote{symbol?} must be at least $2^{-n+\log q}$.  After these queries the remaining min-entropy is at most:
\begin{align*}
\Hav(W_j | View(S)) &\leq  -\log \left(\Pr[\text{some }q_i=1]\times 1+ \Pr[\text{no }q_i=1]\times \Pr[\text{most likely outcome}|q_1,...,q_q]\right)\\
...
\begin{align*}
 \Pr[\text{non zero response location }j \wedge c_j=0] &>2^{-\alpha+1} - 2^{-(n-\log q)}=  2^{-\alpha}
 \end{align*}
