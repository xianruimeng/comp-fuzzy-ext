...
\defref{def:unordered source} is a generalization of block sources~(introduced by Chor and Goldreich~\cite{DBLP:journals/siamcomp/ChorG88}) where blocks must have entropy conditioned on the value of previous blocks.  An unordered block source is also a strengthening of an unguessable block distribution~(\defref{def:block guessable}).  An unordered block source requires there to be worst case entropy while an unguessable block distribution only requires average conditional entropy.  However, we achieve security for significantly lower entropy levels than were needed for unguessable block distributions.
...
\[
\Pr[\Hoo(V_i | \Lambda) \geq \alpha'] \geq 1- \epsilon_{sam}.
\]
\end{lemma}
\begin{proof}
