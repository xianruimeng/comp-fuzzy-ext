...
\begin{itemize}
\item Assuming a balanced code $C$~(where on average, half of the bits of a codeword are 0 and half are 1), in expectation, half of $w$ is information-theoretically unknown conditioned on $p$.  As described in~\cite[Section 3.3]{fuller2013computational} some type of lossiness is necessary to avoid coding bounds.
...
\end{lemma}
\begin{lemma}
If $C$ is a balanced code then, for all $i$, $\Hoo(C_i |View(A^{\mathcal{O}_{(X_1,..., X_\ell)}(\cdot, \cdot)}))\geq 1 - \omega(1)(\log n)$.
\end{lemma}
\begin{proof}
From Lemma 3.4\bnote{change to reference}, we have that in expectation each for each $i$, $\Pr[C_i =1 ] = 1/2$. Let $w$ be the exponent of the particular negligible value above~(note $w = \omega(1)$).  Then we have for $b\in\{0,1\}$
\begin{align*}
...
&=1-\left(\log (n^w + 2) - \log n^w\right)\\
&\geq 1- \left(\log (n+1)^w - \log n^w\right)  \\
&= 1- w\left( \log (n+1) - \log n\right)\\
&\geq 1 - w*1
\end{align*}
\end{proof}
\begin{lemma}
...
\end{lemma}
\begin{corollary}
$\Delta(\ext(C, seed) , U | seed, View(A^{\mathcal{O}_{(X_1,..., X_\ell)}(\cdot, \cdot)}))\leq \ngl(n)$.
\end{corollary}
