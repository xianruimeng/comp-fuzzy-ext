...
\end{align}
\bnote{I think the right thing to say here is that the entropy is $\geq \max\{ |C|, \omega(1)\}$ since the $\ngl$ term probably dominates the above.  If we are more precise with the analysis above we can probably make this $\ngl$ more precise and get better bounds.}Equation~\ref{eq:taylor approx} is derived using the Taylor expansion for $\ln(1+x)$, Equation~\ref{eq:taylor trunc} is derived by noting it is a geometric series and thus all terms $i=3,....$ are contained in the second term $i=2$ which is positive, and Equation~\ref{eq:min ent} is derived using \clref{cl:many locations ent}.
\end{proof}
%\begin{lemma}
...
\begin{definition}
Let $W = W_1,..., W_\ell$ be a source under ordering $i_1,..., i_\ell$.  The \emph{entropy jump} of a block $i_j$ is $J(i_j) = \Hav(W_{i_j} | W_{i_1},..., W_{i_{j-1}})$.
\end{definition}
