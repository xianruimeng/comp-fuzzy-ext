...
\end{lemma}
\begin{proof}
It suffices to show that $H^{\unp}_{0, s_{sec}}( X | \sketch(X)) < b-t + \log (1-\delta)$.  That is, it suffices to construct $D$ such that $\Pr[D(\sketch(X)) = X] > 2^{-b+t} (1-\delta)$.  We describe $D$:
\begin{enumerate}
...
Now 
\[
\Pr[D(\sketch(X)) = X] \geq \Pr[Z \in B_t(X)] \Pr[\rec(Z, \sketch(X)) =X | \dis(Z, X)\leq t] = 2^{-b+t}(1-\delta)
\]
